...
const _paymentMethodList = computed(() => {
  if (comboList.value?.method && comboList.value.method.length > 0)
    return comboList.value.method.filter(a => a.alias === 'qb')
  else
    return []
})
  ...
  <div class="pay-list-wrap flex flex-col gap-[16px]">
    <!-- 选择支付方式 -->
      <template v-if="renderList?.length ?? 0 > 0">
        <div class="flex flex-col rounded-[4px]">
          <!-- 线上支付列表 -->
          <div
      ...
      <template v-else>
        <div class="border-tg-text-lightgrey w-full border-[2px] rounded-[4px] border-dashed  py-[20px]">
