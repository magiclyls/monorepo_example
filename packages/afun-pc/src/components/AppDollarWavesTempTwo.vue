...
// const zonOffset = currentTime.getTimezoneOffset()
// const localTimeStamp = ref(currentTime.getTime())
...
const qaList = computed(() => [
  { q: t('dollar_qa_q_0'), a: t('dollar_qa_a_0') },
  { q: t('dollar_qa_q_1'), a: t('dollar_qa_a_1') },
  { q: t('dollar_qa_q_2'), a: t('dollar_qa_a_2') },
  { q: t('dollar_qa_q_3'), a: t('dollar_qa_a_3') },
  { q: t('dollar_qa_q_4'), a: t('dollar_qa_a_4') },
  { q: t('dollar_qa_q_5'), a: t('dollar_qa_a_5', { n: detailData.value.multiple }) },
])
...
<template>
  <div class="dollar-waves text-tg-secondary-light m-auto mb-[32px] max-w-[650px] pb-[16px] font-medium @md:mt-[32px]" :class="[`lang-${lang}`]" @touchstart="pageClicked" @mousemove="pageClicked" @click="pageClicked">
    ...
    <div class="activity-desc bg-tg-secondary-dark theme-bg my-[16px] rounded px-[12px] py-[17px]">
      ...
      <div class="text-tg-text-lightgrey w-full text-[14px] font-medium leading-[1.5]">
        <ul class="w-full flex flex-col list-disc pl-[16px]">
          ...
          <li>
            <span>{{ t('time_zone_label', { timezone: t(TIMEZONE_LABEL_KEY[currentDollarZone]) }) }}</span>
          </li>
        </ul>
      </div>
    </div>
    <div v-if="timeScopes && timeScopes.length" class="theme-bg bg-tg-secondary-dark bonus-time my-[16px] rounded px-[12px] py-[17px]">
      ...
      <section class="all-box gap-y-[6px] md:gap-y-[16px]">
        <template v-for="(i, idx) in timeScopes" :key="i + idx">
          <div>
            <div
              class="b-time-item relative mx-auto h-[34.66vw] max-h-[130px] max-w-[110px] w-[30.8vw] cursor-pointer text-center font-semibold text-[#FFDD64]"
              :class="[
            ...
            >
              <div class="whitespace-nowrap pt-[29px] text-[12px] leading-[16px]">
              ...
              <div class="mt-[39px] text-[18px] leading-[25px]">
                {{ i[0] }}
              </div>
            </div>
          </div>
...
.b-time-item {
  ...
  @include getBackgroundImage('/promotions/bonus-time-1');
  // &.mounted-active-tag {
  //   background-image: url(/webp/promotions/dollar_bg.webp);
  // }
  &.has-b {
    @include getBackgroundImage('/promotions/bonus-time-2');
  }
  &.bonus-received.yes-received {
    @include getBackgroundImage('/promotions/bonus-time-0');
  }
  &.mounted-active-tag {
    .lucky-hand {
      display: block;
