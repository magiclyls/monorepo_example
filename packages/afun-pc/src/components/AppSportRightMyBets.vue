...
function settleChange() {
  appEventBus.emit(EventBusNames.SPORTS_BETTING_TO_BETSLIP_BUS, settle.value)
}
</script>
...
<template>
  <div class="app-sport-right-my-bets">
      ...
      <div class="scroll-y scrollable center-content">
        ...
        <div v-else class="h-min-100">
          <div v-if="sportBetList.length">
            <AppSportsMyBetSlip v-for="item in sportBetList" :key="item.ono" :data="item" disable-result />
          </div>
    ...
    <footer class="footer">
      <!-- 我的投注 -->
      <BaseButton size="md" @click="toMyBets">
        {{ t('view_all') }}
      </BaseButton>
    </footer>
  </div>
