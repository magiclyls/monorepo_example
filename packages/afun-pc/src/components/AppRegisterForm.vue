...
import { ApiMemberBrandDetail, ApiMemberExists, ApiMemberReg, ApiMemberSendMailCaptcha } from '@tg/apis'
...
const birthday = ref('')
const parentUid = ref(router.currentRoute.value.query.u)
const parentC = ref(router.currentRoute.value.query.c)
...
async function getMemberReg(callBack: (data: any) => void) {
  ...
  if (!usernameErrorMsg.value
    ...
    const paramsReg = {
      ...
      parent_uid: parentUid.value || (Local.get(STORAGE_UUUUU)?.value || ''),
...
<template>
  <div class="app-register">
    ...
    <div class="app-register-check-box">
      <BaseButton
        class="app-register-btn" bg-style="primary"
        size="xl"
        :loading="isLoading"
