...
/** 更新二维码的相关数据 */
function updateVirAddress(virDepositList) {
  if (virDepositList && virDepositList.length > 0) {
...
const defaultCurrency = computed(() => {
  ...
  const routeCurrencyItem = getCurrencyConfigByCode(routeCurrency)
  const initVirCurrencyCode: CurrencyCode = (routeCurrency && application.isVirtualCurrency(routeCurrencyItem.name)) ? routeCurrency : virCurrencyListOptions.value[0]?.currency_id
...
application.allSettled([ runAsyncFinanceCoinContractList()/* ,loadingPromise */])
</script>
