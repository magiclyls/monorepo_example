...
<template>
  ...
  <div class="app-vip-progress">
    <div class="percent-top">
      ...
      <p class="percent">
        <slot name="content" />{{ toFixed(progress > 100 ? 100 : progress) }}%
      </p>
    </div>
    <div class="percent-mid">
      <BaseProgress
        ...
        :show-info="false"
        :stroke-width="6"
        stroke-color="var(--tg-vip-stroke-color)"
      />
    </div>
    <!-- <div class="percent-btm">
      <div>
        <BaseImage width="40px" :is-network="true" :url="`/images/vip/${vip}.webp`" />
        <span>{{ `VIP${vip}` }}</span>
      </div>
      <div v-if="!isMaxLevel">
        <BaseImage width="40px" :is-network="true" :url="`/images/vip/${nextLevel?.level}.webp`" />
...
:root {
  ...
  --tg-vip-stroke-color: var(--tg-primary-success);
}

[theme="greenblack"] {
  --tg-vip-stroke-color: var(--tg-base-progress-active-bg);
}

[theme="white"] {
  --tg-vip-stroke-color: #F2CA5C;
}
</style>
