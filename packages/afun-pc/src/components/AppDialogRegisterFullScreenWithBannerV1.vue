...
import { IconUniArrowUpSmall, IconUniWarningColor, IconUniArrowUpSmall2, IconUniClose } from '@tg/icons'
/**
 * 与V2的不同点：
...
// }
defineOptions({
  name: 'AppDialogRegisterFullScreenWithBannerV1',
})
...
const { isRegisterDialogOpen } = useGlobalStateLoginRegisterDialogOpen()
...
const {
  ...
  onSuccess: async (res) => {
    // 在关闭弹窗前保存用户名来用发提示语
    const _userNameField = curParams.value?.username
...
<template>
  <div class="register-form">
    <AppFullScreenLoginAndRegisterBanner class="mb-[6px]" />
    <div class="grid gap-y-[12px]">
      <h6 class="text-tg-text-white px-[20px] text-[20px] font-semibold leading-[25px]">
        {{ t('reg') }}
      </h6>
      <!-- 表单 -->
      <div v-if="needPhone" class="flex flex-col px-[20px]">
        <div class="theme-input theme-inner-input relative flex" style="--tg-input-lefticon-px: 0;--tg-input-lefticon-py: 0;">
          <BaseInput ref="phoneInputRef" v-model="phone" hide-spin-btn input-mode="numeric" type="number" :placeholder="t('new_phone')" @blur="onEmailUsernameBlur(3)">
            <template #left-icon>
              ...
              <div class="theme-block h-[38px] w-[10px]" />
            </template>
          </BaseInput>
        </div>
        <div
      ...
      <BaseInput
        v-if="needEmail"
        ref="emailRef"
        v-model="email" class="theme-input px-[20px]" :placeholder="t('mail')" :msg="emailErrorMsg"
        msg-after-touched type="email" name="email" @blur="onEmailUsernameBlur(2)"
      />

      <div v-if="needUsername" class="theme-input flex flex-col px-[20px]">
        <BaseInput
      ...
      <BaseInput
        ref="passwordRef"
        v-model="password" class="theme-input px-[20px]" :placeholder="t('login_pwd')" type="password" name="password"
        autocomplete="current-password" msg-after-touched :msg="pwdErrorMsg" @focus="onPasswordFocus"
        @blur="onPasswordBlur"
      />
      <AppPasswordVerify
        v-show="password"
        class="px-[20px]" :password="password" :has-error-msg="!!pwdErrorMsg" :pwd-touched="pwdTouched"
        type="row" @pass="passwordVerifyPass"
      />
      <!-- CPF -->
      <BaseInput v-if="isOpenCPFOnRegister" v-model="cpf" class="theme-input px-[20px]" max="11" :msg="cpfError" input-mode="numeric" placeholder="CPF" />
      <BaseInputBirthday v-if="needBirthday" ref="birthdayInputRef" v-model="birthday" class="px-[20px]" />
      <div class="flex flex-col gap-[4px] px-[20px]">
        <div class="flex origin-center cursor-pointer items-center" @click="isCode = !isCode">
          ...
          <IconUniArrowUpSmall2 v-else />
        </div>
        <BaseInput v-if="isCode" v-model="parentUid" class="theme-input" :placeholder="t('code2')" />
      </div>
      <div class="theme-check flex px-[20px]">
        <BaseCheckBox
        ...
        >
          ...
          >
            <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);" @click.stop="page = 4">
              {{ t('user_agreement2') }}
            </BaseButton>
          </i18n-t>
        </BaseCheckBox>
      ...
      <div class="items-center px-[20px]">
      ...
      <!-- 邮箱或手机验证 -->
      <template v-if=" isVerifyPage || isNoVerifyPage">
        <teleport to=".app-dialog-register-v2-h5">
          <div class="other-page-wall" @click="page = 1">
            <div class="verify-dialog rounded px-[16px] pb-[32px] pt-[16px]" @click.stop>
              <template v-if="isVerifyPage">
                <div class="flex">
                  <div class="w-full flex cursor-pointer items-center justify-between gap-[5px] text-[18px] font-semibold leading-[25px]">
                    <span>{{ isEmailRegType ? t('label_email_check') : t('label_phone_check') }}</span>
                    <div @click="page = 1">
                      <IconUniClose class="text-[14px]" />
                    </div>
                  </div>
                </div>
                ...
                <i18n-t
                  ...
                  class="text-tg-text-lightgrey my-[12px] text-[14px] font-semibold leading-[1.5]"
                >
                ...
                <div class="my-[12px] flex">
                  <BaseButton type="text" size="none" @click.stop="page = 3">
              ...
              <template v-if="isNoVerifyPage">
                <div class="grid gap-y-[12px]">
                  <!-- 没收到验证码 -->
                  <AppDialogRegisterPartNoReceiveCode
                    :type="isEmailRegType ? 'email' : 'phone'"
                    @back="page = 2"
                  />
                </div>
              </template>
      ...
      <teleport v-if="isUserAgreement" to=".app-dialog-register-v2-h5">
        <div class="other-page-wall" @click="page = 1">
          <div class="theme-terms-box rounded px-[16px] pb-[32px] pt-[16px]" @click.stop>
            <div class="mb-[12px] flex cursor-pointer items-center justify-between gap-[5px] text-[18px] font-semibold leading-[25px]">
              <span>{{ t('user_agreement') }}</span>
              <div @click="page = 1">
                <IconUniClose />
              </div>
            </div>
            <div class="scroll-y theme-bg policies-terms bg-tg-secondary max-h-[324px] rounded-[4px] px-[12px] py-[20px]" @touchstart.stop @touchmove.stop>
              <AppPoliciesTerms />
            </div>
          </div>
        </div>
...
<style lang="scss">
.app-dialog-register-v2-h5 {
  --tg-base-dialog-close-only-padding-top: 22px;
  --tg-base-dialog-card-border-radius: 0px;
}
</style>
...
.theme-check {
  @include webTheme('green') {
    --tg-radius-default: 4px;
    --tg-border-color-deep-grey: #3cb389;
    --tg-border-color-main: #3cb389;
    --tg-base-checkbox-outer-border-color: #197b59;
  }
  @include webTheme('white') {
    --tg-base-checkbox-outer-border-color-active: #f2ca5c;
    --tg-base-checkbox-outer-border-color: #e3e3e3;
  }
}
.theme-select {
  @include webTheme('green') {
...
.register-form {
  @include webTheme('white') {
    ...
    --tg-input-select-bg-color: #f5f5f5;
    --tg-third-btn-bg-color: #f5f5f5;
  }
}
</style>
