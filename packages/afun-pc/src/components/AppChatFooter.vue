...
import { IconUniClose, IconChatRule } from '@tg/icons'
  ...
  <section class="tg-app-chat-footer" @touchstart.stop @touchmove.stop>
    ...
    <div class="chat-input">
      <BaseInput
        ref="msgInput"
        v-model="message"
        :placeholder="enableChat ? t('chat_send_msg_placeholder') : t('forbid_chat_placeholder')"
        textarea
        style="--tg-base-input-height: auto;"
        :disabled="!enableChat"
        :max="maxMsgLen"
      ...
      >
        <template #right-icon>
          <BaseButton
            size="none" type="text"
            style="overflow: visible;height: 32px;--tg-base-textarea-maxheight: 164px;"
            :disabled="maxMsgLen - message.length <= 0 || !enableChat"
    ...
    <div class="actions" @touchstart.stop @touchmove.stop.prevent>
      ...
      <BaseButton
        ...
        size="sm"
        @click="openChatRulesDialog"
      >
        <IconChatRule />
      </BaseButton>
      <BaseButton
        :disabled="maxMsgLen - message.length < 0 || sendLoading || !enableChat"
        bg-style="primary"
        class="send"
        size="sm"
        shadow
