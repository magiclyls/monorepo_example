...
import { IconUniFavorites, IconChessStar } from '@tg/icons';
...
<template>
  <div class="app-sports-market-info" :class="[
          ...
          <template v-else-if="isCountdown">
            <div>
              <svg height="12" width="12" viewBox="0 0 20 20" class="svelte-l8nfzs">
                ...
                <circle r="5" cx="10" cy="10" fill="transparent" stroke="#105EB4" stroke-width="10.5"
    ...
    <template v-else>
      ...
      <!-- 队名 -->
      <div class="teams" :class="[userLanguage]">
        <!-- 主队名称 -->
        <div class="teams-name">
          <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);"
            @click="goEventDetailPage">
            <span :class="[userLanguage]">{{ data.htn }}</span>
          </BaseButton>
        </div>
        <!-- 客队名称 -->
        <div class="teams-name">
          <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);"
            @click="goEventDetailPage">
            <span :class="[userLanguage]">{{ data.atn }}</span>
          </BaseButton>
        </div>
      </div>
    ...
    <template v-if="isStandard">
      ...
      <div class="outcomes" style="--area: outcomes0;">
        <template v-if="standardMarketBtns">
          <AppSportsBetButton v-for="market in standardMarketBtns" :key="market.wid + market.sn"
    ...
    <template v-else>
      ...
      <div class="outcomes-three" :class="{ 'outcomes-three-h5': isH5Layout }" style="--area: outcomes0;">
        <template v-if="threeOptionTotalBtns && threeOptionTotalBtns.length > 0">
          <AppSportsBetButton v-for="market in threeOptionTotalBtns" :key="market.wid + market.sn"
            class="app-sports-bet-button" :title="market.title" :odds="market.ov" :disabled="market.disabled"
      ...
      <div class="outcomes-three" :class="{ 'outcomes-three-h5': isH5Layout }" style="--area: outcomes1;">
        <template v-if="threeOptionWinnerBtns && threeOptionWinnerBtns.length > 0">
          <AppSportsBetButton v-for="market in threeOptionWinnerBtns" :key="market.wid + market.sn"
            class="app-sports-bet-button" :title="market.title" :odds="market.ov" :disabled="market.disabled"
            :cart-info="market.cartInfo" :hdp="market.hdp" layout="horizontal" />
        </template>
        <AppSportsOutcomeLocked v-else />
      ...
      <div class="outcomes-three" :class="{ 'outcomes-three-h5': isH5Layout }" style="--area: outcomes2;">
        <template v-if="threeOptionHandicapBtns && threeOptionHandicapBtns.length > 0">
          <AppSportsBetButton v-for="market in threeOptionHandicapBtns" :key="market.wid + market.sn"
            class="app-sports-bet-button" :title="market.title" :odds="market.ov" :disabled="market.disabled"
    ...
    <div v-if="showBreadcrumb || (!isStandard && !isH5Layout)" class="breadcrumb">
      <BaseBreadcrumbs :list="breadcrumbs" :only-last="isH5Layout" @item-click="onBreadcrumbsClick" />
    </div>
    ...
    <div class="market-count" :class="{ 'market-count-h5': isH5Layout }">
      ...
      <BaseButton class="theme-mc text-btn" type="text" size="none" @click="goEventDetailPage">
        <span>+{{ data.mc }}</span>
      </BaseButton>
    </div>
  </div>
