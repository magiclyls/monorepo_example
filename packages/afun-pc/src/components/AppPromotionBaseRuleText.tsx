...
const AppPromotionBaseRuleText = defineComponent({
  props: {
    ...
    currencyType: {
      type: Object as PropType<EnumCurrencyKey>,
      required: true,
    },
    amount: {
      type: String,
  ...
  setup(props) {
    ...
    // 金额货币要插入的地方
    const regex = /(\d+(\.\d+)?%?\s?)?[A-Za-z]{1,7}\$+\$currency\$\$/g
    const regexCur = /[A-Za-z]{1,7}\$+\$currency\$\$/g
    const regexVar1 = /\$+\$variable1\$\$/g
    const regexVar2 = /\$+\$variable2\$\$/g
    const regexVar3 = /\$+\$variable3\$\$/g
    const regexVar4 = /\$+\$variable4\$\$/g
    const str = computed(() => props.content?.replace('$$site_name$$', getEnv().VITE_SITE_NAME.toUpperCase())?.replace(/[0-9]*\$\$freetimes\$\$/, props.freetimes.toString()))
    ...
    const renderAppIcon = () => {
      return currencyType.value
        ? <>{amount.value}<AppCurrencyIcon class="mx-[3px] translate-y-2" style="--tg-app-currency-display:inline-flex;" currencyType={currencyType.value}></AppCurrencyIcon></>
        : ''
    }
    const renderAppIconTwo = (matchedStr: string) => {
      ...
      if (curArr && curArr.length) {
        ...
        return <><span class="ml-[3px]">{cur_amount ? formatAmountFunc(`${cur_amount}`, cur_currency) : ''}</span><AppCurrencyIcon class="mx-[3px] translate-y-2" style="--tg-app-currency-display:inline-flex;--tg-app-currency-icon-style-margin: 0 3px;" currencyType={cur_currency}></AppCurrencyIcon></>
      }
      return matchedStr
    }
    ...
    return () => <div class="text-tg-promotion-text-color w-full text-[14px] font-medium leading-[1.5]">{replacedStr()}</div>
  },
})
