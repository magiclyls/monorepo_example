...
import { IconUniArrowleftLine } from '@tg/icons'
import type { IMemberReg, IMemberThirdReg } from '@tg/types'

import { ApiMemberReg, ApiMemberThirdReg } from '@tg/apis'
...
defineOptions({
  name: 'AppRegisterTermsConditions',
})
  ...
  <div class="app-register-terms-conditions" :class="{ 'is-auth': isAuth }">
    <div class="title mb-[16px]">
      <div class="arrow" @click.stop="goPrev">
        <IconUniArrowleftLine />
      </div>
      <div class="text">
        {{ t('step_label') }} 2/2{{ t('colon') }} {{ t('read_and_accept') }}
      </div>
    </div>
    <div ref="scrollRef" class="scroll-y scroll-light terms-conditions-outer mb-[16px]">
      <AppPoliciesTerms />
    </div>
    <div class="check-box">
      <BaseCheckBox
      ...
      <BaseButton
        :loading="isLoading"
        size="xl"
        bg-style="primary" @click.stop="getStartGame"
      >
