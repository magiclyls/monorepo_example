...
<template>
  <div class="sub-wrapper">
    ...
    <AppSportsMarket v-for="league, i in list" :key="league.ci" :is-standard="isStandard" :league-name="league.cn"
      :event-count="league.list.length" :base-type="baseType" group-by-date :event-list="league.list"
      :auto-show="i === 0" />
    <AppSportsMarketSkeleton v-if="moreLoading" :num="total - curTotal > 10 ? 10 : total - curTotal" :si="league.si" />
    <BaseButton v-show="curTotal < total && !moreLoading" size="none" type="text" @click="loadMore">
      {{ t('load_more') }}
    </BaseButton>
  </div>
</template>
