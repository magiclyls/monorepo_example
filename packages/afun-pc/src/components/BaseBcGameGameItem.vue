...
<template>
  <BaseAspectRatio
    class="group game-ratio" ratio="100/134.1463414634"
  ...
  >
    <div class="base-game-item" :class="{ maintain: isMaintained }" @click.stop="gameStart">
      <div class="backgrop-filter">
        <BaseImage
          v-if="thumbnailStatus" is-cloud :url="getThumbnailUrl(gameInfo.img ?? '')" loading="eager"
          @error-img="thumbnailLoadError"
        />
      </div>
      <template v-if="showBorder">
        <img class="border" src="/afun-pc/img/border/border.svg" alt="">
        <div class="game-title">
          {{ gameInfo.name }}
        </div>
      </template>
      <BaseImage
        v-if="!isError && showImg" :url="gameImgUrl" :loading="loading" :name="gameInfo.name" is-cloud
      ...
      >
        ...
        >{{ gameInfo.online_num }}</span>
      </div>
      <!-- 遮罩 -->
      <div
        v-if="!isMobile || isMaintained"
