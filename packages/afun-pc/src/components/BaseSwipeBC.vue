<script lang="ts" setup name="BaseSwipeH5Wg">
...
function onSwiperInit(swiper: SwiperInstance) {
  ...
  currentIndex.value = swiper.realIndex // 初始化索引
}
...
<template>
  <div class="hero-wrapper">
    <div class="" :class="props.items.length == 1 ? 'un-after' : ''">
      ...
      <div class="h-[6px] w-full flex justify-center py-[8px]">
        <ul class="flex gap-x-[6px]">
          <li v-for="item, i in props.items" :key="i" :class="`${currentIndex === i ? 'w-[44px]' : 'w-[6px]'}`" class="bg-layer4 h-[6px] cursor-pointer overflow-hidden rounded-full transition-all duration-500" style="transition-timing-function: ease;">
            <span v-if="currentIndex === i" class="bg-brand block h-full w-full rounded-full will-change-transform" style="transform-origin: left center; transform: translateX(-19.5%);" />
          </li>
        </ul>
      </div>
    </div>
...
.bg-brand{
    background-color: rgb(36,238,137)
}
.bg-layer4 {
