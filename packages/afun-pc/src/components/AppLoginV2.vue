...
const { brandReg, isLoginPWAOpen, noLoginDays,isOpenCPFOnRegister } = storeToRefs(useBrandStore())
...
function valiPassword(){
  if(password.value.length < 8){
      openNotify({
        type: 'error',
        message: t('pop_up_login_second_input_min_length', { delta: 8 }),
      })
      return false
  }
  return true
...
const _placeholder = computed(() => isOpenCPFOnRegister.value ? `${`${t('email1')}/${`${t('phone')}/${t('username')}`}`}/CPF` : `${`${t('email1')}/${`${t('phone')}/${t('username')}`}`}`)
...
const {
  ...
  onSuccess: async (res: string) => {
    ...
    // 在关闭弹窗前保存用户名来用发提示语
    const _userNameField = username.value
  ...
  onError: (err) => {
    ...
    if (data.includes('member-verify')) {
      ...
      openNotify({ type: 'error', message: t('your_account_was_locked') })
    }
    // 自我排除
...
async function getMemberLogin(validate?: string) {
  ...
  const psdErr = await valiPassword()
  if(!psdErr) return
...
<template>
  <div>
    <div class="app-login grid grid-cols-1 gap-y-[12px]">
      <BaseInput  ref="userNameRef" :openBorderPlaceholder="true" :deal-input-val="dealLoginOrRegVal" v-model="username" class="theme-input"
        :placeholder="_placeholder" msg-after-touched />
      ...
      <BaseInput ref="passwordRef"  :openBorderPlaceholder="true"
        :floatLabel="t('登入密码')"
         v-model="password" :placeholder="t('登入密码')" type="password"
      ...
      <BaseButton bg-style="primary" :loading="isLoading || loadingMemberCaptcha" size="md"
