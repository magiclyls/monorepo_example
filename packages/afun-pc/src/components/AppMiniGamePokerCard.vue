...
import { IconHeitao, IconHontao, IconFangkuai, IconMeihua } from '@tg/icons'
        ...
        >
          <div class="face-content ml-[10%] w-max flex flex-col items-center" :class="[color]">
            <template v-if="_rank && color">
              <span>{{ _rank }}</span>
              <template v-if="color">
                <IconHeitao v-if="color === PokerColors.HEITAO" name="heitao" />
                <IconHontao v-else-if="color === PokerColors.HONTAO" name="hontao" />
                <IconFangkuai v-else-if="color === PokerColors.FANGKUAI" name="fangkuai" />
                <IconMeihua v-else-if="color === PokerColors.MEIHUA" name="meihua" />
              </template>
            </template>
          </div>
        </div>
...
button.poker-card {
  ...
  .content {
    ...
    .back {
      ...
      background-image: url('/img/game/poker-back.svg');
      @include webTheme('green'){
        background-image: url('/img/game/poker-back_greentheme.svg');
      }
    }
  }
}
