...
const activeVirtCurrency = ref(JSON.parse((routeData?.activeCurrency || '{}') as string))
const backDepositInfo = ref(JSON.parse((routeData?.backDepositInfo || '{}') as string))
const currentNetwork = routeData?.currentNetwork as any
const {
...
function showNotify() {
  ...
  openNotify({
    ...
    message: () => h(AppMemberNotificationsMessage, {
      amount: application.formatNumDecimal(backDepositInfo.value.amount, getCurrencyConfig(type).decimal),
      currencyType: type,
...
<template>
  <div class="app-virtual-deposit mb-[16px]">
    ...
    <div v-if="backDepositInfo.id" class="box-btn" style="--tg-spacing-12:28px">
      <BaseButton size="md" :loading="paymentDepositCoinCancelLoading" style="--tg-text-dark:#ffffff"
        @click="runPaymentDepositCoinCancel({ id: backDepositInfo.id ?? '' })">
        {{ t('cancel_deposit') }}
      </BaseButton>
      <BaseButton bg-style="primary" size="md" :loading="paymentDepositCoinConfirmLoading"
