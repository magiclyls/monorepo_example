...
const {
  ...
  if (!value)
    ...
    return !isPhone.value ? t('validate_msg_regexp_code') : t('validate_msg_regexp_code_phone')
  return ''
})
const {
...
async function onRegisterClick() {
 ...
 phoneErrorMsg.value = ''
  if (needEmail.value && !isPhone.value) {
    ...
    emailErrorMsg.value = await validateEmail(middleVal.value)
  ...
  if (needUsername.value) {
    if(username.value.length < 6){
      openNotify({
        type:'error',
        message: t('pop_up_create_account_name_3_char')
      })
      return
    }

    if(!hasLetter.test(username.value)){
      openNotify({
        type:'error',
        message: t('为确保账户安全，您的用户名必须包含至少一个英文字母')
      })
      return
    }
  ...
  // 需要验证手机
  if (needCheckPhone.value && isPhone.value) {
  ...
  // 需要验证邮箱
  if (needCheckEmail.value && !isPhone.value) {
    regType.value = 'email'
...
function dealLoginOrRegVal(val: string) {
...
async function onEmailUsernameBlur(type: 1 | 2 | 3) {
  let result
  if (type === 1 && username.value && username.value.length > 5)
  ...
  <div>
    <div class="grid gap-y-[12px] p-[16px] pb-[32px] pt-[20px]">
      <template v-if="isRegisterForm">
        <h6 class="text-tg-text-white text-[18px] font-semibold leading-[20px] text-center pb-[10px]">
          {{ t('register') }}
        </h6>
        <!-- 表单 -->
        <div v-if="needPhone || needEmail" class="flex flex-col">
          <div class="relative flex" style="--tg-input-lefticon-px: 0;--tg-input-lefticon-py: 0;">
            <BaseInput  :openBorderPlaceholder="true" ref="phoneInputRef" v-model="middleVal" hide-spin-btn :placeholder="regPlaceholder" @blur="onEmailUsernameBlur(3)">
              <template #left-icon v-if="needPhone && isPhone">
        ...
        <div v-if="needUsername" class="flex flex-col">
          <BaseInput
          :openBorderPlaceholder="true"
          :floatLabel="t('username')"
            ref="userNameRef" v-model="username" :max="18" :placeholder="t('用户名必须包含至少一个英文字母')"
            name="name" msg-after-touched :deal-input-val="dealLoginOrRegVal" @blur="onEmailUsernameBlur(1)"
          />
        ...
        <BaseCpfInput v-model="cpf" placeholder="000.000.000-00" input-mode="numeric" name="cpf" :required="true" :msg-after-touched="false" />
        <BaseInput
        :openBorderPlaceholder="true"
          :floatLabel="t('登入密码')"
          ref="passwordRef" v-model="password" :placeholder="t('登入密码')" type="password" name="password"
          autocomplete="current-password" msg-after-touched  @focus="onPasswordFocus"
          @blur="onPasswordBlur"
        />
        <AppPasswordVerify
        ...
        />
        <!-- CPF -->
        <!-- <BaseInput v-if="isOpenCPFOnRegister" :openBorderPlaceholder="true" v-model="cpf" max="11" :msg="cpfError" input-mode="numeric" placeholder="CPF" /> -->
        ...
        <div class="flex flex-col gap-[4px]">
          <div class="flex origin-center cursor-pointer items-center" @click="isCode = !isCode">
            <span class="text-tg-text-lightgrey mr-[4px] mb-[4px] text-[14px] font-semibold">{{ t('input_invite_code') }}</span>
            <IconUniArrowUpSmall v-if="!isCode" />
            <IconUniArrowUpSmall2 v-else name="uni-arrow-up-small2" />
          </div>
          <BaseInput v-if="isCode" :openBorderPlaceholder="true" v-model="parentUid" :placeholder="t('code2')" />
        </div>
        <div class="flex">
          <BaseCheckBox
          ...
          >
            <i18n-t
              keypath="i_agree_and_i_am_18_2" tag="div"
              class="text-tg-text-lightgrey text-[14px] font-semibold leading-[1.5]"
            >
              <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);" @click="goToPdf">
                {{ t('privacy_policy') }}
              </BaseButton>
              <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);" @click="goToPdf">
                {{ t('terms_conditions_vault') }}
              </BaseButton>
            </i18n-t>
          </BaseCheckBox>
        </div>
        <BaseButton
          :loading="isApiLoading" :disabled="isApiLoading" bg-style="primary" size="md"
      ...
      <template v-else-if="isVerifyPage">
        <div class="flex">
          <div class="flex cursor-pointer items-center gap-[5px] text-[18px] font-semibold leading-[25px]" @click="page = 1">
            ...
            <span>{{ !isPhone ? t('label_email_check') : t('label_phone_check') }}</span>
          </div>
        </div>
        ...
        <i18n-t
          ...
          class="text-tg-text-lightgrey text-[14px] font-semibold leading-[1.5]"
        >
          <span>{{ !isPhone ? middleVal : `${areaCode} ${middleVal}` }}</span>
        </i18n-t>
        <BaseInput
        ...
        <BaseButton
          bg-style="primary" size="md" :loading="isApiLoading" :disabled="isApiLoading"
      ...
      <AppDialogRegisterPartNoReceiveCode
        v-else-if="isNoVerifyPage" :type="!isPhone ? 'email' : 'phone'"
        @back="page = 2"
      />
