  ...
  onSuccess(res) {
    if (res && res.length > 0) {
      ...
      if (arrCurrentCur.length > 0)
        tempNum = arrCurrentCur[0].total_rebate || 0
      ...
      if (arrExceptCurrentCur.length > 0) {
        ...
        Promise.allSettled(arrPromise).then((result) => {
          ...
          expectAmount.value = Number.parseFloat(add(tempNum, total))
...
<template>
  ...
  <div v-else class="default-card">
    ...
    <BaseButton v-if="showRebateBtn" class="mt-[12px] h-[44px]" bg-style="primary" @click="rebateHandler">
      {{ t('vip_get_rebates') }}
    </BaseButton>
  </div>
</template>
