...
<template>
  <BaseList v-if="getList && getList.length > 0" ref="baseListRef"
    :finished="getCurrentTotal > 0 ? getCurrentTotal >= pageTotal : false" :loading="isLoading" :need-stop="false"
    class="app-notice-notify" @load="handleLoad">
    ...
    <div class="notice-scroll">
      ...
      <div v-for="item, index of getList.slice(0, getCurrentTotal)" :key="item.id"
        class="contain-item relative cursor-pointer" @click="openDialogDetail(item)">
        <div class="w-[100%] flex" @touchstart.capture="touchStart" @touchend.capture="touchEnd" @click="touchClick"
          ...
          <div class="over-ellipsis item-right right-radius">
            <div class="right-state">
              ...
              <BaseBadge v-if="getState(item.state ?? item.is_read)" class="state-text" :max="99999" status="success"
            ...
            <div class="over-ellipsis content-text">
              <div class="c-block">
                {{ t('amount') }}
                <AppAmount style="display: inline-block" :amount="getAmount(item)"
                  :currency-type="item.content.currency_name" />
              </div>
            </div>
          </div>
          <!-- h5滑动删除按钮 -->
          <div v-if="showDel(item.state ?? item.is_read, item.title) && isMobile"
          ...
          <div v-if="showDel(item.state ?? item.is_read, item.title) && !isMobile" class="absolute right-0 top-0">
            <BaseButton custom-padding style="--tg-base-button-padding-y: 2px;--tg-base-button-padding-x: 2px"
