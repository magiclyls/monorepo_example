...
import type { IAvailableCurrency, EnumCurrencyKey, VirtualCoin,ISortedListItem } from '@tg/types'
...
const { balanceData, isCompleteKYC, isLogin,allContractListData } = storeToRefs(appStore)
...
const {
  ...
  if (!value) {
    ...
    return `${t('min_amount_is')}${application.formatNumDecimal(minWithdraw.value, getCurrencyConfig(activeVirCurrency.value.currency_name).decimal)}`
  }
...
const virCurrencyListOptions = computed(() => props.virCurrencyList.filter(a => a.currency_name.includes(virSearchValue.value.toUpperCase())))
/** 虚拟钱包列表 */
...
function onVirCurrencyChange(item: ICurrencyOption) {
  ...
  const network = getVirtualCurrencyContractType(item?.currency_name)
      ...
      <template v-if="curContractList && curContractList.length">
        ...
        <template v-if="addrOptions.length">
          ...
          <BaseButton
            ...
            bg-style="primary" size="md" @click="handleWithdraw"
          >
            {{ t('confirm_submit') }}
          </BaseButton>
        </template>
        ...
        <template v-else>
          ...
          <BaseButton style="--tg-base-button-font-size:16px;" bg-style="primary" size="md" @click="toAddVirAddress">
