...
import type {EnumCurrencyKey, EnumLanguageKeys} from '@tg/types'
...
const menuData = computed(() => [
  {
    title: t('晋升'),
    children: [
      { title: t('promotions'), path: '/promotion' },
  ...
  },
  {
    ...
    children: [
      { title: t('体育首页'), path: '/sports/401' },
      { title: t('soccer'), path: `/sports/401/1` },
      { title: t('体育规则'), icon: false, path: '/policies/spt-book' },
    ],
  },
  {
    title: t('about_us'),
    children: [
      { title: t('about_us'), path: '/policies/about-us' },
      { title: t('条款和条件'), path: '/policies/terms' },
      { title: t('help_space_title'), path: '/help' },
    ],
  },
  {
    title: t('责任'),
    children: [
      ...
      { title: t('一般规则'), path: '/policies/rules' },
  ...
  {
    title: t('contact_us_short'),
    children: [
      { title: t('contact_us_short'), onClick: () => openService({ isFootBar: false, needDeal: true }) },
    ],
  },
])
...
const textInfoImgs = [
  {img: '/png/footer/footer-img1.png', url: '', alt: '' },
  {img: '/png/footer/footer-img2.png', url: 'https://betblocker.org/', alt: '' },
  {img: '/png/footer/footer-img3.png', url: 'https://gamblingtherapy.org/', alt: '' },
  {img: '/png/footer/footer-img4.png', url: 'https://sigap.fazenda.gov.br/', alt: '' },
]
...
// } = useRequest(ApiBrand, {
//   manual: false,
//   defaultParams: [{ tag: 'bottom' }],
//   onSuccess(data) {
//     console.log('ApiBrand 接口返回数据:', data);
//   },
// })
</script>
...
<template>
  <div class="app-footer" :style="{ gap: isMobile ? '12px' : '32px' }">
    <div v-if="!isMobile && brandConfig?.bottom && getBottomData(brandConfig?.bottom)?.length">
      <div class="w-[full] h-[20px] leading-[20px] text-[14px] text-center mb-[16px]">{{ t('加入我们的社区') }}</div>
      <div class="w-[full] text-center flex center wz-css">
        <component
        ...
        >
          <BaseImage :url="`/png/settings/social-${bottomItem.name}.png`" width="32px" class="promotion-base-image" />
        </component>
      </div>
    </div>
    ...
    <div v-else class="footer-nav">
      <div v-for="item, index of menuData" :key="index" class="layout-spacing reset">
        ...
        <div v-for="tmp of item.children" :key="tmp.title" @click="pathTo(tmp)" class="h-[20px]">
          {{ tmp.title }}
    ...
    <BaseDivider size="1" />
    <div v-if="!isMobile" class="text-[#B1BAD3] text-[14px] leading-[21px]">
      <div class="w-[815px] h-[52px] flex justify-between items-center mb-[42px] mx-[auto]">
        <component
          ...
          rel="noopener noreferrer"
        >
          <BaseImage height="50px" :url="imgItem.img" :alt="imgItem.alt" />
        </component>
      </div>
      <i18n-t
        keypath="解释说明" tag="div"
        class="text-[14px] font-semibold leading-[1.5]"
      >
    ...
    <template v-if="!isMobile">
      <div class="footer-copyright">
        <BaseLogo :static="false" :originHeight="40" />
