...
import { IconNavbarHome, IconNavbarWallet, IconUniShare, IconUniPromo, IconTabbarChat } from '@tg/icons'
...
<template>
  <div class="app-footer-bar" @touchstart.stop @touchmove.stop.prevent>
    ...
    <!-- 存款   -->
    <div class="bar-item">
      <BaseButton type="text" size="none" @click="openWallet">
        <div class="bar-btn">
          <div class="bar-icon">
            <IconNavbarWallet />
          </div>
          <span>{{ t('pop_up_wallet_tap_card_deposit').toLowerCase() }}</span>
        </div>
      </BaseButton>
    </div>
