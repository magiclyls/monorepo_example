...
<template>
  <div class="base-tab" :class="{ center }">
    <div ref="scrollDom" class="scroll-x hide-scrollbar base-tab-wrap" :class="{ full }">
      <div ref="parent" class="square tab-wrap" :class="{ 'line-style': lineStyle }">
            ...
            <div class="content column">
              <slot name="tab" :item="t">
                <div v-if="t.icon && showIcon" class="icon" style="--app-sport-image-error-icon-size:14px;">
                  <AppImage v-if="useCloudImg || t.useCloudImg" style="width: auto; height: 22px" :url="t.icon" is-cloud
                    loading="eager" />
        ...
        <div class="dash" :class="[dashMountedClass]"
          :style="{ width: dashWidth, transform: `translateX(${percent}px)` }" />
      </div>
    </div>
  </div>
</template>
...
.base-tab-wrap {
  ...
  .tab {
    ...
    padding: var(--tg-tabvn-style-inner-padding-y) var(--tg-tabvn-style-inner-padding-x);
    display: flex;
    align-items: center;
