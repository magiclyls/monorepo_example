...
import { IconUniArrowDown, IconUniWarningColor } from '@tg/icons'
interface Props {
  modelValue?: any
  options: {
    [text: string]: any
  ...
  lableJustify?: 'space-between' | 'center'
  showIcon?: boolean
  hasActiveStyle?: boolean
  // 相同值依旧隐藏
  sameHide?: boolean
  popperCenter?: boolean
  isPayTheme?: boolean
...
function onClickPopperItem(item: any, hide: () => void) {
  ...
  if (item.state === 2 || item.disabled)
    return
  else if (v === props.modelValue && !props.sameHide)
    return
  hide()
  emit('update:modelValue', v)
  emit('select', v)
    ...
    >
      <div
        ...
        :class="{
          ...
          'is-pop-label-mobile': isMobile,
          'is-label-border': isLabelBorder,

        }"
      >
        <div v-if="props.hasActiveStyle" class="mask" />
        <div class="content" :style="{ justifyContent: lableJustify }">
          <div class="content-label">
            <slot name="label" :data="selectedOption" :popper-label="popperLabel">
              <span v-if="showPlaceholder && !modelValue" class="inner-label placeholder">{{ placeholder }}</span>
              <span v-else class="inner-label">{{ popperLabel }}</span>
            </slot>
          </div>
          <div v-if="props.showIcon" class="icon" :class="{ up: isPopperOpen }">
            <IconUniArrowDown />
          </div>
        </div>
      </div>
      ...
      <template #popper="{ hide }">
        <slot name="popper" :hide="hide">
          <div
            class="scroll-y hide-scrollbar need-pad-y popper-wrap" :style="{ maxHeight: popperMaxHeight }"
            @touchmove="popInnerTouchmove"
          >
            ...
            <a
              ...
              :class="{
                ...
                'is-center': popperCenter,
                'pay-theme': isPayTheme,
              }"
              @click="onClickPopperItem(item, hide)"
            >
              <slot
                name="option"
                :data="{ item, parentWidth, active: item.value === modelValue }"
              >
                {{ item.label }}
              </slot>
            </a>
...
<style lang="scss">
:root {
  --tg-base-select-disabled-opacity:0.5;
  ...
  --tg-base-select-popper-label-hover-bg-color: var(--tg-secondary-deepdark);
  ...
  --tg-base-select-border: var(--tg-border-width-sm) solid var(--tg-secondary-main);
  ...
  --tg-base-select-popper-label-radius: var(--tg-radius-default);
  --tg-base-select-small-style-padding-right: var(--tg-spacing-28);
  --tg-base-select-text-align: start;
  --tg-base-select-small-style-padding-left: var(--tg-spacing-7);
  --tg-base-select-pop-plain-bg-color: transparent;
  --tg-base-Select-arrow-icon-color-on-white:#111111;
  --tg-base-select-select-warp-background-color: var(--tg-secondary-dark);
  --tg-base-select-select-hover-border-color: var(--tg-border-color-deep-grey);
}

[theme="green"] {
  --tg-base-select-popper-active-color: var(--tg-text-yellow);
  --tg-icon-arrow-color:#5ecea6;
  --tg-icon-color-grey:#b1bad3;
  --tg-green-dark: #02432D;
...
[theme="white"] {
  ...
  --tg-base-select-popper-label-hover-bg-color: #cbcbcb;
  --tg-base-select-bg-theme-white: #f5f5f5;
  --tg-base-select-popper-active-color:#F2CA5C;
  --tg-base-select-select-hover-border-color:#F2CA5C;
}
</style>
...
.popper-label {
  ...
  border-radius: var(--tg-base-select-popper-label-radius);
  ...
  // }
  &.plain {
    background-color: var(--tg-base-select-pop-plain-bg-color);
    &:hover {
      background-color: var(--tg-base-select-pop-plain-bg-color);
    }
  }
  ...
  &:hover:not(.disabled) {
    ...
    background-color: var(--tg-base-select-popper-label-hover-bg-color);
  ...
  &.disabled{
    opacity: var(--tg-base-select-disabled-opacity);
    cursor: not-allowed;
  }
}
.msg {
...
.popper-option {
  ...
  &.is-pop-not-mobile:hover {
    ...
    @include webTheme('green') {
      background-color:  var(--tg-base-select-popper-active-color);
      color: var(--tg-popper-color-default);
    }
  }
  ...
  &.pay-theme{
    padding:var(--tg-spacing-button-padding-vertical-xs) 28.5px;
    --tg-base-select-popper-active-color:#02432D;
  }
}
...
.select-warp {
  ...
  select {
    ...
    background-color: var(--tg-base-select-select-warp-background-color);
    box-shadow: var(--tg-base-select-box-shadow);
    color: var(--tg-base-select-style-color);
    position: relative;
    border-radius: var(--tg-radius-default);
    font-weight: var(--tg-base-select-popper-font-weight);
    text-align: var(--tg-base-select-text-align);
    @include webTheme('white'){
      background-color: var(--tg-base-select-bg-theme-white);
      --tg-base-select-box-shadow:#fff;
    }
    &:hover {
      border-color: var(--tg-base-select-select-hover-border-color);
    }

    &.small {
      padding: var(--tg-spacing-8) var(--tg-spacing-28) var(--tg-spacing-8) var(--tg-spacing-7);
      padding-right: var(--tg-base-select-small-style-padding-right);
      padding-left: var(--tg-base-select-small-style-padding-left);
    }
  ...
  .icon {
    ...
    align-items: center;
    @include webTheme('green'){
      --tg-icon-color:var(--tg-icon-arrow-color);
    }
    @include webTheme('white'){
      --tg-icon-color: var(--tg-base-Select-arrow-icon-color-on-white);
    }
  }
}
...
<style>
  ...
  .v-popper__inner::-webkit-scrollbar {
  display: none
}
</style>
