...
import {IconPasswordHook2} from '@tg/icons';
...
const props = withDefaults(defineProps<Props>(), {
  width: '100%',
  percent: 0,
  strokeColor: 'var(--tg-primary-active)',
  strokeWidth: 8,
  showInfo: true,
  type: 'line',
  showPadding: true,
...
<template>
  <div
    ...
    :style="`width: ${totalWidth};padding: ${showPadding ? ((24 - strokeWidth) / 2) : 0}px 0`"
  >
    <div class="m-progress-inner">
      <div
        class="u-progress-bg" :style="`background:${lineColor};
    ...
    <template v-if="showInfo">
      <Transition mode="out-in">
        <span v-if="percent >= 100" class="m-success">
          <IconPasswordHook2 class="u-icon" name="password-hook-2" />
        </span>
  ...
  <div v-else class="m-progress-circle" :style="`width: ${totalWidth}; height: ${totalWidth};`">
    <svg class="u-progress-circle" viewBox="0 0 100 100">
      ...
      <path
        ...
        :stroke-width="strokeWidth" :stroke="lineColor" :style="`stroke-dasharray: ${(percent / 100) * perimeter}px,
         ${perimeter}px;`" :opacity="percent === 0 ? 0 : 1" fill-opacity="0"
      />
    </svg>
    <template v-if="showInfo">
      <Transition mode="out-in">
        <IconPasswordHook2 v-if="percent >= 100" class="u-icon" name="password-hook-2" />
        <p v-else class="u-progress-text">
          {{ percent >= 100 ? 100 : percent }}%
        </p>
      </Transition>
...
// [theme="white"] {
//   --tg-base-progress-active-bg: #F2CA5C ;
// }
</style>
