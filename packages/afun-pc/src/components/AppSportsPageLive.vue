...
import { IconSptBallPlate, IconEmpty1 } from '@tg/icons'
...
<template>
  <div class="tg-sports-type" :class="{ 'on-page': onPage }">
    <div class="sports-page-title">
      <div class="left">
        <IconSptBallPlate v-if="onLobby" />
        <h6>{{ t('sports_tab_live_events') }}</h6>
      </div>
      <AppSportsMarketTypeSelect v-model="currentLiveBetType" v-model:is-standard="isStandard"
        :base-type-options="baseTypeOptions" />
    </div>
    <template v-if="sportLiveNavs.length > 0">
      <AppSportsTab v-model="currentLiveNav" :list="sportLiveNavs" @change="onSportsSiChange" />
    </template>
    <div class="market-wrapper">
      <AppSportsMarketSkeleton v-if="switchLoading" :num="marketNum" :si="currentLiveNav" />
      <template v-else>
        <template v-if="isHaveDataToShow">
          <AppSportsMarket v-for="item, idx in list" :key="item.ci + item.list.length" :is-standard="isStandard"
            :league-name="item.cn" :event-count="item.list.length" :event-list="item.list"
            :base-type="currentLiveBetType" :class="[
              idx < list.length - 1 ? 'mb-[12px]'
                : (moreLoading || curTotal < total || onPage ? 'mb-[12px]' : ''),
            ]" />
          <AppSportsMarketSkeleton v-if="moreLoading" :num="10" :si="currentLiveNav"
            :class="[curTotal < total || onPage ? 'mb-[12px]' : '']" />
          <BaseButton v-show="curTotal < total && isHaveDataToShow && !onPage" size="none" type="text"
            :class="[curTotal < total && onPage ? 'mb-[12px]' : '']" @click="loadMore">
            {{ t('load_more') }}
          </BaseButton>
          <BaseButton v-if="onPage" size="none" type="text" style="padding-left: var(--tg-spacing-16);"
