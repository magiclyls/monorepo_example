...
import type { DepositStoryRecordList, EnumCurrencyKey, IAvailableCurrency, LoginWalletPaymentDepositActivity, WalletPaymentDepositActivity } from '@tg/types'
      ...
      <div v-if="currentSubTab === 'wallet'" class="bg-deep payment p-[8px]">
        <div v-for="item in loginFullRes.wallet" :key="item.id" class="flex">
          ...
          <div class="bg-main-light h-[50px] flex flex-1 items-center justify-between rounded rounded-l-none pl-[6px] pr-[6px]">
            ...
            <BaseButton bg-style="primary" class="h-[28px] w-[85px] shrink-0" @click="() => openWalletDialogIfNotApp({ subtab: 'wallet', detail: item })">
              {{ t('fast_go_recharge') }}
            </BaseButton>
          </div>
        </div>
        ...
        <div class="payment bg-deep p-[8px]">
          ...
          <div v-for="item in currentContract" :key="item.id" class="flex">
            ...
            <div class="bg-main-light h-[50px] flex flex-1 items-center justify-between rounded rounded-l-none pl-[6px] pr-[6px]">
              ...
              <BaseButton bg-style="primary" class="h-[28px] w-[85px] shrink-0" @click="() => openWalletDialogIfNotApp({ subtab: 'virtual', detail: item })">
                {{ t('fast_go_recharge') }}
              </BaseButton>
            </div>
          </div>
