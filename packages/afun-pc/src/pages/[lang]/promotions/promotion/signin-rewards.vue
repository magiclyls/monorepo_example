...
<template>
  <div class="text-tg-text-lightgrey m-auto mt-[20px] max-w-[650px] pb-[20px]">
    ...
    <div class="mb-[16px] rounded-[4px] bg-[#213744] px-[10px] py-[12px] text-[14px]">
      ...
      <BaseButton v-if="!isLogin" class="h-[44px] w-[100%]" bg-style="primary" @click="openLoginDialog">
        {{ t('login_first_please') }}
      </BaseButton>
      <BaseButton v-else-if="currentDayState === 0" class="h-[44px] w-[100%]" bg-style="primary" disabled>
        {{ t('vip_right_now') }}
      </BaseButton>
      <BaseButton v-else-if="currentDayState === 1" class="h-[44px] w-[100%]" bg-style="primary" disabled>
        {{ t('expired') }}
      </BaseButton>
      <BaseButton v-else-if="currentDayState === 2" class="h-[44px] w-[100%]" bg-style="primary" disabled>
        {{ t('Received') }}
      </BaseButton>
      <BaseButton v-else-if="currentDayState === 3" class="h-[44px] w-[100%]" bg-style="primary" :loading="loadingClaim" @click="getBonus">
        {{ t('vip_right_now') }}
      </BaseButton>
    </div>
    <AppActivityVenues
