...
const {
  ...
  if (!value)
    return t('pls_enter_confirm_pwd')
  else if (value.length < 8)
...
<template>
  <div class="tg-settings-security">
    ...
    >
      <BaseLabel :label="t('old_pwd')" must-small>
        <BaseInput
          ...
          type="password" msg-after-touched :placeholder="t('old_pwd')"
        />
      </BaseLabel>
      <BaseLabel :label="t('new_pwd')" must-small>
        <BaseInput
          ref="newPwd" v-model="newPassword" :placeholder="t('new_pwd')"
          :msg="newPwdErrorMsg"
          type="password"
          msg-after-touched
      ...
      <BaseLabel :label="t('confirm_new_pwd')" must-small>
        <BaseInput
          ref="repeatPwd" v-model="repeatPassword" :placeholder="t('confirm_new_pwd')"
    ...
    <!-- <template v-if="isOpenPayPwd">
      <AppPayPassword :key="brandStore.brandChangeNum" />
    </template> -->
  </div>
</template>
