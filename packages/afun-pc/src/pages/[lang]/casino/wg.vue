...
<template>
  <div data-cl="casino-wg" class="casino-wg">
    <div class="layout-spacing">
      ...
      <div v-if="!isMobile" class="tg-mt-24 px-[3vw]">
        <AppGameSearchWg :key="routeName" game-type="1" is-lobby />
      </div>
      <div class="mx-[3vw] mt-[7px] flex items-center gap-[5px] pl-[2px]">
        <BaseImage class="w-[18px]" url="/png/home/icon_dt_pmd.webp" loading="eager" />
        <AppNoticeMarquee class="flex-1" />
        <AppEmailMsg />
      </div>
      <div v-show="casinoNav && casinoNav.length > 0" class="tg-mt-24 @md:px-[3vw]" :class="{ isMobile }">
        <!-- <BaseTab2Wg
          v-if="isMobile"
      ...
      <div v-if="isMobile" class="px-[3vw] pt-[14px]">
        <AppPrizePool />
      </div>
      <div class="content-wrapper px-[3vw]">
        <!-- 大厅 -->
        <div v-show="showAll">
    ...
    <div class="layout-spacing px-[3vw]">
      <AppBetDataWg mode="casino" />
    </div>
    <!-- 公司介绍 -->
    <div v-if="!isLogin" class="px-[3vw] pb-[32px]">
