...
const rebateRecordComp = defineAsyncComponent(()=>import('./record.vue'))
const { openDialog } = useDialog({
  showClose:false,
  default: ()=>h(rebateRecordComp)
})
    ...
    <div class="tabs">
      <div :style="{ marginBottom: !isMobile && !isTiered ? '16px' : 0 }">
        <div v-if="isTiered && !tabList.length && !uniformOption.length" class="flex justify-between">
          ...
          <span class="theme-go cursor-pointer text-[14px]" @click="openDialog">
            {{ t('vip_rebate_record') }}
          </span>
        </div>
        ...
        <template v-else-if="tabList.length">
          <div class="flex">
          <AppGameTypeIconTabs v-model:active="tab"
            :list="tabList" :center="false" />
            <div class="ml-[20px] text-[14px] center text-[#B1BAD3] font-[500] cursor-pointer whitespace-nowrap" @click="openDialog">{{ t('vip_rebate_record') }}</div>
            </div>
        </template>
        <template v-else-if="uniformOption.length">
          <div class="flex">
          <AppGameTypeIconTabs v-model:active="uniformTab" :list="uniformOption" :center="false" @update:active="onUniformTabChange"/>
            <div class="ml-[20px] text-[14px] center text-[#B1BAD3] font-[500] cursor-pointer whitespace-nowrap" @click="openDialog">{{ t('vip_rebate_record') }}</div>
            </div>
        </template>
      </div>
          ...
          <div v-for="item in tieredRebateValidBetData" :key="item.platform_id"
            ...
            <div
              ...
              <div class="flex flex-col">
                ...
                <div
                  class="progress-bar relative my-[6px] h-[6px] w-[400px] overflow-hidden rounded-[16px] bg-[var(--tg-rebatetab-progress-bg)]">
