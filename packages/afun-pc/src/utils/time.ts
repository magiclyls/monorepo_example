import type { TIMEZONEABBR } from './enums'
...
// 年月日
const dateFormatNumber: { [t: string]: string } = {
  'en-US': 'DD/MM/YYYY',
  'zh-CN': 'YYYY-MM-DD',
  'vi-VN': 'DD/MM/YYYY',
  'pt-BR': 'DD/MM/YYYY',
...
export function timeToZoneFormat(ts: number, zone?: TIMEZONEABBR): string {
  dayjs.locale(langKey[getCurrentLanguageForFrontend()])
  if (getCurrentLanguageForFrontend() === 'vi-VN')
    return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangFormat[getCurrentLanguageForFrontend()]).replace('T', 'Th ').replace('tháng', 'Thg')

  return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangFormat[getCurrentLanguageForFrontend()])
}
...
export function timeToZoneDayFormat(ts: number, zone?: TIMEZONEABBR): string {
  dayjs.locale(langKey[getCurrentLanguageForFrontend()])
  if (getCurrentLanguageForFrontend() === 'vi-VN')
    return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangDayFormat[getCurrentLanguageForFrontend()]).replace('T', 'Th ').replace('tháng', 'Thg')

  return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangDayFormat[getCurrentLanguageForFrontend()])
}
/** 按时区：月日 时分 */
export function timeToZoneDayFormat2(ts: number, zone?: TIMEZONEABBR): string {
  dayjs.locale(langKey[getCurrentLanguageForFrontend()])
  if (getCurrentLanguageForFrontend() === 'vi-VN')
    return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangDayFormat2[getCurrentLanguageForFrontend()]).replace('T', 'Th ').replace('tháng', 'Thg')

  return dayByZoneStamp({ t: checkTs(ts), zone }).format(timeWithLangDayFormat2[getCurrentLanguageForFrontend()])
}
...
export function timeToFromNow(ts: number): string {
  ...
  if (resSec < 60 && resSec > 0)
    return t('last_seconds', { delta: resSec })

  const lang = getCurrentLanguageForFrontend()
...
/** 统一为2024/3/05样式 */
export function timeToFormatDay(ts: number): string {
  return timeToCustomizeFormat(ts, 'YYYY/MM/DD')
}
/** 符合老板要求格式的完整时间 */
 ...
 * 获取本年时间段，精确到时分秒
 */
export function getCurrentYearTimeRange() {
  const now = new Date()
  // 一月是 0
  const startOfYear = new Date(now.getFullYear(), 0, 1)
  startOfYear.setHours(0, 0, 0, 0)

  // 12 表示下一年的一月，再减一天就是本年的最后一天
