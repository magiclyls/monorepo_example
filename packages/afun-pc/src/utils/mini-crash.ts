...
// 加载 crash 游戏资源
export async function loadCrashAssets(theme: TTheme) {
  if (globalPixi.image_greybg === null || globalPixi.image_greybg === undefined || !globalPixi.image_greybg) {
    globalPixi.app = new Application({ width: canvasWidth, height: canvasHeight, antialias: true, background: themeBgc[theme] ?? themeBgc.default, backgroundColor: themeBgc[theme] ?? themeBgc.default })
    ...
    globalPixi.image_currency = await Assets.load('/afun-pc/webp/currency/701.webp')
