  ...
  <div class="size-full relative">
    <div
      ...
      :class="[
        ...
        modelValue && !isFocus ? 'text-[#B1BAD3]' : 'text-[var(--afun-cpf-input-border-color-focus)]',
      ]"
    >
      CPF
    </div>
    ...
    <div v-if="error" class="mt-[5px] flex leading-[17px] text-[12px] font-[600]">
      <div class="h-[17px] mr-[4px] flex items-center">
        <IconInfo class="text-[14px] text-[var(--afun-cpf-input-error-icon-color)]" />
      </div>
      <span class="text-[var(--afun-cpf-input-error-text-color)]">{{ msg }}</span>
    </div>
  </div>
</template>
...
<style>
:root {
  --afun-cpf-input-border-radius: 4px;
  --afun-cpf-input-border-color: #2f4553;
  --afun-cpf-input-border-color-focus: #24ee89;
  --afun-cpf-input-padding-left: 12px;
  --afun-cpf-input-padding-right: 12px;
  --afun-cpf-input-padding-y: 9px;
  --afun-cpf-input-background-color: #0f212e;
  --afun-cpf-input-disabled-background-color: #1a2c38;
  --afun-cpf-input-font-size: 14px;
  --afun-cpf-input-font-weight: 600;
  --afun-cpf-input-color: #fff; /* 输入文字颜色（通常为白色） */
  --afun-cpf-input-style-placeholder-color: #566771; /* 默认 Placeholder 颜色（灰色） */
  --afun-cpf-input-style-placeholder-font-size: 14px;
  --afun-cpf-input-line-height: 20px;
  --afun-cpf-input-shadow: none;
  --afun-cpf-input-error-text-color: #ed4163;
  --afun-cpf-input-error-icon-color: #ed4163;
  --afun-cpf-input-height: 43px;
}
</style>
...
.base-input {
  position: relative;
  width: 100%;
  height: var(--afun-cpf-input-height);
  display: flex;
  overflow: hidden;
  border-radius: var(--afun-cpf-input-border-radius);
  border: 1px solid var(--afun-cpf-input-border-color);
  padding-left: var(--afun-cpf-input-padding-left);
  padding-right: var(--afun-cpf-input-padding-right);
  background-color: var(--afun-cpf-input-background-color);
  box-shadow: var(--afun-cpf-input-shadow);

  &.isFocus {
    border-color: var(--afun-cpf-input-border-color-focus);
  }

  &.disabled {
    background-color: var(--afun-cpf-input-disabled-background-color);
  }
  ...
  .input-wrap {
    ...
    display: flex;
    align-items: center;
    padding: var(--afun-cpf-input-padding-y) 0;
    position: relative;
    ...
    .ghost-layer {
      ...
      pointer-events: none;
      white-space: pre;
      font-size: var(--afun-cpf-input-font-size);
      font-weight: var(--afun-cpf-input-font-weight);
      line-height: var(--afun-cpf-input-line-height);
      padding: var(--afun-cpf-input-padding-y) 0;
      ...
      .visible-placeholder {
        /* 默认状态：灰色 */
        color: var(--afun-cpf-input-style-placeholder-color);
        transition: color 0.2s;

        /* 输入状态：变更为输入文字颜色 (白色) */
        &.is-typing {
          color: var(--afun-cpf-input-color);
        }
      }
    }
...
.border-placeholder {
  opacity: 0;
  background: linear-gradient(to top, var(--afun-cpf-input-background-color) 50%, transparent 50%);
}
