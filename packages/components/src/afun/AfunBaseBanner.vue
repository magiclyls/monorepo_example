...
defineOptions({ name: 'AfunBaseBanner' })
...
<template>
  <BaseAspectRatio ratio="351/160">
    <Swiper
      ...
      class="rounded-[10px] w-full h-full overflow-hidden"
      @swiper="onSwiper"
      ...
      >
        ...
        <div class="w-full h-full rounded-[10px] overflow-hidden">
          <BaseImage
            ...
            fit="contain"
            @error-img="errorList[i] = false"
          />
          <div v-else class="bg-[#EBEBEB] text-[#9DABC9] w-full h-full center flex-col">
            <BaseImage class="mb-[6px]" url="/ph-h5/png/game-img-error.png" width="24px" height="24px" />
            <span class="text-[12px]">{{ t('加载失败了x_x') }}</span>
          </div>
          ...
          <div class="other" :class="[item.align === 'left' ? 'other-left' : 'other-right']">
            ...
            <div>
              <button
                v-if="item.button"
                class="relative min-w-[120px] rounded-[4px] p-[12px] text-[#fff] h-[40px] center top-[-3px] border border-solid border-[#fff]" @click.stop="jumpToUrl({
