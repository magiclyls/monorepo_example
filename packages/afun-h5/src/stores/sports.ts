...
import { application, getCurrencyConfig, globalInitMap, Local, numberReg, SportsCart, STORAGE_SPORTS_CURRENT_PROVIDER, STORAGE_SPORTS_ODDS_TYPE_KEY } from '@tg/utils'
...
import { SportsOdds } from '~/utils/sports'
...
export const useSportsStore = defineStore('sportsInner', () => {
  const { t } = useI18n()
  ...
  /** 体育场馆登录 */
  const { run: runAsyncSportsLogin } = useRequest(() => ApiSportsLogin({ cur: getCurrencyConfig(currentGlobalCurrency.value).cur }), {
    errorRetryCount: 3,
  ...
  const sportsMenu = computed(() => {
    return [
      ...
      {
        title: t('我的投注'),
        path: `/sports/my-bets?type=sports`,
        icon: IconSptUserBet,
        list: [],
        domId: '',
        token: true,
      ...
      return topGamesObj?.list?.map((sport) => {
        return {
          ...
          list: [
            ...
            { title: 'Live & Upcoming', path: `/sports/${sport.si}?tab=1`, icon: 'spt-ball-plate' },

            { title: 'Outrights', path: `/sports/${sport.si}?tab=2`, icon: 'spt-timing' },
            ...(sport.list
              ? sport.list.map((league) => {
                  return {
                    ...
                    path: `/sports/${sport.si}/${league.pgid}/${league.ci}?${application.objectToUrlParams({ sn: sport.sn, pgn: league.pgn, cn: league.cn })}`,
                  }
                })
              : []),

            { title: 'View All', path: `/sports/${sport.si}?tab=3`, icon: 'uni-view-all' },
          ],
        }
      })
    }
  ...
  const sportOddType = computed<MenuItem[]>(() => [
    {
      title: `${t('赔率', { other: 'cược' })}${getLang() === 'zh-CN' ? '：' : ': '}${t(sportsOddsType.value)}`,
      path: '',
      icon: IconSptOdds,
      type: 'radio',
