...
import { AfunBaseButton, AfunBaseDialog, AfunBaseImage, AfunBaseInput, BaseForm } from '@tg/bccomponents'
...
import { application, dealLoginOrRegVal, getBrandInfo, getEnv, Local, sendMsgToFlutterThirdPartyApp, STORAGE_LOGIN_MQTT_ID, STORAGE_PROMO_AD_POP_LOGIN } from '@tg/utils'
...
const _placeholder = computed(() => isOpenCPFOnRegister.value ? `${`${$t('邮箱地址')}/${`${$t('手机号码')}/${$t('用户名')}`}`}/CPF` : `${`${$t('邮箱地址')}/${`${$t('手机号码')}/${$t('用户名')}`}`}`)
const isDisableLogin = computed(() => !userOrEmail.value)
...
function onSubmit() {
  ...
  if (psdVal) {
    openNotify({ type: 'error', message: psdVal as string })
    return
  }
  showCaptcha()
}
