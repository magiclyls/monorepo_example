...
import { AfunBaseButton, AfunBaseCheckbox, AfunBaseCurrencyIcon, AfunBaseInput, AfunBaseLabel } from '@tg/bccomponents'
import { useAppStore } from '@tg/stores'
import { Local, STORAGE_WALLET_ROUTE, virtualCoinAddressReg } from '@tg/utils'
...
const isDefault = computed(() => Number(routeData?.isFirst) === 1)
...
<template>
  <AppPageLayout :title="$t('绑定加密货币')">
    <div class="flex flex-col gap-[16px] mx-12">
      <AfunBaseLabel :label="t('货币')" required>
        <AppSelect
        ...
        >
          <template #item-icon="{ item }">
            <AfunBaseCurrencyIcon :currency-type="item.label as EnumCurrencyKey" />
          </template>
        </AppSelect>
      </AfunBaseLabel>
      <AfunBaseLabel
      ...
      <AfunBaseLabel :label="t('钱包地址')" required>
        <AfunBaseInput v-model="address" style="--afun-base-input-padding-y:10px;" :placeholder="t('钱包地址')" :msg="addressMsg" />
      </AfunBaseLabel>
      <AppPasswordInput ref="passwordRef" v-model="password" />
      <div class="flex items-center">
        <AfunBaseCheckbox v-model="isDefault" style="--tg-base-checked-bg:#1A2C38;--tg-base-checked-border:#2F4553;--tg-base-checkbox-size:16px;" />
