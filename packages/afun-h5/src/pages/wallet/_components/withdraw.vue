...
const {
  ...
  runAsync: getFinanceWithdrawData,
...
const tabList = computed(() => {
  ...
  if (fiatCurrencyList.value.length > 0)
    arr.push({ label: t('法币'), value: 'fiat', icon: '/m/afun-h5/png/fiat.png' })
  if (virtualCurrencyList.value.length > 0)
    arr.push({ label: t('加密货币'), value: 'virtual', icon: '/m/afun-h5/png/virtual.png' })
  return arr
})
...
const showLoading = computed(() => {
  return isDataLoading.value || (!loaded.value && !getComponent.value)
})
function onLoad() {
  loaded.value = true
}
async function onChangeTab(t: TabValue) {
...
await application.allSettled([getFinanceWithdrawData()])
</script>

<template>
  <div class="d">
    <div v-if="getComponent" class="my-[16px] mx-[10px] flex flex-col gap-[16px]  ">
      <component :is="getComponent" :key="brandStore.brandChangeNum" :is-component="true" :tip-text="$t('提款')" @toggle="toggleEmail" />
    </div>
    <template v-else>
      <div class="mt-[16px]">
        <AfunBaseTabs v-if="tabList.length > 1" v-model="tab" :type="6" :full="true" :list="tabList" @change="onChangeTab" />
      </div>
