...
import { AfunBaseButton, AfunBaseCurrencyIcon, AfunBaseDialog, AfunBaseInput, AfunBaseLabel, AfunBaseTabs } from '@tg/bccomponents'
...
<template>
  ...
  >
    <template #left>
      <div class="unselectable grid w-full gap-y-[12px]">
        <AfunBaseTabs
        ...
        />
        <AfunBaseLabel class="order-1 set-label-color" :label="t('投注额')" :right-text="currencyPrefix + betAmountMulRate">
        ...
        <div class="flex-col-12 order-2 min-h-[59px] flex flex-col">
          ...
          <div v-show="isManual" class="flex-col-12 order-2 flex flex-col">
            <AfunBaseLabel class="order-2 set-label-color" :label="t('获胜利润')" :right-text="currencyPrefix + profitMulRateTrans">
              <AfunBaseInput
              ...
              >
                <template #right>
                  <AfunBaseCurrencyIcon
                    ...
                    style="--tg-app-currency-icon-size:16px"
                  />
                </template>
              </AfunBaseInput>
            </AfunBaseLabel>
          ...
          <div v-show="isAutoBet" class="order-2 flex flex-col gap-[12px]">
            <AfunBaseLabel class="order-2 set-label-color" :label="t('投注次数')">
              <AppMiniGamePublicBetTimes v-model.number="betTimes" />
            </AfunBaseLabel>
            <AfunBaseLabel class=" order-2 set-label-color" :label="t('若赢')">
              <div class="bg-[#2f4553] rounded-[4px] py-[2px] pl-[2px] pr-[40px]">
                <AppMiniGamePublicAutoDouble v-model="incOnWin" :disabled="gameStart || reqLoading" />
              </div>
            </AfunBaseLabel>
            <AfunBaseLabel class=" order-2 set-label-color" :label="t('若输')">
              <div class="bg-[#2f4553] rounded-[4px] py-[2px] pl-[2px] pr-[40px]">
                <AppMiniGamePublicAutoDouble v-model="incOnLoss" :disabled="gameStart || reqLoading" />
              </div>
            </AfunBaseLabel>
            <AfunBaseLabel class=" order-2 set-label-color" :label="t('止盈')" :right-text="currencyPrefix + stopOnProfitMulRate">
              <AfunBaseInput
            ...
            <AfunBaseLabel class=" order-2 set-label-color" :label="t('止损')" :right-text="currencyPrefix + stopOnLossMulRate">
              <AfunBaseInput
    ...
    <template #right>
      <div class="unselectable main-game px-[16px] pb-[16px] pt-[48px] @[800px]:min-h-[630px]">
        <!-- 游戏历史数据 -->
        <div class=" absolute right-[16px] top-[16px] flex justify-end overflow-hidden ">
          <div class="relative flex">
            <TransitionGroup name="list">
              <AfunBaseButton
                v-for="button in reversedButtons" :key="button.billId" class="button-number mr-[4px]" :class="[button.iswill ? '' : 'button-lose']" round
          ...
          >
            ...
            <div class="svelte translate">
              <div class="svelte translate-x" :style="{ transform: `translate(${randomNumber}%, 0px)` }">
                ...
                >
                  <img class="svelte dice" src="/afun-h5/svg/classic-dice.svg" alt="Dice">
        ...
        <!-- 底部参数 -->
        <div class="theme-grey-bg grid grid-cols-3 items-center rounded-[4px] gap-[8px] p-[16px]">
          <AfunBaseLabel :label="t('乘数')" class="set-label-color">
            <AppMiniGamePublicNumber
          ...
          <AfunBaseLabel :label="roll ? t('掷大于') : t('掷小于')" class="set-label-color" @click="changeRollOver">
              ...
              <div class="input-wrap">
                <div class="readonly input-box">
                  ...
                  <div class="right-icon">
                    <IconUniRefresh class="text-[16px]" />
                  </div>
                </div>
              </div>
            </div>
          </AfunBaseLabel>
          <AfunBaseLabel :label="t('获胜机率')" class="set-label-color">
            <AppMiniGamePublicNumber
  ...
  <AfunBaseDialog v-if="selectedItem" v-model="showDetailDialog" :icon="IconUniRebate" :title="t('投注')">
    <AppDialogBetSlipCasino v-if="showDetailDialog" :casino-data="selectedItem.data" />
  </AfunBaseDialog>
  <AfunBaseDialog v-model="showDiceGameInfoDialog" :icon="IconUniGameInfo" :title="t('游戏规则')" :show-close="true" :show-close-icon="true">
    <AppMiniGamePartDiceGameInfo />
  </AfunBaseDialog>
  <AfunBaseDialog v-model="showDiceHotKeysDialog" :icon="IconUniKeyboard" :title="t('快捷键')" :show-close="true" :show-close-icon="true">
    <AppMiniGamePartDiceHotKeys />
  </AfunBaseDialog>
</template>
...
:root {
  --slider-height: 8px;
  --lines-asset: url(/afun-h5/svg/lines.svg);
  --radius-base: 4px;
  --spacingEm-1-5: 1.5em;
  --grey-400: #2f4553;
