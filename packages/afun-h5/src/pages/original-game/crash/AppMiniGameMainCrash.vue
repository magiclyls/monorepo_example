...
import { AfunBaseAmount, AfunBaseButton, AfunBaseCurrencyIcon, AfunBaseDialog, AfunBaseInput, AfunBaseLabel, AfunBaseTabs, BaseAspectRatio } from '@tg/bccomponents'
...
/** 横坐标 */
function drawXAxis() {
  const fillColor = 0xFFFFFF // 横坐标颜色值
  const container = new Container()
  container.x = 0
  container.y = 0
...
<template>
  ...
  >
    <template #left>
      <div class="game-crash-sidebar h-full w-full flex flex-col-reverse @md:flex-col" :class="[isMobile && 'crash-mobile-sidebar']">
        ...
        <AfunBaseTabs v-model="tab" class="set-gap" :list="tabList" full :class="[orderTab]" :disabled="tabDisabled" :disable-click="tabDisabled" :type="6" />
        <AfunBaseTabs v-if="tab === 'auto'" v-model="viewTab" :list="viewList" shape="square" full class="view-tab" :class="[orderTab, tabModelValue === viewTab && 'tab-active']" :type="5" @change="onTabChange" />
        <div v-show="tab === 'manual' ? true : viewTab !== 'rank'" class="w-full flex flex-col" :class="[orderBetAmount]">
          <AfunBaseLabel :label="$t('投注额')" :right-text="currencyPrefix + betAmountMulRate" class="set-label-color">
            <div class="bg-[#13212d] rounded-[4px] border border-[#2f4553] border-solid p-[4px]">
              <AppMiniGamePublicBetAmount
                ref="betAmountRef"
          ...
          <div class="mt-[12px] w-full flex items-end" :class="[tab === 'auto' && !isMobile ? 'max-w-[276px]' : '']">
            <AfunBaseLabel :label="t('兑现于')" right-text="" class="w-full set-label-color">
              <AppNumberCount
                ref="cashOutAtRef"
                v-model="cashOutAt"
            ...
            <template v-if="tab === 'auto'">
              <AfunBaseLabel v-if="tabDisabled" :label="t('投注次数')" class="ml-[8px] @md:max-w-[110px] set-label-color">
                <AppMiniGamePublicBetTimes :model-value="restBetTimes" :disabled="tabDisabled" />
              </AfunBaseLabel>
              <AfunBaseLabel v-else :label="t('投注次数')" class="ml-[8px] @md:max-w-[110px] set-label-color">
        ...
        <div v-if="tab === 'auto' && viewTab !== 'rank'" class="w-full flex flex-col" :class="[orderMidArea]">
          <AfunBaseLabel :label="t('若赢')" class="mb-[12px] set-label-color">
            <div class="bg-[#304554] rounded-[4px] py-[2px] pl-[2px] pr-[40px]">
              <AppMiniGamePublicAutoDouble v-model="percentIfWin" :disabled="tabDisabled" />
            </div>
          </AfunBaseLabel>
          <AfunBaseLabel :label="t('若输')" class="mb-[12px] set-label-color">
            <div class="bg-[#304554] rounded-[4px] py-[2px] pl-[2px] pr-[40px]">
          ...
          >
            ...
            >
              <template #right>
                <AfunBaseCurrencyIcon :currency-type="currentCurrency?.name" />
              </template>
            </AfunBaseInput>
          </AfunBaseLabel>
          <AfunBaseLabel
        ...
        <AfunBaseLabel v-if="isMobile ? tab === 'auto' : true" :label="t('获胜利润')" :right-text="currencyPrefix + betAmountMulRate" class="set-label-color">
          <AfunBaseInput
            v-model="totalProfit" readonly
            style="--tg-base-input-style-pad-x:7px;--tg-base-input-style-pad-y:7px;--tg-base-input-line-height:1.5;"
          >
            <template #right>
              <AfunBaseCurrencyIcon style="--tg-app-currency-icon-size:16px;" :currency-type="currentCurrency?.name" />
            </template>
          </AfunBaseInput>
        </AfunBaseLabel>
        ...
        <div v-if="tab === 'manual' || (tab === 'auto' && viewTab === 'rank')" class="w-full flex grow flex-col" :class="[orderRanking]">
          <div class="ranking text-[#b1bad3] mb-[12px] flex items-center justify-between text-[14px] font-semibold">
            <div class="flex items-center">
              <IconUniUsers /><span class="ml-[8px] mr-[16px]">{{ cashInList.length }}</span>
            </div>
            <div>
              <AfunBaseAmount reverse :amount="totalCashInAmount" :currency-code="currentCurrency?.cur" />
            </div>
          </div>
            ...
            <div class="scroll-y scroll-wrapper h-full w-full overflow-x-hidden pb-[8px]" @scroll="rankTableScroll">
              <table class="h-min min-w-[250px] w-full">
                <template v-for="(item, idx) in uniqSortedCashInList" :key="item.uid">
                  <tr v-if="idx < 30" class="text-tg-secondary-light text-[14px] font-semibold" :class="[`uid_${item.uid}`]">
                    <td class="w-[40%] pl-[12px]">
                      <div v-if="item.hidden" class="hoverable">
                        <VTooltip placement="top" :triggers="['click', 'hover']">
                          <div class="ghost hidden-user flex items-center text-[14px] font-semibold">
                            <IconUniHidden />
                            <span class="ml-[4px]">{{ t('隐身') }}</span>
                          </div>
                          <template #popper>
                      ...
                      <div v-else class="overflow-hidden pr-[4px] hoverable">
                        <div class="max-w-[96px] overflow-hidden text-ellipsis whitespace-nowrap">
                          {{ item.name }}
                        </div>
                      </div>
                    </td>
                    <td class="text-[#b1bad3] width-[20%]">
                      <span v-if="+item.cashOutAt > 0">{{ item.cashOutAt }}x</span>
                      <span v-else>-</span>
                    </td>
                    <td class="text-[#b1bad3] width-[40%] pr-[8px]">
                      <VTooltip placement="top" :triggers="['click', 'hover']">
                        <AfunBaseAmount class="set-color" :amount="+item.cashOutAt > 0 ? (+item.amount) * (+item.cashOutAt) : item.amount" :currency-code="item.currency" reverse show-prefix :show-color="+item.cashOutAt > 0" />
                        <template #popper>
                          <div class="tiny-menu-item-title">
                            <AfunBaseAmount show-prefix :amount="+item.cashOutAt > 0 ? (+item.amount) * (+item.cashOutAt) : item.amount" :currency-code="item.currency" />
                          </div>
                        </template>
                      </VTooltip>
                    </td>
    ...
    <template #right>
      <div ref="gameCrashWrap" class="mini-game-crash-outer relative w-full flex grow grow flex-col justify-center" :style="{ minHeight: `${minHeightGameRoot}px` }">
        <div class="relative right-[16px] top-[16px] z-[1] flex flex-row-reverse">
          <div class="side-btns ml-[8px] flex flex-col justify-between">
            <AfunBaseButton size="xs" round class="bg-dark" @click="showCrashPointRecordDialog = true">
              <IconChessFrame1 class="text-color-whites text-[10px]" />
            </AfunBaseButton>
            <span class="text-tg-secondary-light cur-user mt-[4px] flex items-center text-[10px]">
          ...
          <div class="past-bets w-full flex flex-row-reverse">
            <template v-for="(item, idx) in crashPointList" :key="item.issue_id + item.crash_point">
              <div v-if="idx <= 5" class="wrap flex flex-col items-center">
                <AfunBaseButton class="button-number" :class="+item.crash_point >= 2 ? '' : 'button-lose'" round @click="() => openCrashIssueResult(item.issue_id)">
                  {{ floor(+item.crash_point, 2).toFixed(2) }}x
                </AfunBaseButton>
                <span v-if="+item.cashOutAt > 0" class="text-tg-text-green mt-[4px] text-center text-[10px] leading-[18px]">
        ...
        <div v-if="showPlayerResult" class="cashout-wrapper absolute right-[1px] top-[45%] flex origin-right scale-50 gap-[8px] transition-opacity duration-500 @md:scale-100" :class="[['starting', 'pending', 'start'].includes(renderFrame.current_status) || cashOutList.length === 0 ? 'opacity-0' : 'opacity-100', isMobile ? 'bottom-[8px]' : '']">
          <div class="cashout-listing height-[15px] flex flex-col items-end">
            <div v-for="(item, idx) in cashOutList" :key="idx" class="wrap text-[#b1bad3] index text-[14px] font-semibold" :class="[idx < cashOutList.length - 1 ? 'mb-[8px]' : '']">
              <div v-if="item.hidden" class="hoverable">
                <div class="ghost hidden-user-2 flex items-center">
                  <IconUniHidden />
                  <span class="ml-[4px] text-[14px]">{{ t('隐身') }}</span>
                </div>
              </div>
              <div v-else>
                <div class="text-[14px] hoverable">
                  {{ item.name }}
                </div>
              </div>
              <VTooltip placement="top" :triggers="['click', 'hover']">
                <AfunBaseAmount class="set-color" :amount="item.payout" :currency-code="item.currency" reverse show-prefix show-color />
                <template #popper>
                  <div class="tiny-menu-item-title">
                    <AfunBaseAmount :amount="item.payout" :currency-code="item.currency" show-prefix />
                  </div>
                </template>
              </VTooltip>
            </div>
  ...
  <AfunBaseDialog v-model="showGameInfoDialog" :icon="IconUniGameInfo" :title="t('游戏规则')">
    <AppMiniGamePartMinesGameInfo :game-type="GAMES_LIST_ENUM.CRASH" />
  </AfunBaseDialog>
  <AfunBaseDialog v-model="showHotKeysDialog" :icon="IconUniKeyboard" :title="t('快捷键')">
    <AppMiniGamePartMinesHotKeys :game-type="GAMES_LIST_ENUM.CRASH" />
  </AfunBaseDialog>
  <AfunBaseDialog v-if="showCrashPointRecordDialog" v-model="showCrashPointRecordDialog" :title="t('历史记录')">
    <AppDialogCrashPointRecord />
  </AfunBaseDialog>
  <AfunBaseDialog v-if="showCrashResultDialog" v-model="showCrashResultDialog" :title="t('详情')">
...
.set-color {
  --afun-base-amount-positive-color: #f23038;
}
.set-gap {
  --afun-tabs-item-gap: 0;
}
.set-label-color {
  --afun-base-label-color: #b1bad3;
}
.bg-dark {
  --afun-base-button-primary-background-color: #2f4553;
  --afun-base-button-border-radius: 27px;
}
.text-color-whites {
...
.view-tab {
  ...
  --afun-tabs-item-rounded: 0;
  --afun-tabs-wrap-padding-x: 0;
  --afun-tabs-item-gap: 0;
