  ...
  <button :disabled="isHasResult || isFetching" :class="isHasResult ? 'cursor-default' : 'cursor-pointer'">
    ...
    >
      <div
        ...
        :class="[isHasResult ? 'bg-deep-dark rounded-[8px]' : '']"
      >
        <div
          :class="[
        ...
        >
          ...
          <template v-if="isBomb">
            <img
              ...
              src="/afun-h5/svg/mine-effect.webp" alt=""
            >
        ...
        <!-- 自动投注时始终显示的选中框 -->
        <div v-if="isAuto && isChosen" class="theme-br absolute h-full w-full border-[5px] rounded-[8px] border-solid group-hover:translate-y-[-0.15em]" />

        <!-- 默认状态 -->
        <div
          v-if="!isShowResult"
          class="absolute left-0 top-0 h-full w-full rounded-[8px] opacity-100 active:translate-y-0"
          :class="[`tg-tile-${index}`,
...
.theme-box-shadow {
  box-shadow: 0 0.3em #213743;
}

.tg-gem,
.tg-bomb {
  background-image: url('/afun-h5/svg/game-mines-diamond.svg');
  background-size: 70%;
...
.tg-bomb {
  background-image: url('/afun-h5/svg/game-mines-bomb.svg');
}
