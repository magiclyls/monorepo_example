...
import { AfunBaseAmount, AfunBaseButton, AfunBaseChatInput, AfunBaseCurrencyIcon, AfunBaseDialog, AfunBaseLabel } from '@tg/bccomponents'
...
<template>
  ...
  >
    <template #left>
      <div class="flex flex-col">
        <AfunBaseLabel :label="$t('投注额')" :right-text="currencyPrefix + betAmountMulRate" class="mb-[10px] set-label-color">
          <div class="bg-[#13212d] rounded-[4px] border border-[#2f4553] border-solid p-[4px] rounded-[4px]">
            <AppMiniGamePublicBetAmount
              ref="betAmountRef" v-model="amount" :disabled="formDisabled || isBet"
              style="--afun-base-button-border-radius:4px" :currency="currentGlobalCurrencyMap.cur"
            />
          </div>
        </AfunBaseLabel>
        <AppMiniGamePublicBetButton
    ...
    <template #right>
      <div class="game-right flex flex-col items-center h-auto">
        ...
        <div class="center w-[100%] flex bg-no-repeat p-[48px]" :class="[isH5 ? 'h5-bg' : 'pc-bg']">
          <div
            ...
            style="--tg-base-button-font-size: 16px;--tg-spacing-button-padding-vertical-xs:0;--tg-spacing-button-padding-horizontal-xs:0"
          >
            <AfunBaseButton type="none" :disabled="formDisabled || !isBet" @click="betThrottle(getUpBtn.betVal, 1)">
              <div class="higher leading-[1.5] font-[700]">
                <span>{{ getUpBtn.text }}</span>
                <span>{{ getHigherEqualWinning }}%</span>
              </div>
            </AfunBaseButton>
            <AfunBaseButton type="none" :disabled="formDisabled || !isBet" @click="betThrottle(getDownBtn.betVal, 2)">
          ...
          <div class="center mx-[0.5em]">
            <div class="relative text-[24px]">
              ...
              >
                <AfunBaseButton
                  shadow size="sm" class="!bg-[#2f4553]" :disabled="formDisabled || skipCount >= maxSkip"
        ...
        <!-- 中，利润 -->
        <div
          class="middle-profit grid mb-[8px] bg-[#213743] gap-[16px] rounded-[4px] bg-light1 p-[16px] text-[14px] font-[600] "
        >
          <AfunBaseLabel :label="$t('总利润（{rate}x）', { rate: totalProfitScale })" style="--afun-base-label-color: #b1bad3;">
            <AfunBaseChatInput
              :model-value="totalProfitAmount" readonly
              style="--afun-base-input-background-color: #2f4553;--afun-base-input-border-color-focus:#cdcaca;--afun-base-input-padding-y:9px;"
            >
              <template #right>
                <AfunBaseCurrencyIcon style="--tg-app-currency-icon-size:16px" class="absolute right-[12px] top-[50%] translate-y-[-50%]" :currency-type="showCurrency" />
              </template>
            </AfunBaseChatInput>
          </AfunBaseLabel>
        </div>
        <!-- 下，牌记录 -->
        <div
          class="scroll-x scroll-contain down-record h-[102px] bg-[#00000040] mb-[16px] overflow-x-scroll rounded-[4px] px-[8px] pb-[24px] pt-[8px] md:w-[96.5%]"
        >
          <div class="flex-row-8 w-full flex items-center text-[16px]">
            <div
            ...
            >
              <div class="h5-w">
                <AppMiniGamePokerCard
                  style="--tg-mini-game-poker-card-height: 64px; --tg-mini-game-poker-card-width: 40px;--tg-mini-game-poker-rank-font-size:16px; "
                  :class="{ 'opacity-[0.5]': item.betVal === EnumBetType[5] }" class="ml-[10px] mr-[10px]"
                  :rank="item.rank" :color="item.color" :face-down="item.faceDown"
                />
                <!-- :class="{ 'scale-[0]': !item.isInit, 'opacity-0': !item.isInit }" -->
                <div
                  v-if="index !== 0"
                  class="center h5-multiplier text-[12px] absolute bg-[#fff] left-[-0.05em] top-[34%] translate-x-[-50%] translate-y-[-50%] rounded-[4px] p-[6px] shadow-[0_0_0_2px_#2a2f3c33]"
                >
                  <component
      ...
      <transition name="fade">
        <div v-if="showBetResult" class="absolute left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]">
          <div
            class="result shadow-tg-box-shadow-lg max-w-[100%] min-w-150 border-[3px] rounded-[8px] border-solid text-center text-[14px] font-extrabold"
          >
  ...
  <AfunBaseDialog v-model="showGameInfoDialog" :icon="IconUniGameInfo" :title="t('游戏规则')" style="--afun-base-dialog-background-color: #F6F7F8">
    <AppMiniGamePartMinesGameInfo :game-type="GAMES_LIST_ENUM.HILO" />
  </AfunBaseDialog>
  <AfunBaseDialog v-model="showHotKeysDialog" :icon="IconUniKeyboard" :title="t('快捷键')" style="--afun-base-dialog-background-color: #F6F7F8">
...
.game-right {
  background: #0f212e;
  --btn-hi: url(/afun-h5/svg/game_hilo_button_hi.svg);
  --btn-lo: url(/afun-h5/svg/game_hilo_button_lo.svg);
  --tg-base-button-style-bg: #42bc91;
  --bgHi: url(/afun-h5/svg/game_hilo_bg_hi_white.svg);
  --bgLo: url(/afun-h5/svg/game_hilo_bg_lo_white.svg);
  --bgHiStacked: url(/m/afun-h5/png/game_hilo_bg_hi_stacked_white.png);
  --bgLoStacked: url(/m/afun-h5/png/game_hilo_bg_lo_stacked_white.png);
