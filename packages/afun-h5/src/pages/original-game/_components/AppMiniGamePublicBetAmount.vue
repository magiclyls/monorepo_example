...
import { AfunBaseButton, AfunBaseCurrencyIcon } from '@tg/bccomponents'
    ...
    <div class="bg group w-full flex hilo-bg" :class="[_disabled ? 'cursor-not-allowed' : '']">
      <div class="relative w-full flex grow">
        <input
          ...
          class="text-[#fff] focus:border-tg-border-color-deep-grey theme-border leading[1.5] w-full rounded-l-[4px] rounded-r-[0px] bg-[--app-mini-game-public-bet-amount-bg] p-[7px] text-[13px] font-semibold duration-[0.25s]"
          :class="[
      ...
      <div class="border-theme-green flex items-center">
        <slot name="button-one">
          <AfunBaseButton
            class="btn-small mr-[2px]" :disabled="_disabled" size="sm" @click="onClickHalf"
          >
            <span>½</span>
          </AfunBaseButton>
        </slot>
        ...
        <div v-if="isMaxBetAmount && hasMax" class="relative">
          <AfunBaseButton
            class="btn-small btn-max ml-[2px]"
...
.btn-small {
  ...
  --afun-base-button-padding-y: 4px;
  --afun-base-button-padding-x: 7px;
