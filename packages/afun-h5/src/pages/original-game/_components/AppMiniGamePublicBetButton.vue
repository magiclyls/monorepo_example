...
<template>
  <AfunBaseButton :disabled="disabled" class="box-shadow" @click="onBetBtnClick">
    <template v-if="isAuto">
      <div class="flex items-center">
        <slot />
        <div v-if="loading || autoStart" :class="[game]" class="h-[22.85px] ml-[8px] flex items-center text-[16px]">
    ...
    <template v-else>
      <template v-if="loading">
        <div :class="[game]" class=" flex items-center h-[22.85px] text-[16px]">
          <component :is="loadingIcon" />
        </div>
      </template>
      <slot v-else />
