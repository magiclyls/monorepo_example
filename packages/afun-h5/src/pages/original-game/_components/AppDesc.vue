...
import { AfunBaseAmount, AfunBaseButton, AfunBaseImage, AfunBaseTable, AfunBaseTabs } from '@tg/bccomponents'
import { useBoolean } from '@tg/hooks'
import { IconChessStar, IconUniArrowDown, IconUniFavorites, IconUniHidden } from '@tg/icons'
...
function checkPlatGameList() {
  if (props.isOriginalGame)
    router.push('/group/provider/801?vid=801&ty=5') // ${query.ty ? `&ty=${query.ty}` : ''}
  else
    router.push(`/group/provider/${props.vid}?vid=${props.vid}${query.ty ? `&ty=${query.ty}` : ''}`)
}
...
<template>
  ...
  >
    ...
    <!-- 包裹过渡 -->
    <div v-show="showContent" class="wrap mt-[12px]">
      <div class="desc-tab">
        <AfunBaseTabs
          v-model="tab" :list="tabList" :center="false" :type="5"
          style="--afun-tabs-wrap-padding-y: 5px;--afun-tabs-wrap-bg:#1a2c38;"
        />
      </div>
      <div class="scroll-x winner-content">
        <AfunBaseTable
          v-if="tab !== '2'"
          class="theme-table mt-[12px]"
          style="--afun-table-odd-background: #213743"
          :columns="columns"
          :data-source="list"
        >
          <template #rank="{ index }">
            <div v-if="index < 3" class="rank-icon">
              <img
                :src="`/afun-h5/svg/uni-rank${index + 1}.svg`" alt=""
              >
            </div>
            <div v-else>
              {{ index + 1 }}th
          ...
          <div class="mobile-header">
            <div class="img-wrap">
              <div v-if="gameDetails?.img" class="img">
                <AfunBaseImage :url="gameDetails?.img" />
              </div>
            </div>
            <div class="info-wrap">
              <div class="main-info">
                ...
                <AfunBaseButton size="none">
                  <span
                    class="game-provider"
                    @click="router.push(`/group/provider/${vid}?vid=${vid}`)"
                  >
                    {{ gameProviderName }}
                  </span>
                </AfunBaseButton>
...
.theme-table {
  --afun-table-odd-background: var(--tg-secondary-grey);
}
.app-desc {
  ...
  .wrap {
    ...
    .winner-content {
      margin-top: var(--tg-spacing-12);
      --afun-table-even-background: initial;
      --afun-table-odd-background: #02432d;
