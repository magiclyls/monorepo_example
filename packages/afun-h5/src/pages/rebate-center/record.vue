...
import { AfunBaseAmount, AfunBasePagination, AfunBaseTable } from '@tg/bccomponents'
...
<template>
  <AppPageLayout :title="t('返水记录')">
    ...
    <div v-if="total > pageSize" class="my-[16px]">
      <AfunBasePagination :total="total" :page="page" :page-size="pageSize" @previous="onPrevious" @next="onNext" />
    </div>
  </AppPageLayout>
</template>
...
.tabs {
  ...
  .theme-tab-btn {
    ...
    &.active {
      ...
      color: #05080a;
      border-color: linear-gradient(90deg, #24ee89 0%, #9fe871 100%);
    }
  }
}
</style>
