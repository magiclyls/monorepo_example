...
import { AfunBaseAmount, AfunBaseSelect, AfunBaseTable } from '@tg/bccomponents'
...
<template>
  <div>
    ...
    <div class="flex my-[8px]">
      <AfunBaseSelect v-model="currency_id" :options="currencyList" active-color="#00AD01" class="flex-1" style="--afun-base-select-border-color: #0F212E" />
    </div>
    <div class="flex flex-col gap-[1px] rounded-[6px] overflow-hidden mb-[8px]">
      <div class="bg-[#213743] px-[16px] flex justify-between h-[53px] items-center">
        <div class="font-500 leading-normal text-[#B1BAD3] text-[14px]">
          {{ t('有效投注') }}
        </div>
        <AfunBaseAmount
          class="font-[600] leading-normal text-[#fff] text-[14px] text-center" :amount="detail.valid_bet_amount || '0'"
      ...
      <div class="bg-[#213743] px-[16px] flex justify-between h-[53px] items-center">
        ...
        <AfunBaseAmount
          class="font-[600] leading-normal text-[#fff] text-[14px] text-center" :amount="detail.commission_amount || '0'"
          :currency-type="getCurrencyConfig(bonus_currency)?.name"
        />
      </div>
    </div>
