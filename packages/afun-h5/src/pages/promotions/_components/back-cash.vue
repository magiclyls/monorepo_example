...
const tableColumn = computed<Column[]>(() => {
  if (dataProfitConfig.value?.reward_mode === 2) {
    ...
    return [
      ...
      {
        title: dataProfitConfig.value?.prize_config?.bonus_type === 2 ? t('返现比例') : t('奖金'),
        dataIndex: 'bonus_rate',
        align: 'center',
...
function showText(s: number) {
  ...
  const type = dataProfitConfig.value?.reward_mode === 2 ? 'vip_mode' : 'profit_prize_config'
  const number = dataProfitConfig.value?.prize_config?.[type]?.slice(-1)?.[0]?.bonus_rate + str
  switch (s) { // period
...
<template>
  <div class="back-cash-active page-back-cash text-tg-text-lightgrey m-auto mt-[16px] max-w-[650px]">
    <div class="mb-[16px] center" :class="{ ' mb-[16px]': imgUrl }">
      <AfunBaseImage v-if="imgUrl" class="set-radios" :url="imgUrl" is-network />
    </div>

    <div v-if="dataProfitConfig?.reward_mode !== 2" class="bg-[#213743] bg-box mb-[16px] rounded-[4px] p-[12px] font-[600]">
      <div class="h-[48px] flex items-center justify-between">
        <div class="text-[#fff] text-[20px]">
          {{ t('当前等级') }}{{ dataProfitLevel?.level || '0' }}
        </div>
        <AfunBaseImage class="w-[48px]" url="/m/afun-h5/png/back-cash-currency.png" />
      </div>
      <AfunBaseProgress class="mt-[16px]" width="100%" :value="percentNextLevel()" :show-percentage="false" :stroke-width="6" height="6px" background-color="#1a2c38" bar-color="#1fff20" stroke-color="var(--bg-layer-3)" />
      <div v-if="nextLevel" class="text-right text-[#b1bad3] mt-[10px] text-[12px]">
    ...
    <div class="bg-[#213743] bg-box grid gap-y-[6px] rounded-[4px] p-[12px] font-[600]">
      <div class="text-[#fff] text-[28px]">
        {{ t('负盈利返现') }}
      </div>
      <div class="theme-text text-[16px] text-[#b1bad3]" :class="[userLanguage === 'th-TH' ? 'leading-6' : '']">
    ...
    <AfunBaseTable class="table" :columns="tableColumn" last-first-padding :data-source="tableTabData">
    ...
    />
    <div class="text-[#fff] my-[16px] text-[18px] font-[600]">
      {{ t('活动规则') }}
    </div>
    <div>
