...
import { AfunBaseAmount, AfunBaseButton, AfunBaseCurrencyIcon, AfunBaseImage, AfunBaseRichArea, AfunBaseTable, AfunBaseTabs } from '@tg/bccomponents'
...
const configDaysMap: { [key: number]: { img: string, label: string } } = {
  2: {
    img: '/m/afun-h5/png/level_1.png',
    label: t('第几天', { day: 2 }),
  },
  3: {
    img: '/m/afun-h5/png/level_2.png',
    label: t('第几天', { day: 3 }),
  },
  5: {
    img: '/m/afun-h5/png/level_3.png',
    label: t('第几天', { day: 5 }),
  },
  7: {
    img: '/m/afun-h5/png/level_4.png',
    label: t('第几天', { day: 7 }),
  },
  15: {
    img: '/m/afun-h5/png/level_5.png',
    label: t('第几天', { day: 15 }),
  },
  30: {
    img: '/m/afun-h5/png/level_6.png',
    label: t('第几天', { day: 30 }),
  },
}
...
<template>
  ...
  <div v-else class="recharge-active fixed-container flex flex-col gap-[16px]">
    <AfunBaseImage v-if="imgUrl" class="w-full" style="--tg-base-img-style-radius: 12px;" :url="imgUrl" is-network />
    <div class=" font-[600] text-[#B1BAD3]">
      <div v-if="activityType === 1" class="rounded-[4px] bg-[#213743] p-[12px]">
        <div class="h-[14px] mb-[8px] flex justify-between">
          <span>{{ t('注册时间') }}{{ t('冒号') }}</span>
        ...
        <div class="h-[14px] mb-[8px] flex justify-between">
          ...
          <AfunBaseAmount class="" :amount="fullRes?.first_amount || '0.00'" :currency-type="usedCurrency" />
        </div>
        <div v-if="isLogin" class="second-color bg-deep flex flex-col items-center justify-center rounded-[4px] p-[12px]">
          ...
          <template v-else-if="fullRes?.state === 0">
            <template v-if="accumulatedRechargeType === 1">
              ...
              <i18n-t :keypath="[0, 1].includes(Number(configData?.config.period.ty)) ? '几分钟内首充奖有机会获得' : '几小时内首充奖有机会获得'" tag="div" class="flex font-normal">
                <template #n>
                ...
                <template #currency>
                  <AfunBaseCurrencyIcon :currency-type="usedCurrency" />
                </template>
              </i18n-t>
            </template>
            <template v-else>
              <i18n-t keypath="首充将有机会获得{money}" tag="div" class="min-h-[52px] center">
                <template #money>
                  <AfunBaseAmount :amount="max_prize" :currency-type="usedCurrency" />
                </template>
              </i18n-t>
              <AfunBaseButton
                class="w-[100%]"
          ...
          <template v-else>
            <span class="text-[#FFFFFF] font-[600]">{{ t('您已获取奖金') }}</span>
            <div class="my-[6px] flex">
              &nbsp;<span class="">{{ max_prize }}</span>&nbsp;
              <AfunBaseCurrencyIcon v-if="isLogin" :currency-type="usedCurrency" />
            </div>
            <AfunBaseImage class="mb-[10px] h-[30px] w-[43px]" url="/afun-h5/png/dollar.png" />
          </template>
        </div>
      ...
      <div v-else-if="activityType === 2">
        <AfunBaseTabs v-model="tab" style="--afun-tabs-wrap-padding-y:5px;" :full="true" :type="5" :list="tabList" class="mb-[20px]" />
        <div v-if="tab === 1">
          <div class="second-color text-[12px] my-[20px] text-center ">
          ...
          <div class="rounded-[4px] bg-[#213743] p-[12px]">
            ...
            <div class="h-[14px] mb-[8px] flex justify-between">
              <span>{{ t('最近充值金额1') }}</span>
              <span class="flex">
                <AfunBaseAmount class="" :amount=" fullRes?.last_amount || '0.00'" :currency-type="usedCurrency" />
              </span>
            </div>
            <div class="h-[14px] mb-[8px] flex justify-between">
              <span>{{ accumulatedRechargeLabel[accumulatedRechargeType - 1] }}</span>
              <span class="flex">
                <AfunBaseAmount class="" :amount=" fullRes?.cumulative_amount || '0.00'" :currency-type="usedCurrency" />
              </span>
            </div>

            <div v-if="isLogin" class="second-color bg-[#1A2C38] flex flex-col items-center justify-center rounded-[4px] p-[12px]">
              <template v-if="fullRes?.state === 0">
                <i18n-t keypath="最高有机会获得{money}" tag="div" class="min-h-[52px] center">
                  <template #money>
                    &nbsp;<span class="">{{ max_prize }}</span>&nbsp;
                    <AfunBaseCurrencyIcon v-if="isLogin" :currency-type="usedCurrency" />
                  </template>
              ...
              <template v-else>
                <span class="text-[#FFFFFF] font-[600]">{{ t('领取倒计时') }}</span>
        ...
        <div v-else>
          <AfunBaseTable class="table" :columns="pastEventsTableColumn" last-first-padding :data-source="recordExpand ? (past_events.d || []) : (past_events.d || []).slice(0, RECORD_SHOW_NUM)">
            ...
            <template #bonus_rate="{ record }">
              <AfunBaseButton
                v-if="record.state === 1"
                class="w-[85px]"
                style="--afun-base-button-padding-y:2px;--afun-base-button-font-size:14px;--afun-base-button-border-radius:4px;"
              ...
              <AfunBaseButton
                ...
                class="w-[85px] disabled-btn"
                style="--afun-base-button-padding-y:2px;--afun-base-button-font-size:14px;--afun-base-button-border-radius:4px;--afun-base-button-primary-background-color:#F88D22;"
                :disabled="true"
              >
                {{ t('审核中') }}
              </AfunBaseButton>
              <AfunBaseButton
                ...
                class="w-[85px] "
                style="--afun-base-button-padding-y:2px;--afun-base-button-font-size:14px;--afun-base-button-border-radius:4px;--afun-base-button-primary-background-color:#025BE8;"
      ...
      <div v-else-if="activityType === 3">
        <div class="grid grid-cols-6 mb-[20px] items-start gap-[13px]">
          ...
          >
            <div
              class="img-wrap bg-days h-[48px] w-[48px] center rounded-[6px]"
            >
              <AfunBaseImage class="h-[38px] w-[38px]" :url="configDaysMap[day]?.img" />
            </div>
        ...
        <div class="rounded-[4px] bg-[#213743] p-[12px]">
          ...
          <div class="h-[14px] mb-[8px] flex justify-between">
            ...
            <span class="flex">
              <AfunBaseAmount class="" :amount="(!registBeforeStart && fullRes?.cumulative_amount) || '0.00'" :currency-type="usedCurrency" />
            </span>
          </div>
          ...
          <div v-if="isLogin" class="second-color bg-deep flex flex-col items-center justify-center rounded-[4px] p-[12px]">
            ...
            <template v-else-if="fullRes?.state === 0">
              <AfunBaseImage class="mb-[15px] mt-[11px] h-[30px] w-[43px]" url="/m/afun-h5/png/dollar.png" />
            ...
            <template v-else-if="fullRes?.state === 6">
              <span class="text-[#FFFFFF] font-[600]">{{ t('领取奖金时间') }}</span>
              <span class="my-[6px] font-normal">{{ Number(fullRes?.reg_time) > 0 ? dayjs.unix(Number(fullRes?.reg_time)).tz(currentDollarZone).add(activeDay - 1, 'day').startOf('day').tz(currentDollarZone).format(TIME_FORMAT_2) : '-' }}</span>
              <AfunBaseImage class="mb-[15px] h-[30px] w-[43px]" url="/m/afun-h5/png/dollar.png" />
              <div class="flex font-normal">
                <span class="">{{ max_prize }}</span>&nbsp;
                <AfunBaseCurrencyIcon :currency-type="usedCurrency" />
    ...
    <AfunBaseTable class="second-color table" :columns="tableColumn" last-first-padding :data-source="prize_config ?? []">
      <template #amount="{ record }">
        <div class=" center">
          <AfunBaseAmount :amount="record.deposit || '0.00'" :currency-type="usedCurrency" />
        </div>
      </template>
      <template #bonus_rate="{ record }">
        <div class=" center">
    ...
    <div>
      ...
      <div class="mt-[8px]">
        <AfunBaseRichArea v-if="configData?.rule_type === 2" :content="currentDetail" />
...
.recharge-active {
  ...
  --afun-tabs-item-color: #fff;
  --afun-tabs-item-active-color: #fff;
