...
function getStatusImage(item: any) {
  const defaultImg = '/m/afun-h5/png/event_disable.png'
  const activeImg = '/m/afun-h5/png/event_active.png'
  if (!levelName.value)
    return defaultImg
  const periods = levelName.value.split(':')
  const status = periods[Number(item.value) - 1]
...
<template>
  <div class="every-eight-active flex flex-col gap-[16px]">
    <AfunBaseImage v-if="imgUrl" class="w-full" style="--tg-base-img-style-radius: 12px;" :url="imgUrl" is-network />
    <div class="bg-[#213744] rounded-[4px] p-[12px]">
      <div class=" mb-[6px] flex justify-between text-[18px] leading-[25px] font-[600]">
        <span>{{ `${t('活动时间')}` }}</span>
      </div>
      <div class="flex gap-[16px] text-[12px] text-[#b1bad3] font-[600]">
        <div
          ...
          class="bg-[#1A2C38]  aspect-[98/116] flex-1 flex flex-col gap-[4px] items-center justify-center border-[1px] border-[#1A2C38] rounded-[7px] border-solid "
    ...
    <div class="bg-[#213744] rounded-[4px] p-[16px] font-[600]">
      <template v-if="loadingUserConfig">
        <AppLoading :height="134" :full-screen="false" />
      </template>
      <template v-else>
        <div class="center mb-[20px] h-[71px]">
          <div class="flex-1 flex flex-col items-center">
            <span class="text-center font-[600] whitespace-nowrap small-btn">{{ informationText }}</span>
            <AfunBaseAmount class="mt-[8px]  font-[600]" :amount="userConfig?.deposit_amount || 0.00" :currency-type="currencyIcon" :member-amount="true" />
          </div>
          <div class="w-[25px] h-[66px] center">
            <span class="w-[1px] h-full bg-[#2f4554]" />
          </div>
          <div class="flex-1  flex flex-col items-center justify-center">
            <span class=" font-[600]">{{ t('奖金') }}</span>
            <AfunBaseAmount class="mt-[8px]  font-[600]" :amount="userConfig?.net_amount || 0.00" :currency-type="currencyIcon" :member-amount="true" />
          </div>
        </div>
        <template v-if="isLogin">
          <AfunBaseButton
            v-if="userConfig?.bet_count === 0 && userConfig?.net_amount === 0"
            class="w-[100%] every-eight-btn"
          ...
          <AfunBaseButton
            ...
            :loading="loadingDayClaim"
            class="w-[100%] opacity-50 every-eight-btn"
            @click="getBonus"
          >
            {{ t('立即领取') }}
          </AfunBaseButton>
    ...
    <AfunBaseTable class="table" :columns="tableColumn" last-first-padding :data-source="tableData">
      <template #amount="{ record }">
        <div class="center">
          <AfunBaseAmount :amount="record.d || '0.00'" :currency-type="currencyIcon" />
        </div>
      </template>
      <template #bonus_rate="{ record }">
        <div class="center">
          <template v-if="record.e">
            ...
            <template v-else>
              <AfunBaseAmount :amount="record.b || '0.00'" :currency-type="currencyIcon" :show-icon="false" />~
              <AfunBaseAmount :amount="record.e || '0.00'" :currency-type="currencyIcon" />
            </template>
          </template>
          <template v-else>
            <span v-if="showIcon" class="amount-unit" style="font-family: proxima-nova, sans-serif;">{{ record.b }}%</span>
            <AfunBaseAmount v-else :amount="record.b || '0.00'" :currency-type="currencyIcon" />
          </template>
        </div>
      </template>
    </AfunBaseTable>
    ...
    <div v-if="tableData.length" class="mt-16">
      ...
      <div v-if="tableData.length" class="mt-[8px]">
        <AfunBaseRichArea v-if="rule_type === 2" :content="ruleText" />
        ...
        <AfunBaseButton
          ...
          class="mt-16 w-[100%] every-eight-btn"
...
.every-eight-btn {
  --afun-base-button-primary-background-color: #24ee89;
  color: #05080a;
  font-size: 14px;
}
.every-eight-active {
