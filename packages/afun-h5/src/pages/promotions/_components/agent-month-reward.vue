...
<template>
  <div class="text-tg-text-lightgrey promo-agent-month-reward m-auto max-w-[650px]">
    <div class="mb-[16px] center" :class="{ ' mb-[16px]': imgUrl }">
      <AfunBaseImage v-if="imgUrl" class="set-radios" :url="imgUrl" is-network />
    </div>
    <div class="bg-[#fff] rounded-[4px] mb-[16px] flex flex-col rounded p-[12px] text-[14px] text-[#6D7693] font-[600]">
      <div class="mb-[8px] flex justify-between">
        <span>{{ t('上月下级充值') }}:</span>
        <span><AfunBaseAmount class="text-[#000]" :amount="commissionAmount" :currency-code="promoCurrencyCode" /></span>
      </div>
      <div class="mb-[8px] flex justify-between">
        <span>{{ t('本月奖金') }}:</span>
        <span><AfunBaseAmount class="theme-amount-sec" :amount="bonusAmount" :currency-code="promoCurrencyCode" /></span>
      </div>
      <div class="bg-[#F6F7F8] text-[#6D7693] rounded-[4px]  flex flex-col items-center justify-center rounded p-[12px] text-center">
        <template v-if="isLogin">
          <template v-if="receiveState && +receiveState === 4">
            <div class="flex flex-col items-center justify-center">
              ...
              <span class="my-[6px] flex"><AfunBaseAmount class="theme-amount" :amount="bonusAmount" :currency-code="promoCurrencyCode" /></span>
              <AfunBaseImage width="43px" url="/m/afun-h5/png/promo_money01.png" />
            </div>
          </template>
          <template v-else-if="receiveState && +receiveState === 1">
            ...
            <template v-else>
              ...
              <AfunBaseButton :loading="bonusLoading" class="mt-[10px] w-full" bg-style="secondary" size="md" @click="getBonus">
                {{ t('立即领取') }}
              </AfunBaseButton>
            </template>
          </template>
          ...
          <AfunBaseButton v-if="receiveState && +receiveState === 4" style="--afun-base-button-primary-background-color:#6D7693" class="mt-10 w-full">
            {{ t('已领取') }}
          </AfunBaseButton>
          <AfunBaseButton v-if="!receiveState || (receiveState && +receiveState !== 1 && +receiveState !== 4) || (data && +data.activity_state === 25)" class="mt-10 w-full" @click="goPromo">
            {{ t('查看更多活动') }}
          </AfunBaseButton>
        </template>
        <template v-else>
          <div class="mb-[18px] mt-[16px] font-[400]">
            {{ t('登录后查看更多内容') }}
          </div>
          <AfunBaseButton class="mt-[10px] w-full" bg-style="secondary" size="md" @click="openLogin">
            {{ t('立即登录') }}
          </AfunBaseButton>
        </template>
      </div>
    ...
    <AfunBaseTable class="table" :columns="tableColumn" last-first-padding :data-source="tableData">
      <template #deposit="{ record }">
        <div class="center">
          <span class="mr-[2px]">&ge;</span><AfunBaseAmount :amount="record.commission" :currency-code="promoCurrencyCode" />
        </div>
      </template>
      <template #reward="{ record }">
        <div class="center">
          <AfunBaseAmount :amount="record.min" :currency-code="promoCurrencyCode" />
        </div>
      </template>
    </AfunBaseTable>
    <div class="text-[#0D2245] my-[16px] mt-[20px] text-[18px] font-[600]">
      {{ t('活动规则') }}
    </div>
    <div class="mb-[24px]">
      <AfunBaseRichArea v-if="data?.promo_info?.rule_type === 2" :content="descDetail ?? ''" />
