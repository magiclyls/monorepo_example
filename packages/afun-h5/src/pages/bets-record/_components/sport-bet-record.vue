...
import { AfunBaseEmpty, AfunBaseImage, AfunBasePagination } from '@tg/bccomponents'
...
<template>
  <div>
    <template v-if="sportBetList.length">
      <AppSportsMyBetSlip v-for="item in sportBetList" :key="item.a" :data="item" class="mb-[10px]" @show="selectedItem = item; showDetail = true" />
    </template>
    <div v-else-if="loading">
      <AppSportBetRecordSkeleton :num="2" />
    </div>
    <div v-else class="overflow-hidden mt-[26px]">
      <AfunBaseEmpty :description="t('投注单为空')">
        <template #icon>
          <AfunBaseImage class="w-[112px]" url="/m/afun-h5/png/uni-empty-betslip.png" style="--tg-base-img-max-width: 100%" />
        </template>
      </AfunBaseEmpty>
      <div class="text-center text-[#fff] font-[600] mb-[31px] mt-[2px]" @click="router.push('/sports')">
        {{ t('立即开始游戏') }}
      </div>
    </div>
