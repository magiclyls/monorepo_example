...
<template>
  ...
  >
    <div class="">
      <div class="">
        <span class="text-[18px] font-semibold">{{ props.title }}</span>
        <span v-if="props.badge" class="badge ml-[0.5px]">{{ $t('已验证') }}</span>
      </div>
      <div v-if="$slots['top-desc']" class="mt-[8px] text-[14px] font-[600] text-[#6D7693]">
        <slot name="top-desc" />
      </div>
    </div>
    <slot />
