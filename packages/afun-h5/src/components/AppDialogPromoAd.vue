  ...
  <div class="h5-fixed-top bg-[rgba(0,0,0,0.4)] z-1111 center h-[100%] w-[100%]" @click="handleClose">
    <div class="margin-auto relative h-[386px] w-[332px] flex items-center justify-center">
      <div class="flex items-center justify-center">
        <AfunBaseImage width="332px" :url="`${VITE_CASINO_IMG_CLOUD_URL}/images/promo/pop/${getLangForBackend()}/${promoAdDialogProps!.ty}.webp`" @click.stop="openThrottleActivity(promoAdDialogProps)" />
      </div>
      <div
        class="center theme-close absolute right-[14px] top-[0] h-[18px] w-[18px] cursor-pointer rounded-full"
        @click="handleClose"
      >
        <IconUniClose class="text-[12px] text-[#c1c1c1]" />
      </div>
    </div>
  </div>
</template>
