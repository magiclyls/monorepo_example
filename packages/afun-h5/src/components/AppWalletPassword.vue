...
<template>
  <div class="bg-transparent rounded-[8px] p-[12px]">
    <div v-if="!isSetPayPwd && !showBind && isComponent">
      <div class="text-[#fff] text-[18px] font-[600] mb-[16px] flex items-center">
        <span>{{ t('资金密码') }}</span>
      </div>
      <div class="text-[#6D7693] text-[14px] font-[600] mb-[16px]">
        {{ t('您是首次提款,需要先设置资金密码') }}
      </div>
    </div>
    <div v-if="(getMailState && brandStore.isOpenEmailVerify && brandStore.isOpenMobileVerify)" class="text-[14px] text-[#6D7693] font-[600] text-left">
      {{ t('未绑定手机和邮箱') }}
    </div>
    <div v-else-if="brandStore.isOpenEmailVerify && !brandStore.isOpenMobileVerify && !isEmailVerify" class="text-[14px] text-[#6D7693] font-[600] text-left">
      {{ t('未绑定邮箱') }}
    </div>
    <div v-else-if="brandStore.isOpenMobileVerify && !brandStore.isOpenEmailVerify && !isPhoneVerified" class="text-[14px] text-[#6D7693] font-[600] text-left">
      {{ t('未绑定手机') }}
    </div>
    <div v-else>
      <AfunBaseLabel v-if="isSetPayPwd" :label="t('原资金密码')" required class="mb-[16px]">
        <AfunBaseInput
          ref="oldPwdRef"
      ...
      <AfunBaseLabel :label="t('新资金密码')" required class="mb-[16px]">
        <AfunBaseInput
          ref="pwdRef"
          v-model="payPassword"
          name="password"
      ...
      <AfunBaseLabel :label="t('确认资金密码')" required class="mb-[16px]">
        <AfunBaseInput
          ref="aginPwdRef"
      ...
      <div v-if="!isHideCode" class="flex gap-[20px] mb-[5px] items-end">
        <div class="flex-1">
          <AfunBaseLabel :label="isEmailCodeType ? t('邮箱验证码') : t('手机验证码')" required>
            <AfunBaseInput
              ref="mailCodeRef"
              v-model="emailCode"
              name="emailCode"
        ...
        <AfunBaseButton type="primary" :loading="sendMailCodeLoading || phoneCaptchaLoading" :disabled="isCountdown || !isEmptyEmail" class="h-[40px] w-[110px] mt-[25px]" style="--afun-base-button-font-size: 14px; --afun-base-button-font-weight: 500; ;" @click="sendCode">
          <span v-if="isCountdown">{{ `${t('重新获取')} ${current.seconds}s` }}</span>
          <span v-else>{{ t('获取验证码') }}</span>
        </AfunBaseButton>
      </div>
      <div v-if="needToVerifyPhoneOrEmail" class="flex gap-[2px] text-[14px] font-[600]" @click="isEmailCodeType = !isEmailCodeType">
        <span class="text-[#6D7693]">{{ t('没有收到验证码吗') }}</span>
        <span class="text-[#0D2245]">{{ isEmailCodeType ? t('使用手机') : t('使用邮箱') }}</span>
      </div>
      <div v-if="isSetPayPwd" class="text-right mt-[16px]">
        <RouterLink class="text-[14px] text-[#6D7693] font-[600] " to="/service">
          {{ t('忘记密码了') }}
        </RouterLink>
      </div>
    </div>
    <div v-if="showBind" class="text-center text-[#025BE8] font-[600] mt-[16px]" @click="router.push(bindUrl)">
      {{ t('前往绑定') }}
    </div>
    <div class="flex justify-end">
      <AfunBaseButton
        ...
        html-type="submit"
        :loading="payPasswordUpdateLoading"
        class="h-[42px] mt-[16px] w-full"
        :class="[isComponent && 'w-auto!']"
        style="
          --afun-base-button-font-size: 14px;
          --afun-base-button-font-weight: 600;
        "
        :style="[isComponent && '--afun-base-button-padding-x: 26px']" @click="submitPayPwd"
      >
        {{ t('确认') }}
      </AfunBaseButton>
    </div>
