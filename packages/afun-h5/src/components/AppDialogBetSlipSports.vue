  ...
  <div class="px-[16px]">
    <div class="w-full px-[16px] pb-[16px] mt-[15px]">
      <div class="flex justify-center">
        <AfunBaseButton type="none" size="none" @click="goToBet">
      ...
      <div class="text-tg-text-lightgrey text-center text-[14px] font-semibold text-[#6D7693]">
        ...
        <span class="inline-block h-[21px] font-normal">{{ t('于') }}{{ timeToFormatDiffOnChinese(data.bt) }}</span>
      </div>
    </div>
    <AppSportsMyBetSlip :data="data" is-dialog disable-result />
    <AfunBaseButton
      v-if="!isSettled && betList.length > 0" style="--afun-base-button-font-size:14px;" class="my-[16px] mx-auto"
      @click="clickHandler"
    >
