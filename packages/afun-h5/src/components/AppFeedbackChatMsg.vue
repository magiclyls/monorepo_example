...
import { AfunBaseDialog, AfunBaseImage } from '@tg/bccomponents'
...
<template>
  <div class="app-feedback-chat-msg" :class="{ 'is-own': isOwn }">
    <div class="content" :class="{ 'is-own': isOwn }">
      ...
      <slot>
        <div
          class="text-[14px] font-[600] text-[#222] bg-[#fff] rounded-[8px] p-[12px]"
        >
          <span>{{ message.content }}</span>
          <div v-if="messageImages.length" class="flex gap-[8px] flex-wrap mt-[8px]">
            <div v-for="item in messageImages" :key="item" class="w-[72px] h-[72px] bg-[#EBEBEB] rounded-[4px]">
              <AfunBaseImage
                class="size-full"
                :url="item"
                is-network
  ...
  <AfunBaseDialog v-model="showFixedImage" show-close>
    <AfunBaseImage is-network :url="curImage" />
  </AfunBaseDialog>
</template>
