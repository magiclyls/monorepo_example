        ...
        <label class="stacked">
          <div class="input-wrap" :class="{ loading }">
            <div class="input-content" :class="{ 'square-right-border': !hideCopy && msg }">
              <input
                name=""
                :value="msg" autocomplete="on" readonly class="input spacing-expanded placeholder-[#b1bad3]" type="text"
                tooltipvisible="false" :placeholder="placeholder"
              >
              <span v-show="loading" class="absolute left-[7px] top-[7px]">⌛</span>
            </div>
            <div v-if="!hideCopy" class="theme-icon button-box overflow-hidden rounded-r-[4px]">
              <AppTooltip v-if="!hideCopy && msg" :text="t('成功复制')" @click="application.copy(msg)">
                <template #content>
                  <IconUniCopy class="text-[16px] text-[#b1bad3]" />
                </template>
              </AppTooltip>
            </div>
          </div>
...
:root {
  ...
  --tg-app-copyline-font-size: 14px;
  --tg-app-copyline-color: #fff;
  --tg-app-copyline-button-padding: 12px;
  --tg-app-copy-line-input-expanded-px: 6px;
  --tg-app-copy-line-input-expanded-py: 6px;
  --tg-app-copy-line-input-background-color: #2f4553;
  --tg-app-copy-line-button-box-background-color: #2f4553;
  --tg-app-copy-line-input-border-color: #2f4553;
  --tg-app-copyline-theme-icon-color: #b1bad3;
}
</style>
