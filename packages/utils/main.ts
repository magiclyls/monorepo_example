import type {
  ...
  EnumCurrencyKey,
  EnumLanguageKeys,
  EnumOriginalLotteryIDKey,
  FlutterAppMsgType,
  FlutterAppQueryType,
  GetPath,
  GetPathValue,
  SendFlutterAppMessage,
  SendFlutterThirdPartyAppMessage,
} from '@tg/types'
import FingerprintJSLocal from '@fingerprintjs/fingerprintjs'
import * as FingerprintJS from '@fingerprintjs/fingerprintjs-pro'
import { EnumLanguage, EnumOriginalLotteryID, EventBusNames, FlutterAppQuery } from '@tg/types'
...
class Application {
   ...
   * 获取自营彩票游戏路径
   */
  getOriginalLotteryRouter(gameID: string) {
    return `?page_dialog=${EnumOriginalLotteryID[+gameID as EnumOriginalLotteryIDKey]}&type=${String(gameID)[0]}`
  }
}
...
export function getCrashPoint(gameHash: string, blockHash: string) {
  ...
  return [floor(crashPoint as any, 2).toFixed(2), crashPoint, hexStr, intVal.valueOf()]
}
