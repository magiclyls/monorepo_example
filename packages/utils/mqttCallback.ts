...
import { EventBusNames, SendFlutterThirdPartyAppMessage } from '@tg/types'
import { throttle } from 'lodash'
import { getCurrencyConfig } from './currency'
import { appEventBus } from './event-bus'
import { sendMsgToFlutterThirdPartyApp } from './main'
...
export const vipBonusNotificationsThrottle = throttle((data: any) => {
  appEventBus.emit(EventBusNames.VIP_BONUS_NOTIFICATIONS, data)
}, 1.2 * 1000, {
  leading: true,
  trailing: false,
})
 ...
 * @description 1秒内只能触发一次
 */
export const depositPwaNotificationsThrottle = throttle((msg: any) => {
  console.log(msg)
  appEventBus.emit(EventBusNames.DEPOSIT_PWA, msg)
}, 1.2 * 1000, {
  leading: true,
  trailing: false,
})
 ...
 * @description 1秒内只能触发一次
 */
export const depositLimitNotificationsThrottle = throttle((data: any) => {
  appEventBus.emit(EventBusNames.DEPOSIT_LIMIT, data)
}, 1.2 * 1000, {
  leading: true,
  trailing: false,
})
...
export function notificationMsgCallback({ topic, message }: MqttCallbackParamType) {
  ...
  if (message.parsed && message.parsed.type === 'promo') {
  ...
  else if (message.parsed && message.parsed.type === 'pwa') {
    // 存款pwa
    depositPwaNotificationsThrottle(message.parsed)
  }
  else if (message.parsed && message.parsed.type === 'limit_balance') {
    // 自我排除 -- 存款限额
    depositLimitNotificationsThrottle(message.parsed)
  }
  else if (message.parsed && message.parsed.type === 'pixel') {
    // 统计
    const obj = JSON.parse(message.parsed.payload)
    const param = {
      amount: obj.deposit_amount,
      currency: getCurrencyConfig(obj.currency_id).name,
      success: 1,
    ...
    if (obj.is_first) {
      sendMsgToFlutterThirdPartyApp(SendFlutterThirdPartyAppMessage.FIRSTRECHARGE, param)
      appEventBus.emit(EventBusNames.ADJUST, { type: SendFlutterThirdPartyAppMessage.FIRSTRECHARGE, revenue: obj.deposit_amount, currency: getCurrencyConfig(obj.currency_id).name })
    }
    else {
      sendMsgToFlutterThirdPartyApp(SendFlutterThirdPartyAppMessage.RECHARGE, param)
      appEventBus.emit(EventBusNames.ADJUST, { type: SendFlutterThirdPartyAppMessage.RECHARGE, revenue: obj.deposit_amount, currency: getCurrencyConfig(obj.currency_id).name })
    }
  }
  else {
    // 存取款通知
