...
const { cateProviderData, allPlatformList, globleDisplay_num } = storeToRefs(casinoStore)
...
const pids = ref(route.query.pid ? String(route.query.pid) : '')
      ...
      <div v-if="searchValue.length" class="mt-[22px]" :class="{ 'mt-[16px]': isMiniPhoneHeight }">
        ...
        <template v-else>
          <template v-if="searchGameList && searchGameList?.length && searchValue.length >= 3">
            <AppCardList :list="searchGameList ?? []" :ty="ty"  :display-num="globleDisplay_num" />
            <div class="mt-[22px] flex flex-col items-center justify-center text-center">
              ...
              <BaseButton
                ...
                class="mt-[16px] h-[44px] w-[184px]" bg-style="red" size="md" :loading="loadingSearchGame"
