...
import { IconUniHidden, IconUniUsers, IconChessFrame1, IconUniArrowLeft } from '@tg/icons'
...
import { globalPixi, tickerAxis } from '~/utils/mini-crash'
...
const { currentGlobalCurrency, isLogin, userInfo, balanceData, theme } = storeToRefs(appStore)

const { openCrashPointRecord } = useDialogCrashPointRecord()
const { openCrashIssueResult } = useDialogCrashIssueResult()

const { openMiniGameInfoInsertDialog } = useMiniGameInsertDialogMinesGameInfo({ gameType: GAMES_LIST_ENUM.CRASH })
const { openMiniGameHotKeysInsertDialog } = useMiniGameInsertDialogHotKeys({ gameType: GAMES_LIST_ENUM.CRASH })
...
const currentCurrency = computed(() => getCurrencyConfigByCode(currentCurrencyCode.value))
const currentBalance = computed(() => {
  if (balanceData.value && currentCurrency.value)
    return balanceData.value[currentCurrency.value.name] ?? 0
  return 0
})
...
const totalCashInAmount = computed(() => {
  if (allRateData.value && currentCurrencyCode.value) {
    return cashInList.value.reduce((acc, cur) => {
      const rate = cur.currency === currentCurrencyCode.value ? 1 : allRateData.value[cur.currency]
      return acc + (+cur.amount * (+rate))
    }, 0)
  }
  return 0
})
...
const sortedCashInList = computed(() => {
  ...
  const arr = cashInList.value.map((c) => {
    if (allRateData.value && currentCurrencyCode.value) {
      const rate = c.currency === currentCurrencyCode.value ? 1 : allRateData.value[c.currency]
      return { ...c, currentCurrencyAmount: +c.amount * (+rate) }
    }
    else {
      return c
    }
...
const ManualBetButtonStatus = computed<{ type: ManualBetButtonType, text?: string, disabled?: boolean, loading?: boolean, icon?: string, status?: boolean }[]>(() => {
  return [
    ...
    { type: 'bet_next_round', text: t('bet_next'), disabled: !gameGoing.value || betAmountRef.value?.betAmountError || +amount.value > +currentBalance.value, status: ['in_progress', 'crash', 'starting_before'].includes(renderFrame.value.current_status) && !willBet.value && !roundBetted.value },
    { type: 'bet_current_round', text: t('menu_title_settings_bets'), disabled: roundBetted.value || !gameGoing.value || betAmountRef.value?.betAmountError || +amount.value > +currentBalance.value, status: ['starting'].includes(renderFrame.value.current_status) && !willBet.value },
    { type: 'bet_cancel', text: t('cancel'), disabled: !gameGoing.value, status: ['in_progress', 'crash', 'starting_before'].includes(renderFrame.value.current_status) && willBet.value },
    { type: 'cash_out', text: t('mini_cash_out'), disabled: !gameGoing.value || !(+currentPointMult.value > 1.01) || renderFrame.value.current_status !== 'in_progress', status: ['start', 'in_progress'].includes(renderFrame.value.current_status) && roundBetted.value },
    { type: 'pending', text: t('starting_label'), disabled: true, status: ['pending', 'start'].includes(renderFrame.value.current_status) },
  ]
})
...
const AutoBetButtonStatus = computed<{ type: AutoBetButtonType, text?: string, disabled?: boolean, loading?: boolean, icon?: string, status?: boolean }[]>(() => {
  return [
    { type: 'api', icon: 'chess-frame1', disabled: !gameGoing.value, loading: originLoading.value, status: ['starting', 'in_progress'].includes(renderFrame.value.current_status) && originLoading.value },
    { type: 'cash_out', text: t('mini_cash_out'), disabled: !gameGoing.value || !(+currentPointMult.value > 1.01) || renderFrame.value.current_status !== 'in_progress', status: ['start', 'in_progress'].includes(renderFrame.value.current_status) && roundBetted.value },
    { type: 'finishing_bet', text: t('mini_auto_bet_finish'), icon: 'chess-frame1', disabled: true, status: ['in_progress', 'crash', 'pending', 'starting', 'starting_before', 'start'].includes(renderFrame.value.current_status) && autoBetCount.value > 0 && !autoActive.value },
    { type: 'start_auto_bet', text: t('mini_start_auto_bet'), status: autoActive.value !== true, disabled: renderFrame.value.current_status === 'pending' || !gameGoing.value || betAmountRef.value?.betAmountError || +amount.value > +currentBalance.value },
    { type: 'stop_auto_bet', text: t('mini_stop_auto_bet'), icon: 'chess-frame1', disabled: !gameGoing.value, status: ['start', 'in_progress', 'crash', 'starting_before', 'starting', 'pending'].includes(renderFrame.value.current_status) && autoActive.value && autoBetCount.value > 0 },
    { type: 'bet_cancel', text: t('cancel'), disabled: !gameGoing.value, status: ['start', 'starting', 'in_progress', 'crash', 'starting_before', 'pending'].includes(renderFrame.value.current_status) && autoActive.value && autoBetCount.value === 0 },
  ]
})
/** 自动投注按钮 */
...
const viewList = computed(() => [
  { label: t('control_label'), value: 'ctrl' },
...
async function betNextRound() {
  ...
  runDoBet({ type: 1, bet_amount: amount.value, currency_id: currentCurrencyCode.value, multiplier: cashOutAt.value }).then((res) => {
    ...
    if (res) {
      ...
      renderFrame.value.currencyType = getCurrencyConfigByCode(res.currency).name
      Assets.load(`/webp/currency/${res.currency}.webp`).then((tx) => {
...
function drawNetworkStatus() {
  const text = new Text('Network Status', {
    fontFamily: 'Proxima Nova',
    fontSize: 32,
    fill: globalInitMap.currentTemplate == 4 ? 0x111111 : 0xFFFFFF,
    align: 'center',
    fontWeight: '700',
  })
  text.x = 1500
...
function drawYAxisRect(y: number, val: string, index: number) {
  ...
  graphics.clear()
  graphics.beginFill(yAxisColor[theme.value]?.bg || yAxisColor.default.bg)
  ...
  const text = new Text(
    '',
    {
      fontFamily: 'Arial',
      fontSize: 32,
      fill: yAxisColor[theme.value]?.text || yAxisColor.default.text,
      align: 'center',
      fontWeight: '600',
    },
  )
...
function drawYAxisLine() {
  ...
  line.clear()
  line.beginFill(yAxisColor[theme.value]?.line || yAxisColor.default.line)
...
/** 横坐标 */
function drawXAxis() {
  const fillColor = globalInitMap.currentTemplate == 4 ? 0x111111 : 0xFFFFFF
  const container = new Container()
  container.x = 0
  container.y = 0
...
function drawCenterMult() {
  ...
  container.width = 700
  let color = globalInitMap.currentTemplate == 4 ? 0x111111 : 0xFFFFFF
  let mm
  let mstr
  let w
  let totalWidth
  ...
  tickerAxis.add(() => {
    if (renderFrame.value.x_y_points && renderFrame.value.x_y_points.length) {
      if (['crash', 'starting_before'].includes(renderFrame.value.current_status))
        ...
        color = globalInitMap.currentTemplate == 4 ? 0x111111 : 0xFFFFFF
  ...
  switch (data.type) {
    ...
    case 'cashedOut':
      ...
      if (cashOutList.value.filter(c => c.uid === bodyMsg.uid).length === 0)
        cashOutList.value.unshift({ name: bodyMsg.username, currency: bodyMsg.currency_id, amount: bodyMsg.bet_amount, hidden: !bodyMsg.username, cashOutAt: floor(+bodyMsg.cash_out_at, 2), uid: bodyMsg.uid, issue_id: bodyMsg.issue_id, payout: bodyMsg.payout })
...
watch(currentGlobalCurrency, (val) => {
  if (!tabDisabled.value)
    currentCurrencyCode.value = currencyConfig[val].cur
})
  ...
  >
    <template #left>
      <div class="game-crash-sidebar h-full w-full flex flex-col-reverse @md:flex-col" :class="[isMobile && 'crash-mobile-sidebar']">
        ...
        <BaseTab v-model="tab" :list="tabList" full :class="[orderTab]" :disabled="tabDisabled" :disable-click="tabDisabled" />
        <BaseTab v-if="tab === 'auto'" v-model="viewTab" :list="viewList" shape="square" full class="view-tab" :class="[orderTab, tabModelValue === viewTab && 'tab-active']" @change="onTabChange" />
        <div v-show="tab === 'manual' ? true : viewTab !== 'rank'" class="w-full flex flex-col" :class="[orderBetAmount]">
          <BaseLabel :label="$t('hilo_bet_amount')" :right-text="currencyPrefix + betAmountMulRate">
          ...
          <div class="mt-[12px] w-full flex items-end" :class="[tab === 'auto' && !isMobile ? 'max-w-[276px]' : '']">
            <BaseLabel :label="t('cash_out_at')" right-text="" class="w-full">
              <AppNumberCount
                ref="cashOutAtRef"
                v-model="cashOutAt"
            ...
            <template v-if="tab === 'auto'">
              <BaseLabel v-if="tabDisabled" :label="t('mini_bet_frequency')" class="ml-[8px] @md:max-w-[110px]">
                <AppMiniGamePublicBetTimes :model-value="restBetTimes" :disabled="tabDisabled" />
              </BaseLabel>
              <BaseLabel v-else :label="t('mini_bet_frequency')" class="ml-[8px] @md:max-w-[110px]">
        ...
        <div v-if="tab === 'auto' && viewTab !== 'rank'" class="w-full flex flex-col" :class="[orderMidArea]">
          ...
          >
            ...
            >
              <template #right-icon>
                <AppCurrencyIcon :currency-type="currentCurrency?.name" />
              </template>
            </BaseInput>
          </BaseLabel>
          <BaseLabel
            :label="t('mini_stop_lose')"
        ...
        <BaseLabel v-if="isMobile ? tab === 'auto' : true" :label="t('mini_win_profit')" :right-text="currencyPrefix + betAmountMulRate" :class="[orderBetProfit]">
          <BaseInput
            v-model="totalProfit" readonly
            style="--tg-base-input-style-pad-x:7px;--tg-base-input-style-pad-y:7px;--tg-base-input-line-height:1.5;"
          >
            <template #right-icon>
              <AppCurrencyIcon style="--tg-app-currency-icon-size:16px;" :currency-type="currentCurrency?.name" />
            </template>
          </BaseInput>
        </BaseLabel>
        <div class="bet-button w-full" :class="[orderBetButton]">
          <AppMiniGamePublicBetButton class="h-[52px] w-full" :loading="currentButton?.loading" :disabled="currentButton?.disabled || currentButton?.loading" :game="GAMES_LIST_ENUM.CRASH" @bet-btn-click="clickBetButton">
            <div class="flex items-center text-[16px]">
              <div>{{ currentButton?.text }}</div>
              <div v-if="currentButton?.icon" class="wobble ml-8px flex items-center text-[16px]">
        ...
        <div v-if="tab === 'manual' || (tab === 'auto' && viewTab === 'rank')" class="w-full flex grow flex-col" :class="[orderRanking]">
          <div class="ranking text-tg-text-white mb-[12px] flex items-center justify-between text-sm font-semibold">
            <div class="flex items-center">
              <IconUniUsers /><span class="ml-8px mr-[16px]">{{ cashInList.length }}</span>
            </div>
            <div>
              <AppAmount reverse :amount="totalCashInAmount" :currency-code="currentCurrency?.cur" />
            </div>
          </div>
          <div class="ranking-table bg-tg-secondary-dark h-[230px] w-full grow overflow-hidden rounded">
            <div class="scroll-y scroll-wrapper h-full w-full overflow-x-hidden pb-[8px]" @scroll="rankTableScroll">
              <table class="h-min min-w-250px w-full">
                <template v-for="(item, idx) in uniqSortedCashInList" :key="item.uid">
                  <tr v-if="idx < 30" class="text-tg-secondary-light text-sm font-semibold" :class="[`uid_${item.uid}`]">
                    <td class="w-[40%] pl-[12px]">
                      <div v-if="item.hidden" class="hoverable">
                        <VTooltip placement="top" :triggers="['click', 'hover']">
                          <div class="ghost hidden-user flex items-center text-sm font-semibold">
                            <IconUniHidden />
                            <span class="ml-4px">{{ t('hidden_user') }}</span>
                          </div>
                          <template #popper>
                      ...
                      <div v-else class="overflow-hidden pr-4px">
                        <div class="max-w-[96px] overflow-hidden text-ellipsis whitespace-nowrap">
                          {{ item.name }}
                        </div>
                      </div>
                    </td>
                    <td class="text-tg-text-white width-[20%]">
                      <span v-if="+item.cashOutAt > 0">{{ item.cashOutAt }}x</span>
                      <span v-else>-</span>
                    </td>
                    <td class="text-tg-text-white width-[40%] pr-[8px]">
                      <VTooltip placement="top" :triggers="['click', 'hover']">
                        <AppAmount :amount="+item.cashOutAt > 0 ? (+item.amount) * (+item.cashOutAt) : item.amount" :currency-code="item.currency" reverse show-prefix :show-color="+item.cashOutAt > 0" />
                        <template #popper>
                          <div class="tiny-menu-item-title">
                            <AppAmount show-prefix :amount="+item.cashOutAt > 0 ? (+item.amount) * (+item.cashOutAt) : item.amount" :currency-code="item.currency" />
                          </div>
                        </template>
                      </VTooltip>
                    </td>
    ...
    <template #right>
      <div ref="gameCrashWrap" class="mini-game-crash-outer relative w-full flex grow grow flex-col justify-center" :style="{ minHeight: `${minHeightGameRoot}px` }">
        <div class="relative right-[1px] top-[1px] z-[1] flex flex-row-reverse">
          <div class="side-btns ml-[8px] flex flex-col justify-between">
            <BaseButton size="xs" round style="--tg-icon-color: white;" @click="openCrashPointRecord">
              <IconChessFrame1 class="text-tg-text-white text-xs" />
            </BaseButton>
            <span class="text-tg-secondary-light cur-user mt-[4px] flex items-center text-xs">
              <IconUniArrowLeft />
              <span>{{ t('you') }}</span>
            </span>
          </div>
          <div class="past-bets w-full flex flex-row-reverse">
            <template v-for="(item, idx) in crashPointList" :key="item.issue_id + item.crash_point">
              <div v-if="idx <= 5" class="wrap flex flex-col items-center">
                <BaseButton :bg-style="+item.crash_point >= 2 ? 'secondary' : 'default-btn'" round @click="() => openCrashIssueResult(item.issue_id)">
                  {{ floor(+item.crash_point, 2).toFixed(2) }}x
                </BaseButton>
                <span v-if="+item.cashOutAt > 0" class="text-tg-text-green mt-[4px] text-center text-xs leading-18px">
        ...
        <div class="mini-game-crash h-full w-full flex grow flex-col items-center justify-between">
          <base-aspect-ratio ratio="3/2">
            <div ref="crashAppContainer" class="wrapper h-full w-full flex items-center justify-center" />
          </base-aspect-ratio>
          <div class="fake" />
        </div>
        <div v-if="showPlayerResult" class="cashout-wrapper absolute right-[1px] top-[45%] flex origin-right scale-50 gap-[8px] transition-opacity duration-500 @md:scale-100" :class="[['starting', 'pending', 'start'].includes(renderFrame.current_status) || cashOutList.length === 0 ? 'opacity-0' : 'opacity-100', isMobile ? 'bottom-[8px]' : '']">
          <div class="cashout-listing height-[15rem] flex flex-col items-end">
            <div v-for="(item, idx) in cashOutList" :key="idx" class="wrap text-tg-secondary-light index text-sm font-semibold" :class="[idx < cashOutList.length - 1 ? 'mb-8px' : '']">
              <div v-if="item.hidden" class="hoverable">
                <div class="ghost hidden-user-2 flex items-center">
                  <IconUniHidden />
                  <span class="ml-[4px] text-sm">{{ t('hidden_user') }}</span>
                </div>
              </div>
              <div v-else>
                <div class="text-sm">
              ...
              <VTooltip placement="top" :triggers="['click', 'hover']">
                <AppAmount :amount="item.payout" :currency-code="item.currency" reverse show-prefix show-color />
                <template #popper>
                  <div class="tiny-menu-item-title">
                    <AppAmount :amount="item.payout" :currency-code="item.currency" show-prefix />
                  </div>
                </template>
              </VTooltip>
            </div>
...
.cashout-listing {
  .wrap.index {
    ...
    @include webTheme('white') {
      --tg-app-amount-green-color: #f2ca5c;
      background: #585858;
      color: #ffffff;
      border-color: #585858;
    }
...
.past-bets {
  @include webTheme('white') {
    --tg-base-button-style-bg-special: #ffaa09;
    --tg-base-button-style-bg-special-hover: #ffaa09;
  }
...
.game-crash-sidebar {
  ...
  @include webTheme('green') {
    --tg-theme-border-width: 1px;
    --tg-theme-border-color: #5ecea6;
...
.view-tab {
  ...
  --tg-tab-style-active-bg: var(--tg-secondary-dark);
  --tg-tab-style-wrap-bg-color: var(--tg-secondary-main);
