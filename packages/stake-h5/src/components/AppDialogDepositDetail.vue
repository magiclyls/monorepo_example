...
<template>
  <div class="dialog-deposit-detail">
    <div class="title">
      <span v-show="!isCoin" class="prefix">{{ getPrefix() }}</span>
    ...
    <div class="content">
      ...
      <div class="item">
        ...
        <div class="center icon-s" style="gap: var(--tg-spacing-4);">
          <AppCurrencyIcon v-if="!getStateIcon(data.state)" :currency-type="data.currency_name" />
          <component v-else :is="getStateIcon(data.state)" />
      ...
      <template v-if="isHasLimit">
         <!-- 存款金额: -->
      <div class="item">
        <label>{{ $t('deposit_amount_label') }}:</label>
        <span>{{ data.pay_amount }}</span>
      </div>
      <!-- 预扣金额 -->
      <div class="item">
        <label>{{ $t('withholding_amount') }}:</label>
        <span>{{ data.limit_amount }}</span>
      </div>
      <!-- 实际到账 -->
      <div class="item">
        <label>{{ $t('actual_amount') }}:</label>
        <span>{{ data.actual_amount }}</span>
      </div>
      </template>
    ...
    <BaseButton size="md" bg-style="default-yellow" class="th-leading-5 help-btn" @click="onlineHelp">
