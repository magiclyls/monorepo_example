...
const { openMiniGameInfoInsertDialog } = useMiniGameInsertDialogMinesGameInfo({ gameType: GAMES_LIST_ENUM.MINES })
const { openMiniGameHotKeysInsertDialog } = useMiniGameInsertDialogHotKeys({ gameType: GAMES_LIST_ENUM.MINES })
...
<template>
  <AppMiniGamePublicLayout
    v-model:animate-enabled="animateEnabled" :game-type="GAMES_LIST_ENUM.MINES"
    ...
    <template #left>
      <div class="w-full flex flex-col gap-[12px]">
        ...
        <div class="flex-col-12 order-2 min-h-[59px] flex flex-col">
          <BaseLabel v-show="!isManualStart" class="order-2" label="Mines">
            <BaseSelect
              v-model.number="mines"
              class="theme-select" :disabled="gameStart || orderLoading || initLoading"
              :options="minesList" style="--tg-base-select-style-padding-y:8.5px;"
            />
          </BaseLabel>
          <div v-show="isManualStart" class="flex-col-12 order-2 flex flex-col">
            <!-- 手动下注游戏开始 -->
            <div class="flex-row-8 order-2 w-full flex items-center">
              <BaseLabel class="order-2 grow" label="Mines">
                <BaseInput
                  v-model="mines" readonly
                  style="--tg-base-input-style-pad-x:7px;--tg-base-input-style-pad-y:7px;--tg-base-input-line-height:1.5;"
                />
              </BaseLabel>
              <BaseLabel class="order-2 grow" :label="t('mini_diamond')">
                <BaseInput
            ...
            <BaseLabel class="order-2" :label="t('mini_total_profit', { profit: payoutMultiplierFixed })">
              <BaseInput
        ...
        <div v-show="isAutoBet" class="order-2 flex flex-col gap-[12px]">
          ...
          >
            <BaseInput
              ...
              class="theme-input" type="number" input-mode="decimal" :disabled="gameStart"
              style="--tg-base-input-style-pad-x:7px;--tg-base-input-style-pad-y:7px;--tg-base-input-line-height:1.5;"
        ...
        <AppMiniGamePublicBetButton
          ...
          :game="GAMES_LIST_ENUM.MINES" :loading="orderLoading || initLoading" @bet-btn-click="makeOrder"
        >
          {{ t('mini_touzhu') }}
        </AppMiniGamePublicBetButton>
        <AppMiniGamePublicBetButton
        ...
        <AppMiniGamePublicBetButton
          ...
          :disabled="isLogin ? chosenPositions.length === 0 || isFinishingBetting : false" @bet-btn-click="startAutoBet"
        >
          {{ autoBetBtnText }}
        </AppMiniGamePublicBetButton>
      </div>
      ...
      >
        ...
        <transition name="fade">
          <div v-if="settleAmount" class="absolute h-full w-full flex items-center justify-center">
            <div
              class="shadow-tg-box-shadow-lg tg-settle-amount-theme max-w-[100%] min-w-[150px] border-[3px] rounded-[8px] border-solid text-center text-[14px] font-extrabold"
            >
              <div class="w-full flex flex-col items-center px-[1em] py-[0.75em] text-center">
                <span class="text-[2em] font-bold leading-[1.5]" style="font-family: proxima-nova,sans-serif;">{{ payoutMultiplierFixed }}×</span>
                <div class="tg-just-a-line mb-[8px] mt-[4px] h-[3px] w-[30%]" />
                <AppAmount style="font-family: proxima-nova,sans-serif;" class="leading-[1.5]" show-color :amount="settleAmount" :currency-type="orderCurrencyType" />
              </div>
            </div>
          </div>
        </transition>
...
.tg-settle-amount-theme {
  ...
  @include webTheme('green') {
    ...
    color: #eca30c;
    --tg-app-amount-green-color: #eca30c;
  }
  @include webTheme('white') {
    border-color: #f2ca5c;
