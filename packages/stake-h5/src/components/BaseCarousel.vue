...
async function slideToPrev() {
  ...
  setSTrue()
  if (active.value > 0) {
    const temp = trackX.value + outerWidth.value
    await gsap.fromTo(swiperTrack.value, {
      transform: `translate3d(${trackX.value}px, 0px, 0px)`,
      duration: 0.8,
    }, {
      transform: `translate3d(${temp}px, 0px, 0px)`,
    })
    trackX.value = temp
    active.value -= 1
    await nextTick()
    ...
    await nextTick()
    const temp = -(_data.value.length - 2) * outerWidth.value
    await gsap.fromTo(swiperTrack.value, {
      transform: `translate3d(${trackX.value}px, 0px, 0px)`,
      duration: 0.8,
    }, {
      transform: `translate3d(${trackX.value + outerWidth.value}px, 0px, 0px)`,
    })
    trackX.value = temp
...
async function mouseUpEve(event: MouseEvent) {
  setDFalse()
  touchEndPoint.value = event.clientX
  if (trackX.value > 0) {
    const temp = 0
    await gsap.fromTo(swiperTrack.value, {
      transform: `translate3d(${trackX.value}px, 0px, 0px)`,
      duration: trackX.value / outerWidth.value * 800,
    }, {
      transform: `translate3d(${temp}px, 0px, 0px)`,
    })
    trackX.value = temp
    ...
    if (trackX.value < minTrack) {
      await gsap.fromTo(swiperTrack.value, {
        transform: `translate3d(${trackX.value}px, 0px, 0px)`,
        duration: (-trackX.value + minTrack) / outerWidth.value * 800,
      }, {
        transform: `translate3d(${minTrack}px, 0px, 0px)`,
      })
      ...
      if (Math.abs(dragDirection.value) >= outerWidth.value / 4) {
        if (dragDirection.value > 0) {
          const temp = trackX.value + (outerWidth.value - dragDirection.value)
          await gsap.fromTo(swiperTrack.value, {
            transform: `translate3d(${trackX.value}px, 0px, 0px)`,
            duration: (outerWidth.value - dragDirection.value) / outerWidth.value * 0.8,
          }, {
            transform: `translate3d(${temp}px, 0px, 0px)`,
          })
          trackX.value = temp
        ...
        const temp = trackX.value - dragDirection.value
        await gsap.fromTo(swiperTrack.value, {
          transform: `translate3d(${trackX.value}px, 0px, 0px)`,
          duration: dragDirection.value / outerWidth.value * 0.8,
        }, {
          transform: `translate3d(${temp}px, 0px, 0px)`,
        })
        trackX.value = temp
...
<template>
  <div class="swiper-wrapper" :class="[`swiper-${mode}`]">
    <div ref="swiperOuter" class="swiper-outer center-mode" :style="{
      '--swiper-outer-width': `${outerWidth}px`,
    }">
      <div v-if="_data.length > 1" class="arrows-overlay">
        <div class="left">
          <BaseButton type="text" size="none" style="font-size: inherit;" @click="slideToPrev">
            <IconUniArrowLeft name="uni-arrow-left" />
          </BaseButton>
        </div>
        <div class="right">
          <BaseButton type="text" size="none" style="font-size: inherit;" @click="slideToNext">
            <IconUniArrowRight name="uni-arrow-right" />
          </BaseButton>
        </div>
      </div>
      <div class="swiper-track-wrap" :class="{ dragging: isDragging }">
        <div ref="swiperTrack" class="track" :style="{
          width: `${outerWidth * _data.length}px`,
          transform: `translate3d(${trackX}px, 0px, 0px)`,
          // 'transition-duration': `${duration}ms`,
        }" @mousedown="mouseDownEve" @mouseup="mouseUpEve" @mousemove="mouseMoveEve" @mouseleave="mouseLeaveEve">
          <div v-for="item, idx in _data" :key="`${idx}_${item.value ?? item.id}`" class="slide-wrap" :class="[
