...
watchEffect(() => {
  ...
  galleryClass.value = props.displayNum === 4 ?  'card-list-700-500' : props.displayNum === 2 ? 'card-list-300' : 'card-list-500'
})
</script>
  ...
  >
    <template v-if="list && list.length > 0">
      <div v-for="item in list" :key="item.id" class="wrap">
        <slot :item="item">
          <BaseGameItem :game-info="item" :show-man-count="showManCount" :ty="ty" />
        </slot>
      </div>
    </template>
  </div>
