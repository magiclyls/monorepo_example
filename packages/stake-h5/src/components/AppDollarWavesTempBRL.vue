...
const descDetailCondition = computed(() => [
  ...
  t('brl_desc_3'),
  t('brl_desc_4'),
  t('brl_desc_5', { n: detailData.value.multiple }),
].join('\n'))

const qaList = computed(() => [
  { q: t('brl_qa_q_0'), a: t('brl_qa_a_0') },
  { q: t('brl_qa_q_1'), a: t('brl_qa_a_1') },
  { q: t('brl_qa_q_2'), a: t('brl_qa_a_2') },
  { q: t('brl_qa_q_3'), a: t('brl_qa_a_3') },
  { q: t('brl_qa_q_4'), a: t('brl_qa_a_4') },
  { q: t('brl_qa_q_5'), a: t('brl_qa_a_5', { n: detailData.value.multiple }) },
])
...
// const notInRangeScope = computed(() => ['00:00', timeScopes.value[0][0]]) // >=   <
  ...
  if (pid.value) {
    ...
    runGetBonus(pid.value, code, 0).then((res) => {
      ...
      if (res && res.amount && +res.amount > 0) {
        openDialogDollarRain({ pid: pid.value, isBRL: true, isCrystal: false })
      }
      else {
        openDialogDollarRain({ pid: pid.value, isBRL: true, isCrystal: false })
        ...
        openDialogDollarWaveBonus({ pid: pid.value, firstBonusData: { ...res, currencyCode: code }, bonusData: { ...res, currencyCode: code }, nextScope: nextScope?.join('-'), isBRL: true, closeCb: closeDialogDollarRain, currencyName: detailData.value?.conf.currency })
      }
    }).catch(() => {
      const nextScope = timeScopes.value.filter((item: any) => localHM.value < item[0])[0] ?? timeScopes.value[0]
      if (nextScope)
        nextScope[1] = nextScope[1] === '23:59' ? '23:59' : nextScope[1]
      appEventBus.emit(EventBusNames.USER_GETBONUS_RESULT, bonusData.value)
      openDialogDollarWaveBonus({ pid: pid.value, firstBonusData: { }, bonusData: { state: 1 }, nextScope: nextScope?.join('-'), isBRL: true, currencyName: detailData.value?.conf.currency })
    })
  }
}
...
function initJudgeBonus({ p, notify, showAmount }: { p?: string, notify?: boolean, showAmount?: boolean }) {
  ...
  if (_p) {
    runGetBonus(_p, globalCurrencyCode.value, 0).then((res) => {
      if (res && +res.state === 4) {
        if (notify)
...
watch(globalCurrencyCode, (val, old) => {
})

// const ruleDesc = computed(() => [
//   t('brl_wave_desc', [timeScopes.value?.length]),
//   t('brl_time_zone_label'),
// ].join('\n'))
...
<template>
  <div class="dollar-waves text-tg-secondary-light m-auto mb-[32px] max-w-[650px] pb-[16px] font-medium @md:mt-[32px]" :class="[`lang-${lang}`]" @touchstart="pageClicked" @mousemove="pageClicked" @click="pageClicked">
    ...
    <div class="activity-desc bg-tg-secondary-dark my-[16px] rounded px-[12px] py-[17px]">
      ...
      <div class="text-tg-text-lightgrey w-full text-[14px] font-medium leading-[1.5]">
        <ul class="w-full flex flex-col list-disc pl-[16px]">
          ...
          <li>
            <span>{{ t('brl_time_zone_label', { timezone: t(TIMEZONE_LABEL_KEY[currentDollarZone]) }) }}</span>
          </li>
        </ul>
      </div>
      <!-- <div class="text-tg-secondary-light mt-[8px] text-[14px] font-semibold leading-[21px]">
        {{ t('act_desc_dollar', [timeScopes?.length]) }}
        <div class="text-tg-secondary-light mt-[8px] text-[12px] font-semibold leading-[21px]">
          {{ t('time_zone_label') }}
        </div>
      </div> -->
    </div>
    <div v-if="timeScopes && timeScopes.length" class="bg-tg-secondary-dark bonus-time my-[16px] rounded px-[12px] py-[17px]">
      <div class="text-tg-text-white pb-[6px] text-[20px] font-semibold leading-[28px]">
        {{ t('act_time_title') }}
      </div>
      <section class="all-box gap-y-[6px] @md:gap-y-[16px]">
        <template v-for="(i, idx) in timeScopes" :key="i + idx">
          <div>
            <div
              class="b-time-item relative mx-auto h-[20.53vw] max-h-[77px] max-w-[84px] w-[22.4vw] cursor-pointer text-center font-semibold"
              :class="[
                (isExpire(i) || isReceive(i)) && 'text-tg-text-white theme-text',
                isReceive(i) && 'bonus-received yes-received',
                isExpire(i) && 'mounted-active-tag has-b',
              ]"
              @click="() => littleBusterd(i)"
            >
              <BaseAspectRatio ratio="84/41">
                <div class="brl-bg" />
              </BaseAspectRatio>
              <div class="whitespace-nowrap text-[12px] leading-[20px]">
    ...
    <div class="bg-tg-secondary-dark rounded px-[12px] py-[17px]">
    ...
    <div class="bg-tg-secondary-dark my-[16px] rounded px-[12px] py-[17px]">
      ...
      <section class="qa-list flex flex-col">
        ...
        >
          <div class="question flex items-start">
            <div class="text-tg-text-white qa-tag mr-[8px] mt-[2px] h-[16px] w-[16px] rounded-sm bg-[#05A80F] text-center text-[10px] font-medium leading-[16px]">
              {{ t('q_label') }}
            </div>
            <div>{{ item.q }}</div>
          </div>
          <div class="answer mt-[4px] flex items-start">
            <div class="qa-tag text-tg-text-white mr-[8px] mt-[2px] h-[16px] w-[16px] rounded-sm bg-[#1475E1] text-center text-[10px] font-medium leading-[16px]">
              {{ t('a_label') }}
            </div>
            <div class="flex-1">
              {{ item.a }}
...
.b-time-item {
  .brl-bg {
    ...
    @include getBackgroundImage('/promotions/brl-time-1');
  }
  // &.mounted-active-tag {
  //   background-image: url(/webp/promotions/dollar_bg.webp);
  // }
  &.has-b {
    .brl-bg {
      @include getBackgroundImage('/promotions/brl-time-2');
    }
  }
  &.bonus-received.yes-received {
    .brl-bg {
      @include getBackgroundImage('/promotions/brl-time-0');
    }
  }
  &.mounted-active-tag {
    .lucky-hand {
