    ...
    <div class="header">
      ...
      <div class="right-wrap flex">
        <div v-if="showArrow" class="arrows">
          <BaseButton v-if="total" type="square-line" class="center h-[27px]"
            style="--tg-base-button-color:var(--tg-text-lightgrey);--tg-base-button-padding-x:18px; --tg-base-button-font-size:14px;--tg-base-button-square-line-border-color:var(--tg-border-color-grey-light);"
            @click="onTitleClick">
        ...
        <AppSliderMultiLineComponents v-for="line, lineIdx in data" :key="lineIdx"
          ...
          <template #item="{ item, index }">
            <slot name="item" :item="item" :loading="index < pageInfo.pageSize ? 'eager' : 'lazy'">
              <BaseBcGameGameItem :game-info="item" :loading="index < pageInfo.pageSize ? 'eager' : 'lazy'"
