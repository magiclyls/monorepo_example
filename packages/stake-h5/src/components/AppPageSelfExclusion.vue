...
<template>
  <div class="flex flex-col gap-[16px] pb-[32px]">
    ...
    <div class="box-outer">
      <div class="px-[16px]">
        <h6 class="text-[20px] font-[600] text-[#fff] leading-[28px] mb-[16px]">{{ t('suspend_betting') }}</h6>
      ...
      <div class="flex justify-end px-[16px]">
        <BaseButton v-if="isStopBetOver" bg-style="secondary" class="h-[44px]" style="--tg-base-button-min-width:124px;"
          @click="openDialog(1)">
          {{ t('request_to_suspend_betting') }}
        </BaseButton>
        <div v-else
          class="h-[44px] leading-[44px] whitespace-nowrap px-[20px] text-[#B1BAD3] font-[600] text-[14px] rounded-[4px] bg-[#2F4553] border-[1px] border-dashed border-[#B1BAD3]">
    ...
    <div class="box-outer">
      <div class="px-[16px]">
        <h6 class="text-[20px] font-[600] text-[#fff] leading-[28px] mb-[16px]">{{ t('self_exclusion') }}</h6>
        <div class="text-[14px] text-[#B1BAD3] font-[500] flex flex-col gap-[20px] leading-[1.5]">
      ...
      <div class="line"></div>
      <div class="flex justify-end px-[16px]">
        <BaseButton bg-style="secondary" class="h-[44px]" style="--tg-base-button-min-width:124px;"
          @click="openDialog(2)">
          {{ t('extend_btn') }}
        </BaseButton>
      </div>
    ...
    <BaseDialog show-close :showIcon="step !== 1" :icon="step !== 1 ? IconUniArrowLeft : ''" :title="dialogTitle"
      ...
      <div v-if="step === 1" class="flex flex-col gap-[16px] pt-[6px] px-[16px] pb-[16px]">
        <BaseLabel :label="t('duration')" must>
          <BaseSelect class="mt-[4px]" v-model="durationTimeStamp" :options="durationTimeStampList" border popper
            :autoSize="true" style="--tg-base-select-popper-style-padding-x:12px;" />
        </BaseLabel>
        <BaseButton bg-style="secondary" size="md" class="h-[44px] w-full" @click="sendCode">
