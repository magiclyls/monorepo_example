...
function downLoad() {
  if (window.downloadStore.downLoad(2) === 'ShowSafari')
  ...
  <div>
    ...
    >
      <div class="mb-[16px] flex items-center">
        ...
        <div class="text-tg-text-white font-medium">
          ...
          <div :style="{ color: dialogDownLoadData.contentColor }" class="leading-[22px]">
            {{ t('pwa_dialog_content') }}
          </div>
        </div>
      </div>
      <div
      ...
      >
        ...
        <span :style="{ color: dialogDownLoadData.buttonTextColor }" class="ml-[4px] font-semibold">{{ t('download') }}</span>
      </div>
    </div>
  </div>
  <div class="flex items-center justify-center" @click="closeIconClick">
    <IconDialogDownloadClose style="font-size: 30px;color: var(--app-downLoad-dialog-close-icon-color)" />
  </div>
