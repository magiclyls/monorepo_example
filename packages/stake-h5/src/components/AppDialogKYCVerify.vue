...
<template>
  <div class="px-[16px] py-[20px]">
    <template v-if="!cpfNumber">
      ...
      <BaseLabel label="CPF">
        <BaseInput v-model="cpf" max="11" input-mode="numeric" :msg="cpfError" :placeholder="t('setting_kyc_cpf_input_pls')" />
      </BaseLabel>
      <p class="theme-text my-[16px] text-[14px] font-medium leading-[21px]">
        {{ t('setting_kyc_tip1') }}{{ t('setting_kyc_tip2') }}
      </p>
      <BaseButton bg-style="secondary" class="h-[44px] w-full" @click="saveCPF">
        {{ t('confirm_submit') }}
      </BaseButton>
    </template>
    <template v-else>
      ...
      <BaseButton bg-style="secondary" class="h-[44px] w-full" @click="verifyDocument">
