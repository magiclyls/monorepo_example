...
const { bool: isDefault } = useBoolean(!!customRouter.query.isFirst || false)
const isEdit = ref(customRouter.query.isEdit as unknown as boolean || false) // 是否是编辑状态  true 编辑状态 false 新增状态  默认是新增状态
const initData = ref(customRouter.query.data as unknown as VirtualCoin)
...
const password = ref('')
const _currencyId = ref(customRouter.query.currencyId as unknown as CurrencyCode)
const virCurrencyList = ref(customRouter.query.virCurrencyList as unknown as ICurrencyOption[])
const currentNetwork = ref(customRouter.query.contractId as string)
...
const {
  ...
  onSuccess() {
    ...
    if (curCurrencyItem.value)
      customRouter.back({ activeTab: 'withdraw', initCurrency: curCurrencyItem.value.currency_name })
  },
})
...
watch(() => customRouter.query.contractId, () => {
  resetAddress()
  currentNetwork.value = customRouter.query.contractId
})
