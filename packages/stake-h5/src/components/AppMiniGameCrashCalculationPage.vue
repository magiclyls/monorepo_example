...
<template>
  <!-- Plinko -->
  <BaseLabel class="mb-[16px]" :label="$t('hash_short_label')">
    <BaseInput v-model="params.hash" type="text" msg-after-touched />
  </BaseLabel>
  <BaseLabel class="mb-[16px]" :label="$t('plinko_fairnesses_seed')">
    <BaseInput v-model="params.base_seed" type="text" msg-after-touched />
  </BaseLabel>

  <!-- 结果 -->
  <div class="border-tg-secondary result-root min-h-[200px] flex flex-col items-center justify-center border-[2px] rounded-[4px] border-dotted p-[16px]">
  ...
  <template v-if="result">
    ...
    >
      ...
      <div>
        ...
        <div class="scroll-x text-tg-text-white text-sm font-medium leading-[21px]">
          <span>HMAC_SHA256({{ params.hash }}, {{ params.base_seed }})</span>
          <table>
            <tr>
              <template v-for="(c, cdx) in result[2]" :key="cdx">
                <td v-if="cdx % 2 === 0" class="text-center" :class="[cdx > 0 ? 'pl-[8px]' : '']">
      ...
      <div>
        ...
        <div class="scroll-x">
          <p class="text-tg-secondary-light text-sm font-semibold">
            4294967296
            / (
