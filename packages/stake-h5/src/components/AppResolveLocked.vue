...
<template>
  <div class="theme-input grid gap-y-[12px] p-[16px]">
    ...
    <template v-if="page == 'self'">
      ...
      <BaseButton bg-style="secondary" size="md" style="--tg-base-button-font-size:16px;" @click="onResolve">
        {{ t('submit') }}
      </BaseButton>
    </template>
    ...
    <template v-if="page == 'long'">
      ...
      >
        <IconSmallError style="--tg-icon-color:#ED4163;" name="small-error" /><span class="ml-[4px]">{{ t('long_no_login_warn_word') }}</span>
      </div>
    </template>
