...
import type { IAvailableCurrency, BankCard, EnumCurrencyKey, IWithdrawBankOrWalletItem } from '@tg/types'
...
interface Props {
  initFiatCurrency: IAvailableCurrency
  /* 可取款法币列表 */
  // fiatCurrencyList: ICurrencyOption[]
}
...
const { isLogin, userInfo, balanceData, readyGetUpayBalance, uPayBalance, loadingUpayBalance, loadingUpayUrl, uPayUrl, upayBalErrCode, isCompleteKYC } = storeToRefs(appStore)
...
/** 当前选中的货币 */
const activeFiatCurrency = ref(props.initFiatCurrency)
/** 货币搜索值 */
...
function toAddPix() {
  ...
  customRouter.push('/widthdraw/bebank', {
    ...
    withdrawTypeList: withdrawTypeListOptions.value,
  })
}
// 编辑PIX
...
function toAddBankcards() {
  ...
  customRouter.push('/widthdraw/bebank', {
    ...
    currencyName: activeFiatCurrency.value.currency_name,
     withdrawId: currentWithdrawId.value,
    withdrawTypeList: withdrawTypeListOptions.value,
    needDisabledBankIds: memberThreePartyWalletList.value.filter(a => a.cardCount >= 3).map(b => b.bank_id),
    isFullBindedAllWallet: isFullBindedAllWallet.value,
  })
}
// 编辑银行卡
...
<template>
  <div class="wallet-wrap flex flex-col gap-[16px]">
    ...
    <BaseTab
      ...
      style="--tg-tab-style-inner-padding-y:9px;--tg-tab-style-box-padding:4px;" @change="onWithdrawTypeChange"
    />
    <!-- 表单 -->
      ...
      <template v-if="fiatCurrencyList && fiatCurrencyList.length">
        ...
        <template v-if="numberOfAccountListOnCurrentWithdrawType">
          ...
          <div v-if="isEmbed" class="min-h-[32px] flex items-center justify-between">
            ...
            <BaseButton :loading="loadingUpayUrl" class="buy-btn min-w-[78px] flex items-center justify-center" style="--tg-base-button-min-width:78px" bg-style="secondary" size="none" @click="goBuyUpay">
          ...
          <BaseButton
            ...
            bg-style="secondary" size="md" @click="withDrawSubmit"
          >
            {{ t('confirm_submit') }}
          </BaseButton>
        </template>
        ...
        <template v-else>
          ...
          <BaseButton style="--tg-base-button-font-size:16px;" bg-style="secondary" size="md" @click="bind">
