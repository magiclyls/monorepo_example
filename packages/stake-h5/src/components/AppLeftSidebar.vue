      ...
      <template v-if="!isSwitching">
        <div class="header">
          ...
          <div class="game-type">
            ...
            >
              <span style="text-align: center;">{{ t(n.title).toLocaleUpperCase() }}</span>
            </div>
          </div>
        </div>
      </template>
    ...
    <div class="grid grid-cols-2 w-full gap-[8px]">
      <BaseAspectRatio v-for="n in navButtons" :key="n.title" ratio="3.5/1">
        <div
          ...
          @click="push(n.title)"
        >
          <span class="text1">{{ t(n.title).toLocaleUpperCase() }}</span>
        </div>
      </BaseAspectRatio>
    </div>
  </div>

  <div class="content scrollY" @touchstart.stop @touchmove.stop>
