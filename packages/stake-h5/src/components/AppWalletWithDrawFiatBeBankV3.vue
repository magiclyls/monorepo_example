...
const initData = ref(customRouter.query.data as unknown as BankCard)
const isEdit = computed(() => customRouter.query.isEdit as unknown as boolean)
const isFirst = computed(() => customRouter.query.isFirst as unknown as boolean)
const needDisabledBankIds = ref<number[]>(customRouter.query.needDisabledBankIds as unknown as number[])
const currencyId = ref<CurrencyCode>(customRouter.query.currencyId as CurrencyCode)
const currencyName = ref(customRouter.query.currencyName as unknown as string)
const withdrawTypeList = ref(customRouter.query.withdrawTypeList as unknown as IWithdrawTypeListItem[])
const isFullBindedAllWallet = ref(customRouter.query.isFullBindedAllWallet as unknown as boolean)
/** 提款方式 */
...
const openName = computed(() => customRouter.query.openName || realNameMap.value[currencyId.value])
...
/** 当前是支付宝 */
const isAlipayType = computed(() => withdrawType.value === 2 && !isIndiaCard.value)
/** 当前是钱包支付 */
...
const {
  ...
  onSuccess() {
    ...
    customRouter.back({ activeTab: 'withdraw', initCurrency: currencyName.value })
  },
})
/** 编辑银行卡 */
const {
...
async function onSubmit() {
  ...
  if (!isEdit.value) {
    ...
    runBankcardInsert({
      ...
      type_id: currentWithdrawId.value,
      phone: phone.value,
      email: email.value,
  ...
  <div>
    <template v-if="isSetPayPwd">
      <form class="flex flex-col gap-[16px] pt-[16px]">
        ...
        <BaseButton
          style="--tg-base-button-font-size:16px;" :loading="bankcardInsertLoading || updateBankcardLoading" bg-style="secondary"
