...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const renderButton = () => {
      ...
      return (
        <div
          ref={slider}
          role="slider"
          class={`absolute h-[40px] w-[40px] flex items-center justify-center rounded-[50%] border-[2px] border-[var(--app-sports-slider-border)] border-solid bg-white ${disabledMove.value ? '' : 'cursor-pointer'}`}
    ...
    return () => (
      <div
        class={`w-full ${props.isClosed || (props.isMulti && Number(props.count) < 2) ? 'bg-tg-secondary-grey' : 'bg-[var(--app-sports-slider-active-bg)]'} rounded-[31px] h-[48px] relative ${{ disabled: props.disabled }}`}
      >
        ...
        {!props.loading && (
          ...
          <div class="h-full w-full flex items-center justify-between pl-[48px] pr-[22px]">
            {!props.isMulti && !props.isClosed && (
                  ...
                  <span class="text-[14px] font-semibold text-[var(--app-sports-slider-text-win)] opacity-70">
                   可赢{profit.value}
                  </span>
                </div>
            )
            ...
            <div class="flex items-center">
              <IconSptBetArrrow1 style={{ fontSize: '12px', color: 'rgba(var(--app-sports-slider-iocn2),0.2)' }} />
              <IconSptBetArrrow1 style={{ fontSize: '12px', color: 'rgba(var(--app-sports-slider-iocn2),0.6)' }} />
              <IconSptBetArrrow1 style={{ fontSize: '12px', color: 'rgba(var(--app-sports-slider-iocn2),1)' }} />
            </div>
          </div>
          </div>
        ) }
