...
<template>
  <div class="app-safe" :class="{ page: !showBottom }">
    <div class="safe-content rate-value-content">
      <BaseTab v-model="activeTab" :list="tabOptions" @change="handleTabChange(activeTab)" />
      ...
      <template v-else>
        ...
        <BaseButton
          v-if="isDeposit" bg-style="secondary" size="xl" :loading="lockerUpdateLoading"
          :class="[isMobile && !isLogin && 'mobile-nologinn-btn']"
          style="font-size: var(--tg-font-size-base);" @click="handleUpdate"
        >
          {{ t('save_to_vault') }}
        </BaseButton>
        <template v-else>
          <AppPasswordInput v-if="isLogin" ref="passwordRef" v-model="password" class="password-input" />
          <BaseButton
            style="font-size: var(--tg-font-size-base);" bg-style="secondary" size="xl"
            :class="[isMobile && !isLogin && 'mobile-nologinn-btn']"
