...
const drawResult = computed(() => {
  if (isEgg.value && isOpenByPlayer.value)
    showResult(fireEggRef, fireEggAni, false, 0.5, props.egg)
  else if (isSkull.value && isOpenByPlayer.value)
    showResult(skullRef, skullAni, false, 0.5, props.fire, props.skull)
  else
    return ''
})
</script>
    ...
    <div>
      <div class="result-wrapper">
        <div v-if="data.result === 'egg' && !isOpenByPlayer" class="symbol-wrap">
          <div class="symbol">
            <img :src="`/stake/img/game/egg-${difficult}.svg`" alt="medium-win">
          </div>
        </div>
        <div
          v-else class="canvas"
...
.tile-bg.easy svg,.tile-bg.master svg {
  width: 280%;
}
