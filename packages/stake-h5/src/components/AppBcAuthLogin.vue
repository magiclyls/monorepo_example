...
import { IconUniGreyFb, IconUniGoogleGrey, IconUniLineGrey, IconUniGreyTwitch } from '@tg/icons'
...
<template>
  <div v-if="thirdAuth.length" class="app-bottom">
    <div class="app-bottom-divider">
      <BaseDivider title-placement="center" size="1" style="--tg-text-lightgrey: #B3BEC1;--tg-base-divider-bg-color: rgba(228, 234, 240, 0.10);">
        <span class="text-[12px]">直接登入</span>
      </BaseDivider>
    </div>
    <div
      class="app-bottom-icon mt-[20px]"
      :style="{ 'grid-template-columns': thirdAuth.length === 4 ? 'repeat(4,1fr)' : `repeat(${thirdAuth.length},25%)` }"
    >
      <BaseButton v-if="thirdAuth.includes('fb')" :disabled="getAuthUrlLoading" type="line" class="item-svg third-btn" size="xs" @click="goAuth('fb')">
        <IconUniGreyFb class="text-[16px]" />
      </BaseButton>
      <BaseButton v-if="thirdAuth.includes('google')" :disabled="getAuthUrlLoading" type="line" class="item-svg third-btn" size="xs" @click="goAuth('google')">
        <IconUniGoogleGrey class="text-[16px]" />
      </BaseButton>
      <BaseButton v-if="thirdAuth.includes('line')" :disabled="getAuthUrlLoading" type="line" class="item-svg third-btn" size="xs" @click="goAuth('line')">
        <IconUniLineGrey class="text-[16px]" />
      </BaseButton>
      <BaseButton v-if="thirdAuth.includes('twitch')" :disabled="getAuthUrlLoading" type="line" class="item-svg third-btn" size="xs" @click="goAuth('twitch')">
        <IconUniGreyTwitch class="text-[16px]" />
      </BaseButton>
    </div>
  </div>
</template>
...
.app-bottom {
  ...
  &-icon {
    ...
    --tg-icon-color: rgb(179 190 193);
    --tg-base-button-line-border-color: #e4eaf019;
...
.third-btn {
  @include webTheme('green') {
    border: 1px solid var(--tg-border-color-grey);
