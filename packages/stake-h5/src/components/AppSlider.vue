...
watchEffect(() => {
  if (outerWidth.value >= 1150) {
    ...
    if(props.displayNum === 4) {
      itemWidthPercent.value = 0.25
    galleryClass.value = 'gallery-700-500'
    pageInfo.pageSize = 4
    } else if(props.displayNum === 2) {
      itemWidthPercent.value = 0.5
    galleryClass.value = 'gallery-300'
    pageInfo.pageSize = 2
    } else {
    ...
    <div class="header">
      <div class="title">
        <BaseButton type="text" size="none" style="--tg-base-button-text-default-color:var(--tg-text-white);"
          ...
          <div v-if="icon" class="icon">
            <AppImage v-if="useCloudImg" class="icon-img" :url="icon" is-cloud with-theme loading="eager" />
      ...
      <div class="right-wrap flex">
        <BaseButton v-if="total" type="square-line" class="theme-button h-[28px]"
          ...
          @click="onTitleClick">
          <span class="all-text">{{ t('finance_other_tab_all') }}</span>
