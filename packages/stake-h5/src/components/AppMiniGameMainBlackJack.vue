...
import type { BlackJackData, NpcPokerState, PlayerCardActions, PokerCardProps } from '~/types'

import { ApiGameOriginalBlackJack } from '@tg/apis'
import { GAMES_LIST_ENUM } from 'feie-ui'
import gsap from 'gsap'
import { sortedUniq } from 'lodash'
import { BlackJackActionType, PokerColors } from '~/utils/enums'
import { IconCallcard, IconStopcall, IconSplitcard, IconDoubleit, IconUniArrowRight, IconUniArrowLeft } from '@tg/icons'
...
const { currentGlobalCurrency, isLogin, balanceData, userInfo } = storeToRefs(appStore)
...
const money = computed(() => balanceData.value ? balanceData.value[getCurrencyConfigByCode(currentCurrencyCode.value).name] : 0)
// 分牌 加倍 余额判断
...
const { openBlackJackInfo } = useDialogBlackJackInfo()
const { openBlackJackShortCut } = useDialogMiniGameBlackJackHotkeys()
  ...
  runGetCardData({ order_id: originData.value?.id, type: BlackJackActionType.SPLIT, currency_id: currentCurrencyCode.value, bet_amount: amount.value }).then((res) => {
    if (res) {
      ...
      for (let i = 0; i < playerCards.value.length; i++)
        playerValue.value = calcHandValue(playerCards.value[i], playerValue.value)
      for (let j = 0; j < playerCardsSplit.value.length; j++)
        playerSplitValue.value = calcHandValue(playerCardsSplit.value[j], playerSplitValue.value)
...
<template>
  <AppMiniGamePublicLayout
    ...
    @open-hot-keys="openHotKeys"
  >
    <template #left>
      <div class="game-blackjack-sidebar flex flex-col">
        <BaseLabel class="theme-label mb-[12px]" :label="$t('hilo_bet_amount')" :right-text="currencyPrefix + betAmountMulRate">
        ...
        <div class="mid-buttons flex-col-12 mb-0 flex flex-col @md:mb-[12px]">
          <div v-if="showInsurance" class="actions insurance grid grid-cols-2 grid-rows-2 gap-x-[8px] gap-y-[16px] text-sm">
            <div class="desc text-tg-text-white col-span-full flex items-center justify-center font-semibold">
              {{ $t('buy_insurance') }}
            </div>
            <BaseButton class="theme-btn" size="md" :disabled="acceptInsuranceButton" @click="clickAcceptInsurance">
              <div class="call-card relative flex items-center">
                <span>{{ $t('accept_insurance') }}</span>
              </div>
            </BaseButton>
            <BaseButton class="theme-btn" size="md" :disabled="acceptInsuranceButton" @click="clickNoAcceptInsurance">
              <div class="stop-call relative flex items-center">
                <span>{{ $t('reject_insurance') }}</span>
              </div>
            </BaseButton>
          </div>
          <div v-else class="actions btns grid grid-cols-2 grid-rows-2 gap-x-[8px] gap-y-[16px] text-sm">
            <BaseButton size="md" :disabled="callCardButton" @click="clickCallCard">
              <div class="call-card relative flex items-center">
                <span class="mr-[8px]">{{ $t('call_card') }}</span>
                <IconCallcard />
              </div>
            </BaseButton>
            <BaseButton size="md" :disabled="acceptInsuranceButton" @click="clickStopCallCard">
              <div class="stop-call relative flex items-center">
                <span class="mr-[8px]">{{ $t('stop_call') }}</span>
                <IconStopcall />
              </div>
            </BaseButton>
            <BaseButton :disabled="splitButton" size="md" @click="clickSplitCard">
              <div class="split-card relative flex items-center">
                <span class="mr-[8px]">{{ $t('split_card') }}</span>
                <IconSplitcard />
              </div>
            </BaseButton>
            <BaseButton size="md" :disabled="multipleButton" @click="clickMultiple">
              <div class="double-it relative flex items-center">
                <span class="mr-[8px]">{{ $t('double_it') }}</span>
                <IconDoubleit />
              </div>
            </BaseButton>
          </div>
        </div>
        <div class="bet-button order-first mb-[12px] w-full @md:order-last @md:mb-0">
    ...
    <template #right>
      <div ref="gameBlackJackRef" :style="{ fontSize: `${fontSizeGameRoot}em`, minHeight: `${minHeightGameRoot}px` }">
        <div class="game-blackjack wrap aspect-[10/7] w-full flex grow px-[1em] py-[3em]">
          ...
          <div
            class="hands relative grid h-fit min-h-[250px] w-full content-between bg-[length:40%] bg-center bg-no-repeat @md:h-auto @md:gap-[56px]"
          >
            ...
            <div class="player flex flex-row-reverse justify-around">
              ...
              >
                ...
                >
                  ...
                  <IconUniArrowRight class="active-right absolute right-full text-[2em]" />
                  <IconUniArrowLeft class="active-left absolute left-full text-[2em]" />
                </div>
              </div>
              <div
...
.mid-buttons button:disabled:active {
  //
}
...
.game-blackjack {
  ...
  .hands {
    background-image: url(/img/game/blackjackbg.svg);
    grid-template-rows: repeat(2, max-content);
    @include webTheme('greenblack') {
      background-image: url(/img/game/blackjackbg_greentheme.svg);
    }
    @include webTheme('green') {
      background-image: url(/img/game/blackjackbg_greentheme.svg);
    }
    @include webTheme('white') {
      background-image: url(/img/game/blackjackbg_white.svg);
    }
  }
  .hand-wrap {
    .active-left,
