...
import type { EnumCurrencyKey, IAvailableCurrency, virAddreesQrcode, IPromoDetail } from '@tg/types'
...
const virAddreesQrcodeData = ref(customRouter.query.initvirAddreesQrcodeData as unknown as virAddreesQrcode)
/** 当前的支付ID */
const curPaymentId = ref(customRouter.query.paymentId as unknown as string)
/** 当前的支付通道 */
const activeMerchant = ref(customRouter.query.curMerchant as unknown as Merchants)
const contractList = ref(customRouter.query.initContractList as unknown as ContractOption[])
/** 当前的加密货币网络 */
const currentNetwork = ref((customRouter.query.initCurrentNetwork as unknown as string) ?? '')
const activeVirtCurrency = ref(customRouter.query.initVirCurrency as unknown as ICurrencyOption)
const curDepositPromo = ref(customRouter.query.curDepositPromo as unknown as [])
const virCurrencyList = ref(customRouter.query.virCurrencyList as unknown as ICurrencyOption[])
...
const { run: runPaymentDepositCoinApplication, loading: paymentDepositCoinInfoLoading } = useRequest(ApiPaymentDepositCoinApplication, {
  onSuccess(data) {
    ...
    customRouter.push('/deposit/detail/submit', {
      ...
      activeVirtCurrency: activeVirtCurrency.value,
    ...
    <template v-else>
      ...
      <div class="type-other">
        <div class="other-first">
          ...
          <!-- 充值金额面板 -->
          <BaseMoneyKeyboard v-if="oftenAmount && oftenAmount.length" v-model="amount"
            :currency="activeVirtCurrency.currency_id" class="keyboard" :options="oftenAmount" />
          <!-- 确认支付按钮 -->
