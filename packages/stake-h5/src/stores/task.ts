...
import { ApiJobTaskList ,ApiJobTaskCategory } from '@tg/apis'
...
export const useTaskStore = defineStore('task', () => {
  ...
  const { loading: taskLoading, runAsync: getTaskCategoryApi } = useRequest(ApiJobTaskCategory, {
    ...
    onSuccess: (res) => {
      ...
      res?.categories && res.categories.length > 0 && getTaskListAsyncApi({ lang: getCurrentLanguageForBackend(), category_id: currentCategory.value })
    },
  })
