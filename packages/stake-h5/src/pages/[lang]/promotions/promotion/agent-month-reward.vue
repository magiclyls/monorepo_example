  ...
  onSuccess: (res, params) => {
    ...
    if (res && +res.activity_state === 21 && !preview) {
      ...
      openNotify({
        ...
        iconColor: '--tg-text-alltheme-green',
        title: t('warm_tip'),
        message: t('participated_no_again'),
      })
      goPromo()
      return
    }
...
const tableColumn: ComputedRef<Column[]> = computed((): Column[] => [
  {
    title: t('last_month_sub_deposit'),
    align: 'center',
    width: '50%',
    slot: 'deposit',
    dataIndex: 'deposit',
...
onActivated(() => {
  ...
  getDetail(activeCurrencyCode.value)
})
...
<template>
  <div class="text-tg-text-lightgrey promo-agent-month-reward m-auto mt-[16px] max-w-[650px]">
    <div class="mb-[16px] flex items-center justify-center" :class="{ ' mb-[16px]': imgUrl }">
      <BaseImage v-if="imgUrl" class="" :url="imgUrl" is-network />
    </div>
    <div class="bg-tg-secondary-grey theme-bg mb-[16px] flex flex-col rounded p-[12px] text-sm font-semibold">
      <div class="mb-[8px] flex justify-between">
        <span>{{ t('last_month_sub_deposit') }}:</span>
        <span><AppAmount class="theme-amount" :amount="commissionAmount" :currency-code="promoCurrencyCode" /></span>
      </div>
      <div class="mb-[8px] flex justify-between">
        <span>{{ t('month_reward') }}:</span>
        <span><AppAmount class="theme-amount-sec" :amount="bonusAmount" :currency-code="promoCurrencyCode" /></span>
      </div>
        ...
        <template v-if="isLogin">
          <template v-if="receiveState && +receiveState === 4">
            <div class="flex flex-col items-center justify-center">
              ...
              <span class="my-[6px] flex"><AppAmount class="theme-amount" :amount="bonusAmount" :currency-code="promoCurrencyCode" /></span>
          ...
          <template v-else-if="receiveState && +receiveState === 1">
            ...
            <template v-else>
              <div class="flex flex-col items-center justify-center">
                <div>{{ t('have_qualified') }}</div>
                <span class="my-[6px] flex"><AppAmount class="theme-amount" :amount="bonusAmount" :currency-code="promoCurrencyCode" /></span>
                <AppImage width="43px" url="/png/promotions/promo_money01.png" />
              </div>
              <BaseButton :loading="bonusLoading" class="mt-[10px] w-full" bg-style="secondary" size="md" @click="getBonus">
                {{ t('vip_right_now') }}
              </BaseButton>
            </template>
          </template>
          ...
          <BaseButton v-if="receiveState && +receiveState === 4" style="--tg-base-button-default-fake-active-bg: var(--tg-text-grey-third);--tg-base-button-style-bg-hover: var(--tg-text-grey-third);" class="mt-[10px] w-full disabled-btn" bg-style="grey" size="md">
            {{ t('received') }}
          </BaseButton>
          <BaseButton v-if="!receiveState || (receiveState && +receiveState !== 1 && +receiveState !== 4) || (data && +data.activity_state === 25)" class="mt-[10px] w-full" bg-style="secondary" size="md" @click="goPromo">
            {{ t('view_more_event') }}
          </BaseButton>
        </template>
        <template v-else>
          ...
          <BaseButton class="mt-[10px] w-full" bg-style="secondary" size="md" @click="openLogin">
            {{ t('login_now') }}
          </BaseButton>
        </template>
      </div>
