    ...
    >
      <div v-if="step == 1" class="flex flex-col justify-between px-[16px] pb-[32px] pt-[18px]">
        ...
        <div class="h-[2.75rem] flex gap-[1.875rem]">
          ...
          <BaseButton
            bg-style="secondary" size="md" class="w-full" :loading="loadingTransfer" @click="transferToWallet"
          >
      ...
      <div v-if="step === 2" class="px-[16px] pb-[32px] pt-[24px]">
        ...
        <div class="h-[44px] flex gap-[30px]">
          ...
          <BaseButton
            bg-style="secondary" size="md" class="w-full" @click="show = false; openWalletDialog()"
          >
            {{ t('withdraw_now') }}
          </BaseButton>
        </div>
      ...
      <div v-if="step === 3" class="px-[16px] pb-[32px] pt-[16px]">
        ...
        <BaseButton
          bg-style="secondary" size="md" class="w-full" @click="showVerify"
        >
      ...
      <div v-if="step === 4" class="flex flex-col justify-between gap-[16px] px-[16px] pb-[32px] pt-[12px]">
        ...
        <BaseButton bg-style="secondary" :loading="loadingVerify" :disabled="!captcha" @click="submit(useCaptcha === 'email' ? 1 : 2)">
          <span>{{ t('submit') }}</span>
        </BaseButton>
      ...
      <div v-if="step === 5" class="flex flex-col justify-between p-[16px] pb-[32px]">
        ...
        <BaseTextarea v-model="remark" :placeholder="t('enter_your_content')" class="h-[80px]" rows="6" />
        <div class="mt-[4px] text-12 text-[#fc3c3c]">
          {{ remarkError }}
        </div>
        <BaseButton bg-style="secondary" class="mt-[16px]" :loading="loadingBindCpf" :disabled="!!remarkError || !reasonIds.length" @click="runCloseAccount()">
          <span>{{ t('submit') }}</span>
        </BaseButton>
      </div>
    </BaseDialog>
