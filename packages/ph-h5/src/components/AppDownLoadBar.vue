...
<template>
  <!-- 自定义样式 -->
  <div
    v-if="pwaGlobalConfig.barType === 1" class="h-[46rem] overflow-hidden"
    :style="{
      backgroundImage: pwaGlobalConfig.bgColorType === 'gradient' ? pwaGlobalConfig.config.bgGradientColor : `linear-gradient(90deg,${pwaGlobalConfig.config.bgColor},${pwaGlobalConfig.config.bgColor})`,
  ...
  <!-- 图片 -->
  <div v-else-if="pwaGlobalConfig.barType === 2" class="w-[var(--pc-max-width)] bg-[#0f212e] h-[46rem] overflow-hidden pl-[10rem]">
    <PhBaseImage class="absolute left-0 top-0 h-[46rem] w-full" fit="cover" :url="pwaGlobalConfig.imgUrl" is-network />
        ...
        >
          <div class="relative w-full">
            <BaseAspectRatio :ratio="`1/${130 / 375}`">
              <PhBaseImage class="w-full" url="/png/home/pwa-load-banner.png" />
            </baseaspectratio>
          ...
          <div class="px-[23rem]">
            ...
            <div class="mb-16 flex items-center rounded-[4.8rem] bg-[#F5F5F5] py-5">
              <PhBaseImage class="mx-10 h-43 w-43 rounded-[10rem]" :url="iconUrl" is-network />
