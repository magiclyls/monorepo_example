...
<template>
    ...
    >
      <div class="swiper-wrapper">
        ...
        <div v-for="(item, idx) in sliders" :key="item.id + idx" class="swiper-slide relative cursor-pointer">
          <PhBaseImage is-network :url="item.icon[0] === '/' ? item.icon : `/${item.icon}`" @click.stop="openThrottleActivity(item)" />
        </div>
      </div>
    </div>
    <!-- 关闭按钮 -->
  ...
  <!-- 预加载图片（隐藏） -->
  <PhBaseImage v-for="(item, idx) in sliders" :key="idx" width="0px" height="0px" class="invisible h-0 w-0" is-network hidden :url="item.icon[0] === '/' ? item.icon : `/${item.icon}`" />
</template>
