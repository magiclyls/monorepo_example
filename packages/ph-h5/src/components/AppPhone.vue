...
import { PhBaseButton, PhBaseImage, PhBaseInput, PhBaseLabel, PhBaseOriginSelect, PhBaseSelect } from '@tg/bccomponents'
...
<template>
  <div class="bg-[#fff] rounded-[8rem] p-[12rem]" :class="isComponent && 'p-[0]!'">
    ...
    <PhBaseLabel v-if="isComponent" :label="t('手机号码')" required class="mb-[16rem]">
      <PhBaseInput
        ref="phoneRef"
        v-model="phone"
        name="phone"
      ...
      >
        <template #left>
          <div class="center h-full flex text-[#9DABC9]">
            <PhBaseImage v-if="area_code && area_code.length" class="w-[16rem]" :url="`/flag/${area_code.slice(1)}.webp`" />
            <PhBaseOriginSelect
    ...
    <template v-else>
      <PhBaseLabel :label="t('国际电话区号')" required class="mb-[16rem]">
        <PhBaseSelect
          v-model="area_code" :options="safeAreaCodeOptionsFiltered" style="
    ...
    <div v-if="isOpenMobileVerify && !phoneVerified" class="mb-[5rem]">
      ...
      <div class="flex gap-[20rem]">
        ...
        <PhBaseButton v-if="!isComponent" type="primary" :loading="phoneCaptchaLoading" :disabled="isCountdown || !!phoneCodeErrorMsg || !phone" class="h-[40rem] w-[110rem]" style="--ph-base-button-font-size: 14rem; --ph-base-button-font-weight: 500; --ph-base-button-border-color: #EBEBEB;--ph-base-button-secondary-background-color:#fff; --ph-base-button-padding-y: 9rem" @click="onResendClick">
          <span v-if="isCountdown">{{ `${t('重新获取')} ${current.seconds}s` }}</span>
          <span v-else>{{ t('获取验证码') }}</span>
        </PhBaseButton>
      </div>
    </div>
    <div v-if="isComponent && isOpenEmailVerify" class="mt-[11rem] mb-[16rem]">
    ...
    <template v-if="!phoneVerified">
      <PhBaseButton v-if="!isComponent" type="primary" :loading="updatePhoneLoading" :disabled="!hasChangePhone" class="h-[44rem] w-full mt-[16rem]" style="--ph-base-button-font-size: 14rem; --ph-base-button-font-weight: 500; --ph-base-button-border-color: #EBEBEB;--ph-base-button-secondary-background-color:#fff; --ph-base-button-padding-y: 9rem" @click="updatePhone">
        <span>{{ phoneEditBtnText }}</span>
      </PhBaseButton>
      <div v-else class="flex justify-end items-center my-[8rem]">
        <div type="none" class="h-[46rem] mr-[12rem] leading-[46rem]" :class="[current.seconds > 0 && 'text-[#6D7693]']" @click="onResendClick">
          <span>{{ `${current.seconds > 0 ? `${current.seconds}s ` : ''}${t('重新发送')}` }}</span>
        </div>
        <PhBaseButton type="primary" :loading="updatePhoneLoading" :disabled="!!hasChangePhone" class="h-[44rem] px-[26rem]" style="--ph-base-button-font-size: 14rem; --ph-base-button-font-weight: 500; --ph-base-button-border-color: #EBEBEB;--ph-base-button-secondary-background-color:#fff;--ph-base-button-padding-x: 26rem" @click="updatePhone">
          <span>{{ t('保存') }}</span>
        </PhBaseButton>
      </div>
    </template>
