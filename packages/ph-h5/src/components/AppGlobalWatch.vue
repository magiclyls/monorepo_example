...
import type { EnumCurrencyKey, SendFlutterThirdPartyAppMessage } from '@tg/types'
import { sendMsgToAdjust, useAdjust } from '@tg/hooks'
...
import {
  ...
  balanceMsgCallback,
  getEnv,
  kycFacialRecognitionMsgCallBack,
  kycVerifyMsgCallBack,
  maintainCallBack,
  memberCallback,
...
function handleAjust(param: { type: SendFlutterThirdPartyAppMessage, revenue: number, currency: EnumCurrencyKey }) {
  const { type, revenue, currency } = param
  sendMsgToAdjust(type, { revenue, currency })
}
