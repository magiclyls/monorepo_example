...
const msgHtml = computed(() => {
  ...
  for (let i = 0; i < splitByEmojiMsg.value.length; i++) {
    ...
    if (
      ...
      if (idx !== -1)
        temp.push({ type: 'PhBaseImage', content: { alt: matchedEmojis.value[i], url: `/ph-h5/webp/emoji/${allEmojis[idx]}`, class: 'emoji' } as any })
      else
...
<template>
  <span>
    <template v-for="item in msgHtml" :key="item">
      ...
      <template v-else-if="item.type === 'PhBaseImage'">
        <PhBaseImage v-bind="item.content" style="--tg-base-img-max-height: 20rem" @load-img="goBottom2" />
      </template>
