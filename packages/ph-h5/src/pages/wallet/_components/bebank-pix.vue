      ...
      <PhBaseLabel required :label="$t('请选择PIX类型')">
        ...
        >
          <template #item-icon="{ item }">
            <PhBaseImage class="w-full h-full" :url="item.icon" is-network />
          </template>
        </AppSelect>
      </PhBaseLabel>
      <PhBaseLabel v-show="!isCPF" required :label="$t('PIX账号')">
        <PhBaseInput v-model="bank_account" style="--ph-base-input-padding-y:10rem;" :placeholder="t('PIX账号')" :msg="bank_accountError" />
      </PhBaseLabel>
      <PhBaseLabel required label="CPF">
        <PhBaseInput v-model="bank_area_cpf" style="--ph-base-input-padding-y:10rem;" :disabled="!!cpf" placeholder="CPF" :msg="cpfError" />
      </PhBaseLabel>
