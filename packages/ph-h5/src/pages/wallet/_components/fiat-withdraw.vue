...
import { PhBaseAmount, PhBaseButton, PhBaseCurrencyIcon, PhBaseFinanceEmpty, PhBaseImage, PhBaseInput, PhBaseLabel, PhBaseSelectCurrency, PhBaseTabs } from '@tg/bccomponents'
  ...
  <div class="flex flex-col gap-[16rem]">
    ...
    <PhBaseLabel :label="t('提款货币')" required layout="horizontal">
      <template v-if="fiatCurrencyList.length > 0">
        ...
        >
          <PhBaseCurrencyIcon icon-align="left" :show-name="true" style="--ph-app-currency-icon-size:18rem;" :currency-type="activeFiatCurrency.currency_name" />
        </div>
        <PhBaseSelectCurrency v-else v-slot="slotProps" :t="t" :options="fiatCurrencyList" :currency="activeFiatCurrency?.cur" @choose="onFiatCurrencyChange">
          <div
      ...
      <template v-if="fiatCurrencyList && fiatCurrencyList.length">
        ...
        <template v-if="numberOfAccountListOnCurrentWithdrawType">
          ...
          <template v-if="isBankCardType || isThreePartyWalletType">
            <PhBaseLabel required :label="selectTitle">
              <AppSelect
                v-model="bankNameId"
                :msg="bankNameIdErr"
                :options="walletOrBankList"
                :place-holder="selectTitle"
                full
