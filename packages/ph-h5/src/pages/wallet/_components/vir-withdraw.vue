...
import { PhBaseButton, PhBaseCurrencyIcon, PhBaseFinanceEmpty, PhBaseInput, PhBaseLabel, PhBaseSelectCurrency } from '@tg/bccomponents'
  ...
  <div class="flex flex-col gap-[16rem]">
    ...
    <PhBaseLabel :label="t('提款货币')" required layout="horizontal">
      <template v-if="virCurrencyList.length > 0">
        ...
        >
          <PhBaseCurrencyIcon icon-align="left" :show-name="true" style="--ph-app-currency-icon-size:18rem;" :currency-type="activeVirCurrency.currency_name" />
        </div>
        <PhBaseSelectCurrency v-else v-slot="slotProps" :t="t" :options="virCurrencyList" :currency="activeVirCurrency?.cur" @choose="onVirCurrencyChange">
          <div
