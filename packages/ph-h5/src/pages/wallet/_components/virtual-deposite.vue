  ...
  <div class="my-[16rem] flex flex-col gap-[16rem] p-[12rem] rounded-[8rem]  bg-white">
    ...
    <PhBaseLabel :label="t('存款货币')" required layout="horizontal">
      <template v-if="activeVirCurrency">
        ...
        <PhBaseSelectCurrency v-else v-slot="slotProps" :t="t" :options="virCurrencyListOptions" :currency="activeVirCurrency?.currency_id" @choose="onVirtCurrencyChange">
          <div class="flex items-center justify-between h-[40rem] px-[8rem] border-solid border rounded-[4rem]" :class="[slotProps.isMenuShown ? 'border-[#F23038]' : 'border-[#EBEBEB]']">
            <PhBaseCurrencyIcon icon-align="left" :show-name="true" style="--ph-app-currency-icon-size:18rem;" :currency-type="activeVirCurrency.currency_name" />
            <IconUniArrowDown1 v-if="virCurrencyListOptions.length > 1" class="ml-[4rem] text-[#9dabc9]" />
          </div>
