...
import { PhBaseButton, PhBaseCheckbox, PhBaseImage, PhBaseInput, PhBaseLabel } from '@tg/bccomponents'
    ...
    <template v-if="isSetPayPwd">
      <div class="flex flex-col gap-[16rem] bg-[#fff] p-[12rem] rounded-[8rem]">
        <PhBaseLabel required :label="$t('真实姓名')">
          <PhBaseInput v-model="open_name" style="--ph-base-input-padding-y:10rem;" :placeholder="t('真实姓名')" :msg="open_nameError" :disabled="!!openName" />
        </PhBaseLabel>
        ...
        <PhBaseLabel v-if="_withdrawTypeList.length > 1" required :label="$t('提款方式')">
          ...
          >
            <template #item-icon="{ item }">
              <PhBaseImage v-if="item?.icon" class="w-full h-full" :url="item?.icon" is-network />
            </template>
          </AppSelect>
        </PhBaseLabel>
        <template v-if="isBankCardType && bank_nameOptions.length > 0">
          ...
          <PhBaseLabel v-if="isNeedAddress" required :label="$t('开户网点')">
            <PhBaseInput v-model="address" style="--ph-base-input-padding-y:10rem;" :msg="addressError" :placeholder="t('开户网点')" />
          </PhBaseLabel>
          <PhBaseLabel required :label="$t('银行卡号')">
            <PhBaseInput
              v-model="bank_account"
              style="--ph-base-input-padding-y:10rem;" :placeholder="t('银行卡号')" type="number"
        ...
        <template v-else-if="isAlipayType">
          <PhBaseLabel required :label="$t('支付宝账号')">
            <PhBaseInput v-model="bank_account" style="--ph-base-input-padding-y:10rem;" :placeholder="t('支付宝账号')" :msg="bank_accountError" />
          </PhBaseLabel>
        </template>
        <template v-else-if="isThreePartyWalletType">
          ...
          <PhBaseLabel required :label="$t('钱包账号')">
            <PhBaseInput v-model="bank_account" style="--ph-base-input-padding-y:10rem;" :placeholder="t('钱包账号')" :msg="bank_accountError" />
          </PhBaseLabel>
        </template>
        <template v-if="isIndiaCard">
          <PhBaseLabel required :label="$t('电话号码')">
            <PhBaseInput v-model="phone" style="--ph-base-input-padding-y:10rem;" :placeholder="t('电话号码')" :msg="phoneErrorMsg" />
          </PhBaseLabel>
          <PhBaseLabel required :label="$t('电子邮箱')">
            <PhBaseInput v-model="email" style="--ph-base-input-padding-y:10rem;" :placeholder="t('电子邮箱')" :msg="emailErrormsg" />
          </PhBaseLabel>
          <PhBaseLabel v-if="withdrawType === 1" required label="IFSC">
            <PhBaseInput v-model="ifsc" style="--ph-base-input-padding-y:10rem;" placeholder="IFSC" :msg="ifscErrorMsg" />
          </PhBaseLabel>
          <PhBaseLabel v-if="withdrawType === 2" required :label="$t('提款账号')">
            <PhBaseInput
              v-model="bank_account"
              style="--ph-base-input-padding-y:10rem;" :placeholder="t('提款账号')"
              :msg="bank_accountError" hide-spin-btn
            />
          </PhBaseLabel>
        </template>
        <template v-if="isPhp && withdrawType === 2">
          <PhBaseLabel required :label="$t('提款账号')">
            <PhBaseInput
              v-model="bank_account"
              style="--ph-base-input-padding-y:10rem;" :placeholder="t('提款账号')"
              :msg="bank_accountError" hide-spin-btn
