...
import { PhBaseButton, PhBaseCurrencyIcon, PhBaseInput, PhBaseLabel, PhBaseSelectCurrency } from '@tg/bccomponents'
  ...
  if (activeMerchant.value.promo_cfgs && activeMerchant.value.promo_cfgs[0]) {
    ...
    if (amount >= Number(firstStage.deposit)) {
      ...
      if (bonus_type === 1) {
        ...
        resultStr = `${toFixedByLockCurrency((Number(currentStage.min) + total1).toString(), activeVirCurrency.value.currency_name)}~${toFixedByLockCurrency((Number(currentStage.max) + total1).toString(), activeFiatCurrency.value.currency_name)}`
      }
      ...
      <div class="flex flex-col gap-[16rem] p-[12rem] rounded-[8rem]  bg-white">
        ...
        <PhBaseLabel :label="t('存款货币')" required layout="horizontal">
          <template v-if="activeVirCurrency">
            ...
            >
              <PhBaseCurrencyIcon
                icon-align="left" :show-name="true" style="--ph-app-currency-icon-size:18rem;"
                :currency-type="activeVirCurrency.currency_name"
              />
            </div>
            <PhBaseSelectCurrency
            ...
            >
              ...
              >
                <PhBaseCurrencyIcon
                  ...
                  :currency-type="activeVirCurrency.currency_name"
                />
                <IconUniArrowDown1 v-if="virCurrencyList.length > 1" class="ml-[4rem] text-[#9dabc9]" />
              </div>
            </PhBaseSelectCurrency>
          </template>
        </PhBaseLabel>
        ...
        <PhBaseLabel v-if="contractList && contractList.length > 0" :label="t('选择网络')" required layout="horizontal">
          <AppSelect v-model="currentNetwork" :options="contractList" placement="bottom-end" />
        </PhBaseLabel>
        <div class="flex flex-col gap-16 pt-16">
          ...
          <div v-else class="flex flex-col gap-[16rem]">
            ...
            <PhBaseLabel :label="t('存款金额')" required>
              ...
              >
                <template v-if="activeDepositPromo" #right>
              ...
              <div
                ...
                class="w-full flex items-center justify-between rounded-[6rem] border-solid border border-[#EBEBEB]"
              >
                ...
                <div
                  ...
                  class="bg-[#EBEBEB] px-[12rem]  h-[40rem] center whitespace-nowrap rounded-r-[6rem]  font-[500]"
                >
                  {{ t('赠送') }} +{{ activeDepositPromo }}
                </div>
              </div>
