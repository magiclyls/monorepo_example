...
import { PhBaseAmount, PhBaseButton, PhBaseCurrencyIcon, PhBaseInput, PhBaseLabel, PhBaseSelectCurrency } from '@tg/bccomponents'
...
const fixedAmount = computed(() => {
  if (activeMerchant.value.amount_type === 1)
    return strToArray(activeMerchant.value?.amount_fixed ?? '', curMerchant?.bonus_map)
  ...
  if (activeMerchant.value.promo_cfgs && activeMerchant.value.promo_cfgs[0]) {
    ...
    if (amount >= Number(firstStage.deposit)) {
      ...
      if (bonus_type === 1) {
        ...
        /* resultStr = `${Number(currentStage.scale) + Number(curDepositPromo[0].ratio * 100)}%` */
...
watch(activeMerchantId, () => {
  ...
  uPayPwdReset()
  activeDepositPromo.value = ''
  oftenAmount.value = strToArray(activeMerchant.value?.often_amount ?? '', curMerchant?.bonus_map)
})
  ...
  <AppPageLayout :title="$t('存款')">
    <div class="flex flex-col gap-[16rem] p-[12rem] rounded-[8rem]  bg-white">
          ...
          >
            ...
            >
              <PhBaseCurrencyIcon
                ...
                :currency-type="activeFiatCurrency.currency_name"
              />
              <IconUniArrowDown1 v-if="fiatCurrencyList.length > 1" class="ml-[4rem] text-[#9dabc9]" />
            </div>
          </PhBaseSelectCurrency>
        </template>
      </PhBaseLabel>
      ...
      <PhBaseLabel v-if="merchantsArray?.length > 0" :label="currentPaymentLable" required layout="horizontal">
        <AppSelect v-model="activeMerchantId" :options="merchantsArray" placement="bottom-end">
      ...
      <PhBaseLabel v-if="haveBank" :label="t('选择银行')" required layout="horizontal">
        <AppSelect v-model="curPaymentBank" :options="paymentDepositBankData" placement="bottom-end">
          <template #item-icon="{ item }">
            <MerchantIcon size="100%" :currency-type="currencyType" :type="curPaymentType" :item="item" />
          </template>
        </AppSelect>
      ...
      <div v-if="isEmbed" class=" flex items-center justify-between">
        <div class="flex items-center text-[14rem] font-[500]">
          ...
          <PhBaseAmount
            v-if="isLogin" class="text-[#0D2245]" currency-type="CNY"
            :amount="uPayBalance?.Balance ?? '0.00'" reverse
          />
          <span v-else class="mr-4">{{ t('获取失败') }}</span>
          <div
            class=" ml-[4rem] cursor-pointer  center text-[16rem]"
    ...
    <template v-else> -->
      ...
      <div class="flex flex-col gap-[16rem]">
        ...
        <PhBaseLabel v-if="isPaymentDepositBank" :label="t('存款人姓名')" required>
          <PhBaseInput v-model="depositName" :msg="depositNameError" :placeholder="t('请输入存款人姓名')" />
        </PhBaseLabel>
        <!-- 充值金额 -->
        ...
        />
        <!-- 确认支付按钮 -->
        <PhBaseButton v-if="isEmbedPending" loading class=" center">
          <span>{{ t('充值中...') }}</span>
        </PhBaseButton>
        <PhBaseButton v-else :loading="thirdDepositLoading || paymentDepositBankInfoLoading" @click="depositSubmit">
          {{ t('确认支付') }}
        </PhBaseButton>
      </div>
