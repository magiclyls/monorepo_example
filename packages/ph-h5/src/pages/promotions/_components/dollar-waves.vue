...
import { PhBaseAmount, PhBaseImage, PhBaseRichArea } from '@tg/bccomponents'
...
<template>
  <div v-if="detailData && currentConfig">
    <div
      ...
      @click="pageClicked"
    >
      <PhBaseImage v-if="bannerImg" class="set-radios" :url="bannerImg" is-network />
        ...
        <section v-if="detailData.drop === 2" class="grid grid-cols-3 gap-y-[8rem] gap-x-[38rem]">
          <template v-for="(i, idx) in timeScopes" :key="`${i}-${idx}`">
            <div
              ...
              @click="() => littleBusterd(i)"
            >
              <PhBaseImage :url="`/ph-h5/png/${getBgImage(i)}.png`" />
        ...
        <section v-if="detailData.drop === 3" class="grid grid-cols-3 gap-[8rem]">
          <template v-for="(i, idx) in timeScopes" :key="`${i}-${idx}`">
            ...
            >
              ...
              <PhBaseImage class="mx-auto my-[4rem] w-[70rem] h-[70rem]" :url="`/ph-h5/png/${getBgImage(i)}.png`" />
