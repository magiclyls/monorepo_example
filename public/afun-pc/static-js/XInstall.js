...
document.addEventListener('DOMContentLoaded', function () {
  ...
  (function (root, ns, factory) {
    'use strict'
    typeof module !== 'undefined' && module.exports ? module.exports = factory(ns, root) : typeof define === 'function' && define.amd ? define('detect-zoom', function () {
    ...
    var mediaQueryBinarySearch = function (property, unit, a, b, maxIter, epsilon) {
      function binarySearch (a, b, maxIter) {
        var mid = (a + b) / 2
        if (maxIter <= 0 || epsilon > b - a) return mid
    ...
    var detectFunction = (function () {
      var func = fallback
      return isNaN(screen.logicalXDPI) || isNaN(screen.systemXDPI) ? window.navigator.msMaxTouchPoints ? func = ie10 : 'orientation' in window && typeof document.body.style.webkitMarquee === 'string' ? func = webkitMobile : typeof document.body.style.webkitMarquee === 'string' ? func = webkit : navigator.userAgent.indexOf('Opera') >= 0 ? func = opera11 : window.devicePixelRatio ? func = firefox18 : firefox4().zoom > 0.001 && (func = firefox4) : func = ie8, func
    }())
    return {
      zoom: function () {
...
var XInstall = {
  'getInstall': function (appid, data, callback, ua = '') {
    ...
    if (os == 'iOS') {
      ...
      this.getInstallAjax(appid, data, callback, ua, md, os, 2, model, version)
    } else if (os == 'AndroidOS') {
      if (navigator.userAgentData) {
        navigator.userAgentData.getHighEntropyValues(['model', 'platformVersion']).then((uaData) => {
          model = uaData.model
          version = uaData.platformVersion
          this.getInstallAjax(appid, data, callback, ua, md, os, 1, model, version)
        })
      } else {
        var sssArr = ua.split(';')
  ...
  'getInstallAjax': function (appid, data, callback, ua, md, os, type, model, version) {
    var print = getUUID('Cora0105!CRIMson0824@#679Cora0105!CRIMson0824@#679Cora0105!CRIMson0824@#679')
